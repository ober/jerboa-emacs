      ...
      (if (>= i len)
        ...
        (let ((ch (string-ref str i)))
          (cond
                 ...
                 (cond
                   ...
                   ((char=? next #\])
                    (let skip-osc ((j (+ i 2)))
                      (cond
                        ((>= j len) (loop j))
                        ((char=? (string-ref str j) #\alarm) (loop (+ j 1)))
                        ((and (char=? (string-ref str j) #\esc)
                              (< (+ j 1) len)
                              (char=? (string-ref str (+ j 1)) #\\))
                         (loop (+ j 2)))
                        (else (skip-osc (+ j 1))))))
                   ;; Two-char escape: ESC ( ESC ) ESC # etc.
            ...
            ;; Strip other control chars (except newline and tab)
            ((and (char<? ch #\space)
                  (not (char=? ch #\newline))
                  (not (char=? ch #\tab)))
             (loop (+ i 1)))
...
(def (compilation-run-command! app cmd)
  ...
  (let* ((echo (app-state-echo app))
         ...
         (buf (or (buffer-by-name "*compilation*")
                  (qt-buffer-create! "*compilation*" ed #f))))
    ;; Show compilation buffer immediately with progress message
    (qt-buffer-attach! ed buf)
    (set! (qt-edit-window-buffer (qt-current-window fr)) buf)
    ...
    (spawn-worker 'compile
      (lambda ()
        ;; Close inherited fds (3-99) before exec to avoid blocking REPL.
        ;; Use bash exec with fd closing, then run the actual command.
        (let ((shell-cmd (string-append
        ...
        (let ((result (with-catch
                        (lambda (e) "")
                        (lambda ()
                          (call-with-input-file outfile
                            (lambda (p) (get-string-all p)))))))
          ;; Push the UI update to the primordial thread
          (ui-queue-push!
            (lambda ()
              (with-catch
                (lambda (e)
