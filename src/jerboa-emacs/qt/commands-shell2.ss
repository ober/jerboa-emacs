        ...
        (if (and (> (string-length trimmed) 0)
          ...
          (let* ((pr-num (let loop ((i 0) (acc ""))
                           (if (or (>= i (string-length trimmed))
                             ...
                             acc
                             (loop (+ i 1) (string-append acc (string (string-ref trimmed i)))))))
                 (buf (current-qt-buffer app))
                 (path (buffer-file-path buf))
                 (dir (if path (path-directory path) (current-directory))))
            (let-values (((p-stdin p-stdout p-stderr pid)
                          (open-process-ports
                            (string-append "cd " (shell-quote-arg dir)
                              " && gh pr view " pr-num " --web 2>/dev/null")
                            (buffer-mode block) (native-transcoder))))
              (close-port p-stdin) (close-port p-stdout) (close-port p-stderr)
              (echo-message! echo (string-append "Opening PR #" pr-num " in browser"))))
      ...
      (let* ((buf (current-qt-buffer app))
        ...
        (let-values (((p-stdin p-stdout p-stderr pid)
          ...
          (let* ((output (let loop ((lines '()))
                 ...
                 (ed (current-qt-editor app))
                 (fr (app-state-frame app))
                 (diff-buf (qt-buffer-create! (string-append "*PR #" pr-num " Diff*") ed #f)))
            ...
            (qt-plain-text-edit-set-cursor-position! ed 0)
            (qt-highlight-diff! ed)
            (echo-message! echo (string-append "*PR #" pr-num " Diff*"))))))))

;;;============================================================================
;;; Display-buffer-alist: rules for where buffers appear
;;;============================================================================

(def *display-buffer-rules* '())
...
(def (cmd-display-buffer-add-rule app)
  ...
  (let* ((echo (app-state-echo app))
    ...
    (when (and pattern (> (string-length pattern) 0))
      (let ((action-str (qt-echo-read-string-with-completion app "Action: "
                          '("same-window" "other-window" "bottom-window"))))
        (when action-str
          (let ((action (cond
                          ((string=? action-str "same-window") 'same-window)
                          ((string=? action-str "other-window") 'other-window)
                          ((string=? action-str "bottom-window") 'bottom-window)
                          (else 'same-window))))
            (display-buffer-add-rule! pattern action)
            (echo-message! echo
              (string-append "Rule added: " pattern " -> " (symbol->string action)))))))))

(def (cmd-display-buffer-list-rules app)
  "List all display buffer rules."
  (let* ((echo (app-state-echo app))
         (lines (map (lambda (rule)
                       (string-append "  " (car rule) " -> " (symbol->string (cdr rule))))
                     *display-buffer-rules*))
         (text (if (null? lines) "No display buffer rules configured."
                 (string-append "Display Buffer Rules:\n" (string-join lines "\n")))))
...
(def (cmd-project-vterm app)
  ...
  (let* ((echo (app-state-echo app))
         ...
         (name (string-append "*term:" (path-strip-directory root) "*"
                 (if (null? existing) ""
                   (string-append "-" (number->string (+ 1 (length existing))))))))
    ;; Create a new terminal buffer for this project
    (let* ((ed (current-qt-editor app))
           (fr (app-state-frame app))
           (buf (qt-buffer-create! name ed #f)))
      ...
      (qt-buffer-attach! ed buf)
      (set! (qt-edit-window-buffer (qt-current-window fr)) buf)
      ;; Track this terminal for the project
      (hash-put! *project-terminals* root (cons name existing))
      ;; Initialize terminal in project directory
      (qt-plain-text-edit-set-text! ed
        (string-append "Terminal for project: " root "\n$ "))
      (qt-plain-text-edit-move-cursor! ed QT_CURSOR_END)
      (echo-message! echo (string-append "Project terminal: " name)))))
...
(def (cmd-project-vterm-toggle app)
  ...
  (let* ((echo (app-state-echo app))
    ...
    (if (null? terms)
      ...
      ;; Find the current buffer; if it's a project terminal, cycle to next
      (let* ((cur-buf (current-qt-buffer app))
             (cur-name (buffer-name cur-buf))
             (idx (let loop ((ts terms) (i 0))
                    (if (null? ts) #f
                      (if (string=? (car ts) cur-name) i
                        (loop (cdr ts) (+ i 1))))))
             (next-name (if idx
                          (list-ref terms (modulo (+ idx 1) (length terms)))
                          (car terms)))
        ...
        (if next-buf
          (let* ((ed (current-qt-editor app))
            ...
            (set! (qt-edit-window-buffer (qt-current-window fr)) next-buf)
            (echo-message! echo next-name))
          (cmd-project-vterm app))))))

;;;============================================================================
;;; Breadcrumb: file path + function context header line
;;;============================================================================
...
(def (breadcrumb-find-function-at-point ed)
  ...
  (let* ((pos (qt-plain-text-edit-cursor-position ed))
         ...
         (len (string-length text)))
    (if (= len 0) #f
      ;; Search backward for def/define/defun/defclass/defstruct
      (let loop ((p (min pos (- len 1))))
        (if (< p 0) #f
          (if (and (char=? (string-ref text p) #\()
                   (< (+ p 5) len))
            ;; Check if this is a definition form
            (let ((after (substring text (+ p 1) (min len (+ p 20)))))
              (cond
                ((string-prefix? "def " after)
                 (let ((rest (substring after 4 (string-length after))))
                   (let ((name (breadcrumb-extract-name rest)))
                     (or name (loop (- p 1))))))
                ((string-prefix? "define " after)
                 (let ((rest (substring after 7 (string-length after))))
                   (let ((name (breadcrumb-extract-name rest)))
                     (or name (loop (- p 1))))))
                ((string-prefix? "defun " after)
                 (let ((rest (substring after 6 (string-length after))))
                   (let ((name (breadcrumb-extract-name rest)))
                     (or name (loop (- p 1))))))
                ((string-prefix? "defclass " after)
                 (let ((rest (substring after 9 (string-length after))))
                   (let ((name (breadcrumb-extract-name rest)))
                     (or name (loop (- p 1))))))
                ((string-prefix? "defstruct " after)
                 (let ((rest (substring after 10 (string-length after))))
                   (let ((name (breadcrumb-extract-name rest)))
                     (or name (loop (- p 1))))))
                (else (loop (- p 1)))))
            (loop (- p 1))))))))
...
(def (breadcrumb-extract-name text)
  ...
  (let ((trimmed (string-trim text)))
    (if (= (string-length trimmed) 0) #f
      ;; Handle (def (name args) ...) pattern
      (let* ((start (if (char=? (string-ref trimmed 0) #\() 1 0))
             (s (if (> start 0) (substring trimmed 1 (string-length trimmed)) trimmed)))
        (let loop ((i 0) (acc ""))
