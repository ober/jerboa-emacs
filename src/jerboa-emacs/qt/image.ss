...
(def (qt-show-image-buffer! editor buf)
  ...
  (with-catch
    (lambda (e)
      (verbose-log! "IMG: ERROR in qt-show-image-buffer!: "
          ...
          (let* ((state (hash-get *image-buffer-state* buf))
            ...
            (when state
              (let* ((pixmap (list-ref state 0))
                ...
                (verbose-log! "IMG: display " (number->string new-w) "x" (number->string new-h)
                              " zoom=" (number->string (inexact->exact (round (* zoom 100)))) "%")
                (when (and (> new-w 0) (> new-h 0))
                  ;; Skip scaling when zoom=1.0 — use original pixmap directly
                  (if (= zoom 1.0)
                    (begin
                      (qt-label-set-pixmap! label pixmap)
                      (qt-widget-set-minimum-size! label orig-w orig-h))
