...
;; ============================================================
;; Round 146 — ERC extended, Rcirc, SX (batch 2)
;; ============================================================

(def (cmd-erc-netsplit-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-netsplit)
    (if (mode-enabled? app 'erc-netsplit)
      (echo-message! echo "ERC netsplit mode enabled")
      (echo-message! echo "ERC netsplit mode disabled"))))

(def (cmd-erc-ring-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-ring)
    (if (mode-enabled? app 'erc-ring)
      (echo-message! echo "ERC ring mode enabled (input history)")
      (echo-message! echo "ERC ring mode disabled"))))

(def (cmd-erc-pcomplete-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-pcomplete)
    (if (mode-enabled? app 'erc-pcomplete)
      (echo-message! echo "ERC pcomplete mode enabled")
      (echo-message! echo "ERC pcomplete mode disabled"))))

(def (cmd-erc-move-to-prompt-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-move-to-prompt)
    (if (mode-enabled? app 'erc-move-to-prompt)
      (echo-message! echo "ERC move-to-prompt mode enabled")
      (echo-message! echo "ERC move-to-prompt mode disabled"))))

(def (cmd-erc-noncommands-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-noncommands)
    (if (mode-enabled? app 'erc-noncommands)
      (echo-message! echo "ERC noncommands mode enabled")
      (echo-message! echo "ERC noncommands mode disabled"))))

(def (cmd-erc-readonly-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-readonly)
    (if (mode-enabled? app 'erc-readonly)
      (echo-message! echo "ERC readonly mode enabled")
      (echo-message! echo "ERC readonly mode disabled"))))

(def (cmd-erc-irccontrols-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'erc-irccontrols)
    (if (mode-enabled? app 'erc-irccontrols)
      (echo-message! echo "ERC IRC controls mode enabled")
      (echo-message! echo "ERC IRC controls mode disabled"))))

(def (cmd-rcirc-track-minor-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'rcirc-track)
    (if (mode-enabled? app 'rcirc-track)
      (echo-message! echo "Rcirc track minor mode enabled")
      (echo-message! echo "Rcirc track minor mode disabled"))))

(def (cmd-rcirc-multiline-minor-mode app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'rcirc-multiline)
    (if (mode-enabled? app 'rcirc-multiline)
      (echo-message! echo "Rcirc multiline minor mode enabled")
      (echo-message! echo "Rcirc multiline minor mode disabled"))))
...
;; Round 246 — More mode toggles, Quickrun, String-edit (10 in final)
(def (cmd-marginalia-mode-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'marginalia-real)
    (if (mode-enabled? app 'marginalia-real)
      (echo-message! echo "Marginalia mode enabled (rich annotations)")
      (echo-message! echo "Marginalia mode disabled"))))

(def (cmd-marginalia-describe-commands app)
  (let* ((echo (app-state-echo app)))
    (echo-message! echo "Marginalia: describing commands")))

(def (cmd-minimap-mode-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'minimap-real)
    (if (mode-enabled? app 'minimap-real)
      (echo-message! echo "Minimap mode enabled")
      (echo-message! echo "Minimap mode disabled"))))

(def (cmd-nano-theme-real app)
  (let* ((echo (app-state-echo app)))
    (echo-message! echo "Nano theme applied")))

(def (cmd-orderless-mode-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'orderless-real)
    (if (mode-enabled? app 'orderless-real)
      (echo-message! echo "Orderless completion mode enabled")
      (echo-message! echo "Orderless completion mode disabled"))))

(def (cmd-orderless-filter-demo app)
  (let* ((echo (app-state-echo app)))
    (echo-read-string echo "Filter pattern: "
      (lambda (pat)
        (echo-message! echo (str "Orderless: filtering with '" pat "'"))))))

(def (cmd-olivetti-mode-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'olivetti-real)
    (if (mode-enabled? app 'olivetti-real)
      (echo-message! echo "Olivetti mode enabled (centered text)")
      (echo-message! echo "Olivetti mode disabled"))))

(def (cmd-page-break-lines-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'page-break-lines-real)
    (if (mode-enabled? app 'page-break-lines-real)
      (echo-message! echo "Page break lines mode enabled")
      (echo-message! echo "Page break lines mode disabled"))))

(def (cmd-pixel-scroll-precision-mode-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'pixel-scroll-precision-real)
    (if (mode-enabled? app 'pixel-scroll-precision-real)
...
(def (cmd-save-place-mode-real app)
  (let* ((echo (app-state-echo app)))
    (toggle-mode! app 'save-place-real)
