...
(import :std/sugar
        ...
        (only-in :jerboa-emacs/editor-extra-vcs tui-git-run cmd-man)
        (only-in :jerboa-emacs/editor-extra-media2
                 cmd-hippie-expand-file tui-get-or-create-buffer)
        (only-in :jerboa-emacs/editor-core string->alien/nul
                 tui-rows tui-cols alien/nul->string
                 string->alien bytevector->alien cons->alien
                 SCI_GETTEXTRANGE SCI_SETMARGINLEFT SCI_SETMARGINRIGHT
                 SCI_INDICSETALPHA SCI_MOVESELECTEDLINESUP
...
;; Helpers not imported from other modules
(def (shell-quote s)
...
(def (switch-to-buffer frame buf)
  "Attach buf to the current window of frame. Creates a document if buf has none yet."
  (let* ((win (current-window frame))
         (ed (edit-window-editor win)))
    (when (= (buffer-doc-pointer buf) 0)
      (let ((doc (send-message ed SCI_CREATEDOCUMENT 0 0)))
        (buffer-doc-pointer-set! buf doc)
        (buffer-list-add! buf)))
    (buffer-attach! ed buf)
    (set! (edit-window-buffer win) buf)))

(def (create-buffer name)
  "Create a new scratch buffer with deferred document creation."
  (or (buffer-by-name name)
      (make-buffer name #f 0 #f #f #f #f)))
        ...
        (when (and confirm (string=? (string-trim confirm) "yes"))
          (with-catch
            ...
            (lambda ()
              ...
              (let ((ed (edit-window-editor win))
                    (others (filter (lambda (b) (not (eq? b buf))) (buffer-list))))
                (when (pair? others)
                  (buffer-attach! ed (car others))
                  (set! (edit-window-buffer win) (car others))))
...
(def (cmd-find-file-at app file line-num)
  ...
  (let* ((frame (app-state-frame app))
         (win (current-window frame))
         (ed (edit-window-editor win))
         (buf (or (find-buffer-by-file-path file)
                  (if (file-exists? file)
                    (let* ((name (path-strip-directory file))
                           (b (buffer-create! name ed file))
                           (content (with-catch (lambda (e) "") (lambda () (read-file-string file)))))
                      (buffer-attach! ed b)
                      (set! (edit-window-buffer win) b)
                      (editor-set-text ed content)
                      b)
                    (create-buffer (str "*" file "*"))))))
    (switch-to-buffer frame buf)
    (let ((ed2 (edit-window-editor (current-window frame))))
      (when line-num
        (let ((pos (send-message ed2 SCI_POSITIONFROMLINE (- line-num 1) 0)))
          (editor-goto-pos ed2 pos))))))
    ...
    (if (or (not file) (string=? file ""))
      ...
      (if (not (file-exists? file))
        ...
        (let* ((content (read-file-string file))
          ...
          (buffer-file-path-set! new-buf file)
          (switch-to-buffer frame new-buf)
          (let ((ed (edit-window-editor (current-window frame))))
            (editor-set-text ed content))
...
(def (cmd-set-visited-file-name app)
  (let* ((buf (current-buffer-from-app app))
    ...
    (if (or (not new-file) (string=? new-file ""))
      (echo-message! echo "No file specified")
      (begin
        (buffer-file-path-set! buf new-file)
...
(def (cmd-highlight-lines-matching-regexp app)
  (let* ((buf (current-buffer-from-app app))
    ...
    (if (or (not pattern) (string=? pattern ""))
      (echo-message! echo "No pattern specified")
      (let* ((total-lines (editor-get-line-count ed)))
...
(def (cmd-apropos-command app)
  (let* ((echo (app-state-echo app))
    ...
    (if (or (not pattern) (string=? pattern ""))
      (echo-message! echo "No pattern specified")
      (let* ((cmds (hash-keys *all-commands*))
    ...
    (for-each (lambda (buf)
                (let ((file (buffer-file-path buf)))
                  (when (and file (not (string=? file "")))
                    (let ((text (editor-get-text (current-editor app))))
