#!chezscheme
;;; keymap.sls — TUI Keybinding Adapter
;;;
;;; Ported from gerbil-emacs/keymap.ss
;;; Converts termbox key events to Emacs key strings.
;;; Keymap data structures and bindings live in (jerboa-emacs core);
;;; this module provides only the TUI-specific conversion layer.

(library (jerboa-emacs keymap)
  (export
    key-event->string
    key-state-feed!)
  ...
  ;;; Checks mode-specific keymaps before the global keymap.
  ;;; The optional current-buffer parameter enables mode keymap lookup.
  (define key-state-feed!
    (case-lambda
      ((state ev)
       (key-state-feed! state ev #f))
      ((state ev current-buffer)
