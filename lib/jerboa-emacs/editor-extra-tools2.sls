...
;;; Ported from gerbil-emacs/editor-extra-tools2.ss to R6RS Chez Scheme.

(library (jerboa-emacs editor-extra-tools2)
  (export
    cmd-ace-window
    cmd-bookmark-bmenu-list
    cmd-calculator
    cmd-calendar-goto-date
    cmd-calendar-holidays
    cmd-canonically-space-region
    cmd-compare-windows
    cmd-copy-rectangle-to-register
    cmd-count-words-line
    ...
    cmd-isearch-toggle-case-fold
    cmd-isearch-toggle-regexp
    ...
    cmd-org-footnote-new
    cmd-raise-frame
    cmd-recenter-other-window
    cmd-rectangle-mark-mode
    cmd-reverse-words-in-region
    cmd-scroll-bar-mode
    cmd-scroll-down-other-window
    cmd-scroll-up-other-window
    cmd-semantic-mode
    cmd-server-force-delete
    cmd-session-restore
    cmd-session-save
    cmd-set-buffer-mode
    cmd-set-face-attribute
    cmd-set-tab-width
    cmd-shell-command-to-string
    cmd-sort-words-in-line
    cmd-subword-mode
    cmd-superword-mode
    cmd-tabify-region
    cmd-tags-apropos
    cmd-tags-query-replace
    cmd-tags-search
    cmd-toggle-auto-save-visited
    cmd-toggle-caret-style
    cmd-toggle-colon-double-space
    cmd-toggle-comment-auto-fill
    cmd-toggle-cursor-blink
    cmd-toggle-delete-pair-blink
    cmd-toggle-enable-recursive-minibuffers
    cmd-toggle-global-aggressive-indent
    cmd-toggle-global-beacon
    cmd-toggle-global-centered-cursor
    cmd-toggle-global-color-identifiers
    cmd-toggle-global-counsel
    cmd-toggle-global-deadgrep
    cmd-toggle-global-dimmer
    cmd-toggle-global-focus
    cmd-toggle-global-hl-todo
    cmd-toggle-global-org-cliplink
    cmd-toggle-global-org-download
    cmd-toggle-global-org-journal
    cmd-toggle-global-org-noter
    cmd-toggle-global-org-present
    cmd-toggle-global-org-roam
    cmd-toggle-global-org-super-agenda
    cmd-toggle-global-origami
    cmd-toggle-global-prescient
    cmd-toggle-global-prettify
    cmd-toggle-global-projectile-ripgrep
    cmd-toggle-global-ripgrep
    cmd-toggle-global-swiper
    cmd-toggle-global-wgrep
    cmd-toggle-header-line
    cmd-toggle-highlight-changes
    cmd-toggle-hl-todo
    cmd-toggle-line-spacing
    cmd-toggle-prettify-symbols
    cmd-toggle-ring-bell-function
    cmd-toggle-selection-mode
    cmd-toggle-sentence-end-double-space
    cmd-toggle-show-paren-when-point-inside
    cmd-toggle-use-dialog-box
    cmd-toggle-use-short-answers
    cmd-toggle-virtual-space
    cmd-tramp-cleanup-connections
    cmd-unhighlight-regexp
    cmd-untabify-region
    cmd-visit-tags-table
    cmd-what-tab-width
    cmd-whitespace-toggle-options
    cmd-window-divider-mode
    cmd-window-restore-layout
    cmd-window-save-layout
    cmd-yas-insert-snippet
    cmd-yas-new-snippet
    cmd-yas-visit-snippet-file
    dap-gdb-send!
    dap-set-pending-breakpoints!
    dap-show-output!
    diff-refine-words
    fill-words
    org-next-footnote-number
    parse-ctags-file
    yas-expand-snippet
    yas-get-mode)
...
;; Rectangle extras
(define (cmd-rectangle-mark-mode app)
...
(define (cmd-number-to-register app)
  "Store a number in a register."
  (let ((reg (app-read-string app "Register (a-z): ")))
    (when (and reg (not (string=? reg "")))
...
(define (cmd-isearch-toggle-case-fold app)
...
(define (cmd-isearch-toggle-regexp app)
...
(define (cmd-tags-query-replace app)
...
(define (cmd-visit-tags-table app)
    ...
    (with-catch
      ...
      (lambda ()
        (let* ((tags-path (string-append root "/tags")))
          (run-process (list "ctags" "-R" "-o" tags-path root))
    ...
    (let ((pattern (app-read-string app "Tags apropos (regexp): ")))
      (when (and pattern (not (string=? pattern "")))
        (let* ((matches (filter (lambda (name) (string-contains name pattern))
               ...
               (sorted (list-sort string<? matches))
               (text (if (null? sorted)
...
(define (org-next-footnote-number text)
...
(define (cmd-org-footnote-new app)
...
(define (cmd-org-footnote-goto app)
...
;; Whitespace extras
(define (cmd-whitespace-toggle-options app)
...
;; Highlight
(define (cmd-highlight-regexp app)
...
(define (cmd-unhighlight-regexp app)
...
;; Emacs server / client
(define (cmd-server-force-delete app)
...
(define (cmd-diff-refine-hunk app)
...
(define (cmd-ediff-directories app)
...
(define (cmd-display-column-number app)
...
(define (cmd-set-tab-width app)
  "Set tab width."
  (let ((width (app-read-string app "Tab width: ")))
    (when (and width (not (string=? width "")))
...
(define (cmd-display-cursor-position app)
...
(define (cmd-toggle-line-spacing app)
...
(define (cmd-toggle-selection-mode app)
...
(define (cmd-toggle-virtual-space app)
...
(define (cmd-toggle-caret-style app)
...
;; Buffer comparison
(define (cmd-compare-windows app)
...
(define (cmd-list-faces-display app)
...
;; Calendar extras
(define (cmd-calendar-goto-date app)
...
(define (cmd-erc app)
  ...
  (let* ((echo (app-state-echo app))
    ...
    (let ((srv (if (or (not server) (string=? server "")) "irc.libera.chat" server)))
      (let ((nick (app-read-string app "Nickname: ")))
        (if (or (not nick) (string=? nick ""))
...
;; TRAMP extras
(define (cmd-tramp-cleanup-connections app)
  "Clean up TRAMP connections — clears SSH control sockets."
  (guard (e [#t (echo-message! (app-state-echo app) "No SSH connections to clean")])
    (run-process (list "ssh" "-O" "exit" "-S" "~/.ssh/cm_%r@%h:%p" "dummy"))
...
(define (dap-show-output! app lines)
...
(define (dap-set-pending-breakpoints! app)
...
(define (cmd-dap-breakpoint-toggle app)
...
(define (yas-expand-snippet ed template)
...
(define (cmd-yas-insert-snippet app)
...
(define (cmd-yas-new-snippet app)
...
(define (cmd-yas-visit-snippet-file app)
...
(define (cmd-generate-password app)
  ...
  (let* ((ed (current-editor app))
         ...
         (pw (let ((out (open-output-string)))
               (let loop ((i 0))
                 (when (< i len)
                   (write-char
                     (string-ref chars (random chars-len)) out)
                   (loop (+ i 1))))
...
(define (cmd-insert-sequential-numbers app)
...
(define (cmd-untabify-region app)
...
(define (cmd-shell-command-to-string app)
...
(define (cmd-toggle-highlight-changes app)
...
(define (cmd-window-save-layout app)
...
(define (cmd-window-restore-layout app)
...
(define (cmd-set-buffer-mode app)
        ...
        (if canonical
          (begin
            (let ((buf (current-buffer-from-app app)))
              (buffer-lexer-lang-set! buf canonical))
...
(define (cmd-canonically-space-region app)
...
(define (cmd-toggle-cursor-blink app)
...
(define (cmd-recenter-other-window app)
...
(define (cmd-scroll-up-other-window app)
...
(define (cmd-scroll-down-other-window app)
...
(define (cmd-toggle-header-line app)
...
(define (cmd-toggle-auto-save-visited app)
...
(define (cmd-reverse-words-in-region app)
...
(define (cmd-insert-separator-line app)
...
(define (cmd-sort-words-in-line app)
...
(define (cmd-toggle-delete-pair-blink app)
...
(define (cmd-toggle-show-paren-when-point-inside app)
...
(define (cmd-toggle-enable-recursive-minibuffers app)
...
(define (cmd-toggle-use-dialog-box app)
...
(define (cmd-toggle-use-short-answers app)
...
(define (cmd-toggle-ring-bell-function app)
...
(define (cmd-toggle-sentence-end-double-space app)
...
(define (cmd-toggle-colon-double-space app)
...
(define (cmd-toggle-comment-auto-fill app)
...
(define (cmd-toggle-global-prettify app)
...
(define (cmd-toggle-global-hl-todo app)
...
(define (cmd-toggle-global-color-identifiers app)
...
(define (cmd-toggle-global-aggressive-indent app)
...
(define (cmd-toggle-global-origami app)
...
(define (cmd-toggle-global-centered-cursor app)
...
(define (cmd-toggle-global-beacon app)
...
(define (cmd-toggle-global-dimmer app)
...
(define (cmd-toggle-global-focus app)
...
(define (cmd-toggle-global-wgrep app)
...
(define (cmd-toggle-global-deadgrep app)
...
(define (cmd-toggle-global-ripgrep app)
...
(define (cmd-toggle-global-projectile-ripgrep app)
...
(define (cmd-toggle-global-counsel app)
...
(define (cmd-toggle-global-swiper app)
...
(define (cmd-toggle-global-prescient app)
...
(define (cmd-toggle-global-org-roam app)
...
(define (cmd-toggle-global-org-journal app)
...
(define (cmd-toggle-global-org-super-agenda app)
...
(define (cmd-toggle-global-org-noter app)
...
(define (cmd-toggle-global-org-download app)
...
(define (cmd-toggle-global-org-cliplink app)
...
(define (cmd-toggle-global-org-present app)
...
(define (cmd-delete-horizontal-space app)
...
(define (cmd-copy-rectangle-to-register app)
...
(define (cmd-session-restore app)
