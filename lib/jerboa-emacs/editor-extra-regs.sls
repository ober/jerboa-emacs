...
;;; Ported from gerbil-emacs/editor-extra-regs.ss to R6RS Chez Scheme.

(library (jerboa-emacs editor-extra-regs)  (export
    cmd-apropos-emacs
    cmd-butterfly
    cmd-cancel-debug-on-entry
    cmd-check-parens
    cmd-count-lines-page
    cmd-cua-mode
    cmd-debug-on-entry
    cmd-delete-directory
    cmd-desktop-save-mode
    cmd-dired-do-chown
    cmd-dired-do-shell-command
    cmd-dired-hide-details
    cmd-dired-jump
    cmd-dired-up-directory
    cmd-eww-add-bookmark
    cmd-eww-list-bookmarks
    cmd-find-dired
    cmd-find-name-dired
    cmd-forge-create-pr
    cmd-forge-list-issues
    cmd-forge-list-prs
    cmd-forge-view-pr
    ...
    cmd-org-babel-tangle
    cmd-org-capture
    cmd-org-capture-abort
    ...
    find-project-root
    forge-run-gh
    forge-show-in-buffer!
    get-tui-terminal-buffers
    iedit-count-whole-word
    iedit-get-word-at-point
    iedit-word-char?
    magit-run-git-tui
    register-batch12-aliases!
    register-batch6-commands!
    register-parity-commands!
    tui-switch-to-terminal!
  )
          ...
          (if (null? lines)
            ...
            (open-output-buffer app "*Apropos*"
              (string-append "Commands matching \"" pattern "\":\n\n"
                (string-join (list-sort string<? lines) "\n"))))))))

  ;;; --- Comment ---
  (define (cmd-indent-new-comment-line app)
  ...
  (define (cmd-org-capture-finalize app)
  ...
  (define (cmd-org-capture-abort app)
        ...
        (let* ((level (org-heading-stars-of-line (list-ref lines heading-line)))
          ...
          (if (null? labels)
            ...
            (let ((chosen (app-read-string app "Refile to: ")))
              (when (and chosen (not (string=? chosen "")))
  ...
  (define (cmd-org-insert-link app)
  ...
  ;;; --- Project ---
  (define (cmd-project-switch-to-buffer app)
  ...
  (define (cmd-project-kill-buffers app)
  ...
  ;;; --- Sort numeric ---
  (define (cmd-sort-numeric-fields app)
  ...
  ;;; --- Dired details ---
  (define *dired-hide-details* #f)
  (define (cmd-dired-hide-details app)
  ...
  ;;; --- Org babel commands ---
  (define (cmd-org-babel-execute-src-block app)
  ...
  (define (cmd-org-babel-tangle app)
  ...
  ;;; --- Help with tutorial ---
  (define (cmd-help-with-tutorial app)
  ...
  (define *cua-mode* #f)
  (define *cua-saved-bindings* '()) ;; saved (key . original-value) pairs

  (define (cmd-cua-mode app)
  ...
  ;;; --- Org archive subtree ---
  (define (cmd-org-archive-subtree app)
  ...
  ;;; --- Org toggle heading ---
  (define (cmd-org-toggle-heading app)
  ...
  ;;; --- Count lines page ---
  (define (cmd-count-lines-page app)
  ...
  ;;; --- Delete directory ---
  (define (cmd-delete-directory app)
  ...
  (define *debug-on-entry-list* '()) ;; list of symbol names being traced

  (define (cmd-debug-on-entry app)
  ...
  (define (cmd-cancel-debug-on-entry app)
  ...
  (define (register-batch12-aliases!)
    ;; Undo/redo aliases
  ...
  (define (iedit-count-whole-word text word)
  ...
  (define (cmd-eww-list-bookmarks app)
  ...
  (define (cmd-forge-list-issues app)
  ...
  (define (cmd-forge-create-pr app)
  ...
  (define *project-keymaps* (make-hash-table)) ;; project-root -> alist of (key . command)

  (define (cmd-project-keymap-load app)
  ...
  (define (register-batch6-commands!)
