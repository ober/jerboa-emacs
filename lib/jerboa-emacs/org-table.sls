#!chezscheme
;;; org-table.sls — Org table: parsing, alignment, cell navigation, formulas, CSV.
;;;
;;; Ported from gerbil-emacs/org-table.ss
;;; Backend-agnostic (Scintilla API only, no Qt imports).

(library (jerboa-emacs org-table)
  (export
    ;; Internal helpers
    editor-current-line
    ...
    org-table-row?
    org-table-separator?
    org-table-on-table-line?

    ;; Table parsing
    org-table-parse-row
    org-table-find-bounds
    org-table-get-rows
    ...
    org-table-next-cell
    org-table-prev-cell
    org-table-next-row-same-column
    org-table-next-data-line
    org-table-prev-data-line

    ;; Separator completion
    org-table-complete-separator
    ...
    org-table-insert-separator-line

    ;; Table sort
    org-table-sort

    ;; Table replace helper
    org-table-replace-rows
...
(def (org-table-sort ed col-num . rest)
  "Sort table by column col-num. Optional numeric? flag (default #f)."
  (let ((numeric? (if (pair? rest) (car rest) #f)))
    (let-values (((start end) (org-table-find-bounds ed)))
      (when (and start end)
        (let* ((rows (org-table-get-rows ed start end))
