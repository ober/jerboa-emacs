#!chezscheme
;;; org-export.sls — Org export: HTML, Markdown, LaTeX, plain text backends.
;;;
;;; Ported from gerbil-emacs/org-export.ss
;;; Backend-agnostic (pure string processing, no editor deps).

(library (jerboa-emacs org-export)
  (export
    ;; Inline markup
    org-export-inline
    html-escape
    ...
    ;; Block splitting
    org-split-into-blocks

    ;; Export options
    org-parse-export-options
    ...
    ;; Footnotes
    org-collect-footnotes
    org-export-footnote-ref
    org-export-footnotes-section
    org-replace-footnote-refs

    ;; Cross-references
    org-replace-cross-refs

    ;; Backend registry
    org-export-backends
    org-export-register-backend!
    org-export-unregister-backend!
    org-export-list-backends
    org-export-buffer

    ;; Table helper
    org-table-parse-row-simple)
  (import (except (chezscheme)
          ...
          (only (std srfi srfi-13)
                string-trim string-trim-both string-contains string-prefix? string-join
  ...
  ;; Gerbil had: (def (org-export-footnote-ref name backend counter (footnotes #f)))
  ;; Converted to positional optional via rest args.
  (def (org-export-footnote-ref name backend counter . rest)
    (let ((footnotes (if (pair? rest) (car rest) #f)))
      (case backend
  ...
  ;; Gerbil had: (def (org-replace-footnote-refs str backend counter-box used-box (footnotes #f)))
  ;; Converted to positional optional via rest args.
  (def (org-replace-footnote-refs str backend counter-box used-box . rest)
    (let ((footnotes (if (pair? rest) (car rest) #f)))
      (let loop ((s str))
  ...
  (def (org-export-backends) *org-export-backends*)
