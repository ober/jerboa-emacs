#!chezscheme
;;; editor-extra-vcs.sls — Version control, mail, sessions, macros,
;;; compilation, flyspell, multiple cursors, package management,
;;; customize, diff mode, and miscellaneous editing commands.
;;;
;;; Ported from gerbil-emacs/editor-extra-vcs.ss to R6RS Chez Scheme.

(library (jerboa-emacs editor-extra-vcs)
  (export
    ;; VC commands
    cmd-vc-register
    cmd-vc-dir
    ...
    cmd-woman
    ;; Macro extras
    cmd-apply-macro-to-region-lines
    cmd-edit-kbd-macro
    ;; Compilation
    cmd-recompile
    cmd-kill-compilation
    ;; Flyspell
    cmd-flyspell-auto-correct-word
    cmd-flyspell-goto-next-error
    ;; Multiple cursors
    mc-get-selection
    mc-find-all-positions
    ...
    cmd-customize-themes
    ;; Diff mode
    diff-parse-hunk-header
    diff-find-current-hunk
    cmd-diff-mode
    cmd-diff-apply-hunk
    cmd-diff-revert-hunk
    cmd-diff-goto-source
    ;; Fuzzy command
    cmd-execute-extended-command-fuzzy
    ;; Scratch
    cmd-scratch-with-mode
    ;; Buffer nav
    cmd-switch-to-buffer-other-window
    ;; Comment box / format / rename
    cmd-comment-box
    cmd-format-region
    cmd-rename-symbol
    cmd-isearch-occur
    ...
    cmd-toggle-comment-style
    ;; Flymake
    cmd-toggle-flymake-mode
    ;; Indent
    cmd-indent-for-tab
    cmd-dedent-region
    ;; Duplicate and comment
    cmd-duplicate-and-comment
    ;; Scratch message
    cmd-insert-scratch-message
    ;; Count lines
    cmd-count-lines-region
    ;; Cycle spacing
    cmd-cycle-spacing
    ;; Delete selection
    cmd-toggle-delete-selection
    ;; Word count
    cmd-toggle-word-count
    ;; Column ruler
    cmd-toggle-column-ruler
    ;; Shell here
    cmd-shell-here
    ;; Soft wrap
    cmd-toggle-soft-wrap
    ;; Whitespace cleanup
    cmd-toggle-whitespace-cleanup-on-save
    ;; Random line
    cmd-insert-random-line
    ;; Smart backspace
    cmd-smart-backspace
    ;; Line move visual
    cmd-toggle-line-move-visual
    ;; Batch 37 toggles
    cmd-toggle-highlight-indentation
    cmd-toggle-hungry-delete
    cmd-toggle-type-break
    cmd-insert-zero-width-space
    cmd-toggle-delete-trailing-on-save
    cmd-toggle-cursor-in-non-selected-windows
    cmd-toggle-blink-matching-paren
    cmd-toggle-next-error-follow
    cmd-insert-page-break
    ;; Batch 47 toggles
    cmd-toggle-auto-save-default
    cmd-toggle-make-pointer-invisible
    cmd-toggle-kill-whole-line
    cmd-toggle-set-mark-command-repeat-pop
    cmd-toggle-enable-local-variables
    cmd-toggle-enable-dir-local-variables
    cmd-toggle-ad-activate-all
    cmd-toggle-global-hi-lock-mode
    cmd-toggle-next-line-add-newlines
    ;; Batch 61 toggles
    cmd-toggle-global-treemacs-icons
    cmd-toggle-global-all-the-icons-dired
    cmd-toggle-global-centaur-tabs
    cmd-toggle-global-awesome-tab
    cmd-toggle-global-tab-bar
    cmd-toggle-global-mini-frame
    cmd-toggle-global-vertico-posframe
    ;; Batch 70 toggles
    cmd-toggle-global-cmake-mode
    cmd-toggle-global-bazel-mode
    cmd-toggle-global-meson-mode
    cmd-toggle-global-ninja-mode
    cmd-toggle-global-groovy-mode
    cmd-toggle-global-kotlin-mode
    cmd-toggle-global-scala-mode
    ;; Highlight symbol nav
  ...
  (define (run-cmd-capture path-str args)
    (run-process (cons path-str args)))

  ;; Run a command with optional directory, capture stdout
  (define (run-cmd-capture/dir path-str args dir)
    (if dir
      (run-process (cons path-str args) 'directory: dir)
      (run-process (cons path-str args))))

  ;;;==========================================================================
  ;;; Multiple cursor state (module-level, not exported as set!-modified)
  ;;;==========================================================================

  (define *mc-selection* #f)
  (define *mc-positions* '())
  (define *mc-position-idx* 0)

  ;;;==========================================================================
  ;;; Toggle flags (module-level, not exported as set!-modified)
  ;;;==========================================================================

  (define *comment-style* 'line)
  (define *flymake-mode* #f)
  (define *delete-selection-mode* #t)
  (define *word-count-mode* #f)
  (define *column-ruler-mode* #f)
  (define *column-ruler-column* 80)
  (define *soft-wrap-mode* #f)
  (define *whitespace-cleanup-on-save* #f)
  (define *line-move-visual* #t)
  (define *highlight-indentation-mode* #f)
  (define *hungry-delete-mode* #f)
  (define *type-break-mode* #f)
  (define *delete-trailing-on-save* #f)
  (define *cursor-in-non-selected* #t)
  (define *blink-matching-paren* #t)
  (define *next-error-follow* #f)
  (define *auto-save-default* #t)
  (define *make-pointer-invisible* #t)
  (define *kill-whole-line* #f)
  (define *set-mark-command-repeat-pop* #f)
  (define *enable-local-variables* #t)
  (define *enable-dir-local-variables* #t)
  (define *ad-activate-all* #f)
  (define *global-hi-lock-mode* #f)
  (define *next-line-add-newlines* #f)
  (define *global-treemacs-icons* #f)
  (define *global-all-the-icons-dired* #f)
  (define *global-centaur-tabs* #f)
  (define *global-awesome-tab* #f)
  (define *global-tab-bar* #f)
  (define *global-mini-frame* #f)
  (define *global-vertico-posframe* #f)
  (define *global-cmake-mode* #f)
  (define *global-bazel-mode* #f)
  (define *global-meson-mode* #f)
  (define *global-ninja-mode* #f)
  (define *global-groovy-mode* #f)
  (define *global-kotlin-mode* #f)
  (define *global-scala-mode* #f)
  ...
  (define (cmd-vc-register app)
    ...
    (let* ((fr (app-state-frame app))
      ...
      (if file
        (let ((result (guard (e (#t (call-with-string-output-port
                                      (lambda (p) (display e p)))))
                        (run-cmd-capture "git" (list "add" file)))))
          (echo-message! (app-state-echo app)
            (string-append "Registered: " (path-strip-directory file))))
        (echo-message! (app-state-echo app) "No file to register"))))

  (define (cmd-vc-dir app)
    "Show VC directory status."
    (let ((result (guard (e (#t "Error running git status"))
                    (let ((out (run-cmd-capture "git" '("status" "--short"))))
                      (if (string=? out "") "(clean)" out)))))
      (let* ((fr (app-state-frame app))
             (win (current-window fr))
  ...
  (define (cmd-vc-pull app)
    "Pull from remote repository."
    (let ((result (guard (e (#t (call-with-string-output-port
                                  (lambda (p) (display e p)))))
                    (run-cmd-capture "git" '("pull")))))
      (echo-message! (app-state-echo app)
        (string-append "git pull: " (if (> (string-length result) 60)
                                      (substring result 0 60)
                                      result)))))

  (define (cmd-vc-push app)
    "Push to remote repository."
    (let ((result (guard (e (#t (call-with-string-output-port
                                  (lambda (p) (display e p)))))
                    (run-cmd-capture "git" '("push")))))
      (echo-message! (app-state-echo app)
        (string-append "git push: " (if (> (string-length result) 60)
                                      (substring result 0 60)
                                      result)))))

  (define (cmd-vc-create-tag app)
    "Create a git tag."
    (let ((tag (app-read-string app "Tag name: ")))
      (when (and tag (not (string=? tag "")))
        (let ((result (guard (e (#t (call-with-string-output-port
                                      (lambda (p) (display e p)))))
                        (run-cmd-capture "git" (list "tag" tag)))))
          (echo-message! (app-state-echo app)
            (string-append "Created tag: " tag))))))

  (define (cmd-vc-print-log app)
    "Show full git log."
    (let ((result (guard (e (#t "Error running git log"))
                    (let ((out (run-cmd-capture "git" '("log" "--oneline" "-50"))))
                      (if (string=? out "") "(empty log)" out)))))
      (let* ((fr (app-state-frame app))
             (win (current-window fr))
             (ed (edit-window-editor win))
  ...
  (define (cmd-vc-stash app)
    "Stash current changes with git stash."
    (let* ((echo (app-state-echo app))
           (dir (vc-git-dir app)))
      (guard (e (#t (echo-message! echo
                      (string-append "git stash failed: "
                        (call-with-string-output-port
                          (lambda (p) (display e p)))))))
        (let* ((result (run-cmd-capture/dir "git" '("stash") dir)))
          (if (string=? result "")
            (echo-message! echo "git stash failed: (no output)")
            (echo-message! echo (string-append "Stash: " result)))))))

  (define (cmd-vc-stash-pop app)
    ...
    (let* ((echo (app-state-echo app))
      ...
      (guard (e (#t (echo-message! echo
                      (string-append "git stash pop failed: "
                        (call-with-string-output-port
                          (lambda (p) (display e p)))))))
        (let* ((result (run-cmd-capture/dir "git" '("stash" "pop") dir)))
          (if (string=? result "")
            (echo-message! echo "git stash pop failed: (no output)")
  ...
  (define (cmd-desktop-save app)
    ...
    (let* ((bufs (buffer-list))
      ...
      (guard (e (#t (echo-message! (app-state-echo app) "Error saving session")))
  ...
  (define (cmd-desktop-read app)
    ...
    (let ((session-file (string-append (or (getenv "HOME") ".") "/.gemacs-session")))
      (if (file-exists? session-file)
        (guard (e (#t (echo-message! (app-state-echo app) "Error reading session")))
  ...
  (define (cmd-man app)
    ...
    (let ((topic (app-read-string app "Man page: ")))
      (when (and topic (not (string=? topic "")))
        (let ((result (guard (e (#t (string-append "No man page for: " topic)))
                        (let ((out (run-process
                                     (list "/bin/sh" "-c"
                                       (string-append
                                         "MANPAGER=cat COLUMNS=80 TERM=dumb man "
                                         topic " 2>&1")))))
                          (if (string=? out "")
  ...
  (define (cmd-apply-macro-to-region-lines app)
    ...
    (let ((last-cmd (app-state-last-compile app)))
      (if last-cmd
        (begin
          ...
          (guard (e (#t (echo-error! (app-state-echo app) "Recompile failed")))
            (let* ((out (run-process (list "/bin/sh" "-c" last-cmd)))
  ...
  (define (cmd-flyspell-auto-correct-word app)
  ...
  (define (cmd-flyspell-goto-next-error app)
  ...
  (define (mc-find-all-positions ed pattern)
  ...
  (define (cmd-mc-mark-next-like-this app)
  ...
  (define (cmd-mc-mark-previous-like-this app)
  ...
  (define (cmd-mc-mark-all-like-this app)
            ...
            (let* ((row (- (frame-height fr) 1))
                   ...
                   (replacement (echo-read-string echo
                                  (string-append "Replace all (" (number->string (length positions))
                                                " matches) with: ")
                                  row width)))
              (when (and replacement (not (string=? replacement "")))
                ;; Replace from end to start to preserve positions
                (let ((sorted-positions (list-sort (lambda (a b) (> (car a) (car b))) positions)))
                  (for-each
                    (lambda (pos)
  ...
  (define (run-gerbil-pkg args)
    "Run gerbil pkg with given args. Returns output string."
    (guard (e (#t (string-append "Error: "
                    (call-with-string-output-port
                      (lambda (p) (display e p))))))
      (run-cmd-capture "gerbil" (cons "pkg" args))))

  (define (cmd-package-list-packages app)
  ...
  (define (cmd-package-refresh-contents app)
  ...
  (define (cmd-customize-group app)
  ...
  (define (cmd-customize-variable app)
        ...
        (let* ((sym (string->symbol name))
               ...
               (prompt (if current
                         (string-append "Value for " name " (current: "
                           (call-with-string-output-port (lambda (p) (write current p))) "): ")
                         (string-append "Value for " name ": ")))
               (val-str (app-read-string app prompt)))
          (when (and val-str (not (string=? val-str "")))
  ...
  (define (cmd-customize-themes app)
  ...
  (define (diff-parse-hunk-header line)
    "Parse a diff hunk header like @@ -start,count +start,count @@."
  ...
  (define (diff-find-current-hunk ed)
  ...
  (define (cmd-diff-apply-hunk app)
          ...
          (let loop ((i hunk-line) (hunk-lines '()))
            (if (>= i (length lines))
              ...
              (let* ((hunk-text (string-join (reverse hunk-lines) "\n"))
                ...
                (guard (e (#t (echo-error! echo "Failed to apply hunk")))
                  (call-with-output-file tmp-file
                    (lambda (p) (display hunk-text p))
                    'truncate)
                  (let ((out (run-cmd-capture "patch" (list "-p1" "--dry-run" "-i" tmp-file))))
                    (echo-message! echo (string-append "Patch output: "
                      (if (string=? out "") "ok" out))))))
  ...
  (define (cmd-diff-revert-hunk app)
          ...
          (let loop ((i hunk-line) (hunk-lines '()))
            (if (>= i (length lines))
              (let* ((hunk-text (string-join (reverse hunk-lines) "\n"))
                ...
                (guard (e (#t (echo-error! echo "Failed to revert hunk")))
                  (call-with-output-file tmp-file
                    (lambda (p) (display hunk-text p))
                    'truncate)
                  (let ((out (run-cmd-capture "patch" (list "-p1" "-R" "-i" tmp-file))))
                    (echo-message! echo (string-append "Reverted: "
                      (if (string=? out "") "ok" out))))))
              (let ((line (list-ref lines i)))
                (if (and (> i hunk-line) (string-prefix? "@@" line))
                  (loop (length lines) hunk-lines)
  ...
  (define (cmd-diff-goto-source app)
                   ...
                   (begin
                     ...
                     (let* ((hunk-line (diff-find-current-hunk ed))
                       ...
                       (let ((new-buf (buffer-create! (path-strip-directory clean-path) ed clean-path)))
                         (guard (e (#t (echo-error! echo "Cannot read file")))
                           (let ((content (call-with-input-file clean-path get-string-all)))
  ...
  (define (cmd-execute-extended-command-fuzzy app)
          ...
          (if exact-cmd
            ...
            (let* ((matches (filter (lambda (name) (fuzzy-match? input name))
                   ...
                   (scored (map (lambda (name) (cons (fuzzy-score input name) name))
                                matches))
                   (sorted (list-sort (lambda (a b) (> (car a) (car b))) scored)))
              (if (null? sorted)
  ...
  (define (cmd-switch-to-buffer-other-window app)
          ...
          (if (not buf)
            ...
            (let* ((wins (frame-windows fr))
              ...
              (let ((ed (edit-window-editor other-win)))
                (buffer-attach! ed buf)
                (edit-window-buffer-set! other-win buf)
                (echo-message! echo (string-append "Buffer: " choice)))))))))

  ;;;==========================================================================
  ;;; Comment box, format region, rename symbol, isearch occur, helm-mini
  ;;;==========================================================================

  (define (cmd-comment-box app)
    ...
    (let* ((ed (current-editor app))
      ...
      (if (= sel-start sel-end)
        ...
        (let* ((text (let ((full (editor-get-text ed)))
               ...
               (boxed (call-with-string-output-port
                        (lambda (p)
                          (display border p) (display "\n" p)
                          (for-each
                            (lambda (line)
                              (display ";; " p)
                              (display line p)
                              (display (make-string (- max-len (string-length line)) #\space) p)
                              (display "  " p) (display "\n" p))
                            lines)
                          (display border p) (display "\n" p)))))
                         ...
                         (lambda (name)
                           (let ((name-lower (string-downcase name)))
                             (if (string-contains name-lower query-lower)
                               ...
                               #f)))
                         names))
               (sorted (list-sort (lambda (a b) (< (cdr a) (cdr b))) scored)))
          (if (null? sorted)
            (echo-message! echo
  ...
  (define (cmd-toggle-comment-style app)
  ...
  (define (cmd-toggle-flymake-mode app)
  ...
  (define (cmd-duplicate-and-comment app)
  ...
  (define (cmd-insert-scratch-message app)
  ...
  ;;;==========================================================================
  ;;; Toggle commands (batch 32, 37, 47, 61, 70)
  ;;;==========================================================================

  (define (cmd-toggle-delete-selection app)
  ...
  (define (cmd-toggle-word-count app)
  ...
  (define (cmd-toggle-column-ruler app)
  ...
  (define (cmd-shell-here app)
    "Open a shell command in the current file's directory."
    (let* ((echo (app-state-echo app))
           (buf (current-buffer-from-app app))
           (filepath (buffer-file-path buf))
           (dir (if filepath (path-directory filepath) ".")))
      (guard (e (#t (echo-message! echo "Cannot open terminal")))
        (let ((term (cond
  ...
  (define (cmd-toggle-whitespace-cleanup-on-save app)
  ...
  (define (cmd-toggle-highlight-indentation app)
  ...
  (define (cmd-toggle-hungry-delete app)
  ...
  (define (cmd-toggle-type-break app)
  ...
  (define (cmd-insert-zero-width-space app)
  ...
  (define (cmd-toggle-delete-trailing-on-save app)
  ...
  (define (cmd-toggle-cursor-in-non-selected-windows app)
  ...
  (define (cmd-toggle-blink-matching-paren app)
  ...
  (define (cmd-toggle-next-error-follow app)
  ...
  (define (cmd-insert-page-break app)
  ...
  (define (cmd-toggle-auto-save-default app)
  ...
  (define (cmd-toggle-make-pointer-invisible app)
  ...
  (define (cmd-toggle-kill-whole-line app)
  ...
  (define (cmd-toggle-set-mark-command-repeat-pop app)
  ...
  (define (cmd-toggle-enable-local-variables app)
  ...
  (define (cmd-toggle-enable-dir-local-variables app)
  ...
  (define (cmd-toggle-ad-activate-all app)
  ...
  (define (cmd-toggle-global-hi-lock-mode app)
  ...
  (define (cmd-toggle-next-line-add-newlines app)
  ...
  (define (cmd-toggle-global-treemacs-icons app)
  ...
  (define (cmd-toggle-global-all-the-icons-dired app)
  ...
  (define (cmd-toggle-global-centaur-tabs app)
  ...
  (define (cmd-toggle-global-awesome-tab app)
  ...
  (define (cmd-toggle-global-tab-bar app)
  ...
  (define (cmd-toggle-global-mini-frame app)
  ...
  (define (cmd-toggle-global-vertico-posframe app)
  ...
  (define (cmd-toggle-global-cmake-mode app)
  ...
  (define (cmd-toggle-global-bazel-mode app)
  ...
  (define (cmd-toggle-global-meson-mode app)
  ...
  (define (cmd-toggle-global-ninja-mode app)
  ...
  (define (cmd-toggle-global-groovy-mode app)
  ...
  (define (cmd-toggle-global-kotlin-mode app)
  ...
  (define (cmd-toggle-global-scala-mode app)
  ...
  (define (cmd-highlight-symbol-next app)
      ...
      (when (not search)
        ...
        (let* ((text (editor-get-text ed))
          ...
          (when (< start end)
            ...
            (app-state-last-search-set! app search))))
      (if (not search)
  ...
  (define (cmd-highlight-symbol-prev app)
    ...
    (let ((url (app-read-string app "URL: ")))
      (when (and url (> (string-length url) 0))
        (let ((full-url (if (or (string-prefix? "http://" url) (string-prefix? "https://" url))
          ...
          (guard (e (#t (echo-error! (app-state-echo app) "Failed to open URL")))
            (system (string-append "xdg-open " full-url " &"))
            (echo-message! (app-state-echo app) (string-append "Opening: " full-url)))))))

  ;;;==========================================================================
  ;;; Magit parity from TUI
  ;;;==========================================================================

  (define (tui-git-run args)
    (guard (e (#t ""))
      (run-cmd-capture "git" args)))

  (define (tui-git-file-path app)
    (let* ((fr (app-state-frame app))
           (buf (edit-window-buffer (current-window fr))))
      (and buf (buffer-file-path buf))))

  (define (cmd-magit-refresh app)
