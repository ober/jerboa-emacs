...
;;; Source: src/jerboa-emacs/qt/commands-core.ss

(library (jerboa-emacs qt commands-core)
  ...
  (import
   ...
   (only (jerboa-emacs gsh-eshell) gsh-eshell-buffer?
     gsh-eshell-prompt eshell-history-prev eshell-history-next)
  ...
  (def (winner-snapshot-tree node)
       ...
       (cond
         [(split-leaf? node)
          `(leaf
             ,(buffer-name
                (qt-edit-window-buffer (split-leaf-edit-window node))))]
         [(split-node? node)
          `(node
             ,(split-node-orientation node)
  ...
  (def (winner-snapshot-leaf-names snapshot)
       ...
       (cond
         [(and (pair? snapshot) (eq? (car snapshot) 'leaf))
          (list (cadr snapshot))]
         [(and (pair? snapshot) (eq? (car snapshot) 'node))
          (apply
            append
            (map winner-snapshot-leaf-names (cddr snapshot)))]
         [else (list)]))
  (def (winner-current-config fr)
       ...
       (let ([cur-buf (buffer-name
         ...
         (cons (winner-snapshot-tree (qt-frame-root fr)) cur-buf)))
  (def (winner-save! fr)
       ...
       (let ([config (winner-current-config fr)])
         ...
         (when (> (length *winner-history*) *winner-max-history*)
           (set! *winner-history*
             (take *winner-history* *winner-max-history*)))
         (set! *winner-future* (list))))
         ...
         (cond
           ...
           [(< current-count desired-count)
            (let loop ([n (- desired-count current-count)])
              (when (> n 0) (qt-frame-split! fr) (loop (- n 1))))])
           ...
           (let loop ([ws wins] [ns leaf-names])
             (when (and (pair? ws) (pair? ns))
               (let* ([w (car ws)]
                 ...
                 (when (and target-buf
                            (not (string=?
                                   (buffer-name (qt-edit-window-buffer w))
                                   target-name)))
                   (qt-buffer-attach! (qt-edit-window-editor w) target-buf)
                   (qt-edit-window-buffer-set! w target-buf)))
               (loop (cdr ws) (cdr ns)))))
         (let* ([wins (qt-frame-windows fr)]
                [idx (let find-idx ([ws wins] [i 0])
                       (cond
                         [(null? ws) 0]
                         [(string=?
                            (buffer-name (qt-edit-window-buffer (car ws)))
                            cur-buf-name)
                          i]
  ...
  (def (cmd-winner-undo app)
       ...
       (if (null? *winner-history*)
           (echo-error!
             (app-state-echo app)
             "No further window configuration to undo")
           (let* ([fr (app-state-frame app)]
  ...
  (def (current-qt-buffer app)
       (qt-edit-window-buffer
         (qt-current-window (app-state-frame app))))
  (define *qt-app-ptr*--cell (vector #f))
  (define *tab-bar-visible*--cell (vector #t))
       ...
       (let ([clip (and *qt-app-ptr*
         ...
         (or clip
             (let ([kr (app-state-kill-ring app)])
               (and (pair? kr) (car kr))))))
  (define *current-theme*--cell (vector 'dark))
  (def (theme-color key)
       ...
       (let ([theme (theme-get *current-theme*)])
         (and theme
              (let ([pair (assoc key theme)]) (and pair (cdr pair))))))
  (def (load-theme! theme-name)
       ...
       (let ([theme (theme-get theme-name)])
         (unless theme (error "Unknown theme" theme-name))
         (face-clear!)
         (for-each
           (lambda (entry)
             (let ([face-name (car entry)] [props (cdr entry)])
               (when (and (pair? props) (keyword? (car props)))
  ...
  (def (theme-stylesheet)
       ...
       (let ([bg (or (theme-color 'bg) "#181818")]
         ...
         (string-append "QPlainTextEdit { background-color: " bg
          "; color: " fg ";" font-css " selection-background-color: "
          sel "; }" " QLabel { color: " echo-fg "; background: "
          echo-bg ";" font-css " }" " QMainWindow { background: " bg
          "; }" " QStatusBar { color: " ml-fg "; background: " ml-bg
          ";" font-css " }" " QLineEdit { background: " bg "; color: "
          fg "; border: none;" font-css " }"
  ...
  (def (apply-theme! app theme-name: (theme-name #f))
       ...
       (when theme-name (load-theme! theme-name))
       (when *qt-app-ptr*
  ...
  (def (buffer-names-mru)
       ...
       (let* ([all-names (map buffer-name (buffer-list))]
              [mru (filter
                     (lambda (n) (member n all-names))
                     *buffer-recent*)]
              [rest (filter (lambda (n) (not (member n mru))) all-names)])
         (append mru rest)))
  (define *global-auto-revert-mode*--cell (vector #f))
  (define *file-mtimes*--cell (vector (make-hash-table)))
  (define *auto-revert-tail-buffers*--cell
  ...
  (def (file-mtime-record! path)
       ...
       (when path
         (let ([mt (file-mtime path)])
           (when mt (hash-put! *file-mtimes* path mt)))))
         ...
         (let loop ([i 0])
           (when (< i (abs n))
             (qt-plain-text-edit-move-cursor!
               ed
               (if (>= n 0) QT_CURSOR_NEXT_CHAR QT_CURSOR_PREVIOUS_CHAR))
             (loop (+ i 1))))
       ...
       (let ([n (get-prefix-arg app)] [ed (current-qt-editor app)])
         ...
         (let loop ([i 0])
           (when (< i (abs n))
             (qt-plain-text-edit-move-cursor!
               ed
               (if (>= n 0) QT_CURSOR_PREVIOUS_CHAR QT_CURSOR_NEXT_CHAR))
             (loop (+ i 1))))
         (update-mark-region! app ed)))
       ...
       (let ([n (get-prefix-arg app)] [ed (current-qt-editor app)])
         (let loop ([i 0])
           (when (< i (abs n))
             (qt-plain-text-edit-move-cursor!
               ed
               (if (>= n 0) QT_CURSOR_NEXT_WORD QT_CURSOR_PREVIOUS_WORD))
             (loop (+ i 1))))
         (update-mark-region! app ed)))
       ...
       (let ([n (get-prefix-arg app)] [ed (current-qt-editor app)])
         (let loop ([i 0])
           (when (< i (abs n))
             (qt-plain-text-edit-move-cursor!
               ed
               (if (>= n 0) QT_CURSOR_PREVIOUS_WORD QT_CURSOR_NEXT_WORD))
             (loop (+ i 1))))
         (update-mark-region! app ed)))
       ...
       (let ([len (string-length text)])
         (and (> i 0)
              ...
              (let ([prev (string-ref text (- i 1))]
                ...
                (or (and (= direction 1)
                         (or (char=? cur #\_) (char=? cur #\-)))
                    (and (= direction -1)
                         (or (char=? prev #\_) (char=? prev #\-)))
                    (and (char-lower-case? prev) (char-upper-case? cur))
                    (and (char-alphabetic? prev)
                         (not (or (char-alphabetic? cur)
                                  (char-numeric? cur))))
                    (and (not (or (char-alphabetic? prev)
                                  (char-numeric? prev)))
                         (char-alphabetic? cur)))))))
               ...
               (if (not (paredit-delimiter? ch))
                   ...
                   (cond
                     [(or (= ch 40) (= ch 91) (= ch 123))
                      (and (< (+ pos 1) len)
                           (eqv?
                             (char->integer (string-ref text (+ pos 1)))
                             (auto-pair-char ch)))]
                     [(= ch 41)
                      (and (> pos 0)
                           (= (char->integer (string-ref text (- pos 1)))
         ...
         (cond
           ...
           [else
            (let* ([ed (current-qt-editor app)]
              ...
              (if (and *paredit-strict-mode*
                       (> pos 0)
                       (not (qt-paredit-strict-allow-delete?
                              ed
                              (- pos 1)
                              'backward)))
                  (echo-message!
       ...
       (let* ([text (qt-plain-text-edit-text ed)]
              ...
              [line-start (let loop ([i (- pos 1)])
                            (if (or (< i 0)
                                ...
                                (+ i 1)
                                (loop (- i 1))))])
         (let loop ([i line-start] [acc (list)])
           (if (and (< i text-len)
  ...
  (def (cmd-kill-line app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (= pos line-end)
             (let ([killed (qt-plain-text-edit-text-range
                             ed
                             pos
                             (+ pos 1))])
  ...
  (def (cmd-yank app)
       (let* ([ed (current-qt-editor app)]
         ...
         (let ([pos-after (qt-plain-text-edit-cursor-position ed)])
           (app-state-last-yank-pos-set! app pos-before)
           (app-state-last-yank-len-set! app (- pos-after pos-before))
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (when (buffer-mark buf)
           (app-state-mark-ring-set!
             app
             (cons
               (cons (buffer-name buf) (buffer-mark buf))
  ...
  (def (path-char-delimiter? ch)
       ...
       (or (char=? ch #\space)
           (char=? ch #\tab)
           (char=? ch #\newline)
           (char=? ch (integer->char 34))
           (char=? ch (integer->char 39))
           (char=? ch #\()
           (char=? ch #\))))
         ...
         (and (< pos len)
              (let* ([start (let scan ([i pos])
                ...
                (and (> (string-length raw) 0)
                     (let ([colon-pos (let scan ([i (- (string-length raw)
                                                       1)])
                                        (cond
                                          [(< i 0) #f]
  ...
  (def (cmd-switch-buffer app)
       (let* ([echo (app-state-echo app)]
              ...
              [name (qt-echo-read-with-narrowing
                      app
                      "Switch to buffer:"
                      names)])
         (when name
  ...
  (def (cmd-split-window-right app)
       (winner-save! (app-state-frame app))
       (let ([new-ed (qt-frame-split-right!
                       (app-state-frame app))])
         (when (app-state-key-handler app)
           ((app-state-key-handler app) new-ed))))
       ...
       (let* ([fr (app-state-frame app)]
         ...
         (if (<= n 1)
             ...
             (if (= n 2)
                 ...
                 (let* ([labels (let loop ([ws wins] [i 0] [acc (list)])
                                  (if (null? ws)
                                      (reverse acc)
                                      (let* ([w (car ws)]
                                             [bname (buffer-name
                     ...
                     [(or (< num 1) (> num n))
                      (echo-error!
                        ...
                        (string-append
                          ...
                          " does not exist"))]
                     [else
                      (qt-frame-current-idx-set! fr (- num 1))
                      (echo-message!
                        (app-state-echo app)
                        (string-append
                          "Switched to window "
       ...
       (let* ([fr (app-state-frame app)]
         ...
         (if (<= n 1)
             ...
             (let* ([cur-idx (qt-frame-current-idx fr)]
               ...
               (qt-edit-window-buffer-set! cur-win next-buf)
               (qt-edit-window-buffer-set! next-win cur-buf)
  ...
  (define *mx-command-history*--cell (vector (list)))
  (define *mx-history-max*--cell (vector 50))
  (def (qt-mx-history-add! name)
       ...
       (set! *mx-command-history*
         (cons
           name
           (let loop ([h *mx-command-history*] [acc (list)])
             (cond
               [(null? h) (reverse acc)]
       ...
       (when (> (length *mx-command-history*) *mx-history-max*)
         (set! *mx-command-history*
           (let loop ([h *mx-command-history*] [n 0] [acc (list)])
             (if (or (null? h) (>= n *mx-history-max*))
                 (reverse acc)
                 (loop (cdr h) (+ n 1) (cons (car h) acc))))))
       (mx-history-add! name))
  (define *mx-recent-pinned-count*--cell (vector 5))
  (def (cmd-execute-extended-command app)
       (let* ([all-names (sort
                           (map symbol->string (hash-keys *all-commands*))
                           string<?)]
              [check-prefix (string (integer->char 10003) #\space)]
              [pinned-names (let loop ([h *mx-command-history*]
                                       [n 0]
                                       [acc (list)])
                              (if (or (null? h)
                                      (>= n *mx-recent-pinned-count*))
                                  (reverse acc)
                                  (loop
                                    (cdr h)
                                    (+ n 1)
                                    (cons
                                      (string-append check-prefix (car h))
                                      acc))))]
              [recent-set (let loop ([h *mx-command-history*]
                                     [s (make-hash-table)])
                            (if (null? h)
                                s
                                (begin
                                  ...
                                  (loop (cdr h) s))))]
              [non-recent (filter
                            (lambda (n) (not (hash-get recent-set n)))
                            all-names)]
              [ordered (append
                         pinned-names
                         *mx-command-history*
             ...
             (let ([key (car entry)] [val (cdr entry)])
               (cond
                 [(symbol? val)
                  (set! lines
                    (cons
                      (string-append "  " key "\t" (symbol->string val))
                      lines))]
                 [(hash-table? val)
                  (for-each
                    (lambda (sub-entry)
  ...
  (def (cmd-list-buffers app)
       (let* ([fr (app-state-frame app)]
                                                      ...
                                                      (make-string
                                                        (- 13
                                                           (string-length
                                                             mode-str))
                                                        #\space)))])
                              (string-append "  " cur mod " " padded-name
                                padded-mode path)))
                          bufs)]
         ...
         (let ([buf (or (buffer-by-name "*Buffer List*")
           ...
           (sci-send ed SCI_SETCARETLINEBACK (rgb->sci 42 42 74))
  ...
  (def (cmd-duplicate-line app)
       (let* ([ed (current-qt-editor app)]
              ...
              [line-text (if (< line (length lines))
                             (list-ref lines line)
                             "")])
         (let* ([new-lines (let loop ([ls lines] [i 0] [acc (list)])
                             (if (null? ls)
                                 (reverse acc)
                                 (if (= i line)
                                     (loop
    ...
    (identifier-syntax
      ...
      [(set! id val) (vector-set!
                       *winner-max-history*--cell
                       0
                       val)]))
  (define-syntax *qt-app-ptr*
    (identifier-syntax
      [id (vector-ref *qt-app-ptr*--cell 0)]
      [(set! id val) (vector-set! *qt-app-ptr*--cell 0 val)]))
  (define-syntax *tab-bar-visible*
    (identifier-syntax
      [id (vector-ref *tab-bar-visible*--cell 0)]
      [(set! id val) (vector-set!
                       *tab-bar-visible*--cell
                       0
                       val)]))
  (define-syntax *current-theme*
    (identifier-syntax
      [id (vector-ref *current-theme*--cell 0)]
      [(set! id val) (vector-set! *current-theme*--cell 0 val)]))
  (define-syntax *buffer-recent*
    (identifier-syntax
      [id (vector-ref *buffer-recent*--cell 0)]
      [(set! id val) (vector-set! *buffer-recent*--cell 0 val)]))
  (define-syntax *global-auto-revert-mode*
    (identifier-syntax
      [id (vector-ref *global-auto-revert-mode*--cell 0)]
      [(set! id val) (vector-set!
                       *global-auto-revert-mode*--cell
                       0
                       val)]))
  (define-syntax *file-mtimes*
    (identifier-syntax
      [id (vector-ref *file-mtimes*--cell 0)]
      [(set! id val) (vector-set! *file-mtimes*--cell 0 val)]))
  (define-syntax *auto-revert-tail-buffers*
    (identifier-syntax
      [id (vector-ref *auto-revert-tail-buffers*--cell 0)]
      [(set! id val) (vector-set!
                       *auto-revert-tail-buffers*--cell
                       0
                       val)]))
  (define-syntax *dir-locals-cache*
    (identifier-syntax
      [id (vector-ref *dir-locals-cache*--cell 0)]
      [(set! id val) (vector-set!
                       *dir-locals-cache*--cell
                       0
                       val)]))
  (define-syntax *mx-command-history*
    (identifier-syntax
      [id (vector-ref *mx-command-history*--cell 0)]
      [(set! id val) (vector-set!
                       *mx-command-history*--cell
                       0
                       val)]))
  (define-syntax *mx-history-max*
    (identifier-syntax
      [id (vector-ref *mx-history-max*--cell 0)]
      [(set! id val) (vector-set! *mx-history-max*--cell 0 val)]))
  (define-syntax *mx-recent-pinned-count*
    (identifier-syntax
      [id (vector-ref *mx-recent-pinned-count*--cell 0)]
      [(set! id val) (vector-set!
                       *mx-recent-pinned-count*--cell
                       0
                       val)])))
