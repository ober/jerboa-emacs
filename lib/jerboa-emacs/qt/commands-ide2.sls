...
;;; Source: src/jerboa-emacs/qt/commands-ide2.ss

(library (jerboa-emacs qt commands-ide2)
  (export cmd-comment-region cmd-uncomment-region
   cmd-collapse-blank-lines cmd-remove-blank-lines
   cmd-delete-trailing-lines cmd-trim-lines cmd-prefix-lines
   cmd-suffix-lines cmd-sort-lines-reverse
   cmd-sort-lines-case-fold cmd-uniquify-lines cmd-sort-words
   cmd-camel-to-snake cmd-snake-to-camel
   cmd-highlight-word-at-point *wgrep-mode*
   *wgrep-original-lines* cmd-wgrep-change-to-wgrep-mode
   cmd-wgrep-finish-edit cmd-wgrep-abort-changes
   *qt-quoted-insert-pending* cmd-quoted-insert
   qt-quoted-insert-handle! cmd-quick-calc cmd-eval-and-insert
   cmd-shell-command-insert cmd-pipe-region *session-path*
   session-save! session-restore-files cmd-session-save
   cmd-session-restore cmd-duplicate-region cmd-reverse-chars
   cmd-reverse-word cmd-getenv cmd-setenv *hl-todo-mode*
   *hl-todo-keywords* cmd-hl-todo-mode cmd-hl-todo-next
   cmd-hl-todo-previous shell-command-to-string
   shell-command-to-buffer! *user-shell-commands*
   register-shell-command! cmd-run-user-shell-command
   *workspaces* *current-workspace* *workspace-buffers*
   workspace-init! workspace-add-buffer!
   workspace-remove-buffer! cmd-workspace-create
   cmd-workspace-switch cmd-workspace-delete
   cmd-workspace-add-buffer cmd-workspace-list
   qt-enable-multiple-selection! cmd-mc-mark-next
   cmd-mc-mark-all cmd-mc-skip-and-mark-next cmd-mc-edit-lines
   cmd-mc-unmark-last cmd-mc-rotate qt-fill-words
   cmd-fill-region cmd-insert-buffer cmd-prepend-to-buffer
   cmd-copy-rectangle-to-register cmd-undo-tree-visualize
   *dired-expanded-dirs* *project-tree-expanded*
   *project-terminals* project-tree-render
   cmd-dired-subtree-toggle cmd-project-tree cmd-project-term
   rg-available? run-rg parse-rg-line cmd-consult-ripgrep
   cmd-consult-bookmark *qt-goto-address-active*
   *qt-goto-address-indicator* qt-goto-address-setup!
   qt-goto-address-clear! qt-goto-address-scan!
   cmd-goto-address-mode *qt-subword-mode*
   qt-subword-forward-pos qt-subword-backward-pos
   cmd-subword-mode cmd-subword-forward cmd-subword-backward
   cmd-subword-kill)
  (import
   ...
   (jerboa-emacs qt commands-ide) (jerboa core)
   (jerboa runtime))
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (not mark)
             ...
             (let* ([pos (qt-plain-text-edit-cursor-position ed)]
                    ...
                    [commented (map (lambda (l) (string-append ";; " l))
                                    lines)]
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (let-values ([(start end)
           ...
           (let* ([region (substring text start end)]
                  ...
                  [non-blank (filter
                               (lambda (l)
                                 (> (string-length (string-trim-both l))
                                    0))
                               lines)]
             ...
             (let ([suffix (qt-echo-read-string app "Suffix: ")])
               (when suffix
                 (let* ([pos (qt-plain-text-edit-cursor-position ed)]
                        ...
                        [region (substring text start end)]
                        [lines (string-split region #\newline)]
                        [suffixed (map (lambda (l)
                                         (string-append l suffix))
                                       lines)]
  ...
  (def (cmd-wgrep-finish-edit app)
       ...
       (let ([buf (current-qt-buffer app)])
         (if (not (and (string=? (buffer-name buf) "*Grep*")
                       *wgrep-mode*))
             (echo-error! (app-state-echo app) "Not in wgrep mode")
             (let* ([ed (current-qt-editor app)]
                    [text (qt-plain-text-edit-text ed)]
                    [all-lines (let loop ([s text] [acc (list)])
                     ...
                     (lambda (file line-edits)
                       (when (file-exists? file)
                         (let* ([content (read-file-as-string file)]
                                [lines (let loop ([s content] [acc (list)])
                                         (let ([nl (string-index
                                                     s
                                                     #\newline)])
                                           (if nl
                           ...
                           (let ([final (if (and (> (string-length content)
                                                    0)
                                                 (char=?
                                                   (string-ref
                                                     content
  ...
  (def (cmd-wgrep-abort-changes app)
       ...
       (if (not *wgrep-mode*)
           ...
           (begin
             ...
             (echo-message!
               (app-state-echo app)
               "Wgrep changes aborted"))))
  (define *qt-quoted-insert-pending*--cell (vector #f))
       ...
       (with-catch
         ...
         (lambda ()
           (let* ([ed (current-qt-editor app)]
                  ...
                  [entries (let loop ([bufs *buffer-list*] [acc (list)])
                             (if (null? bufs)
                                 (reverse acc)
                                 (let ([buf (car bufs)])
  ...
  (def (cmd-session-restore app)
       ...
       (echo-message!
         (app-state-echo app)
         "Use --restore flag to restore session on startup"))
  ...
  (define *hl-todo-mode*--cell (vector #f))
  (define *hl-todo-keywords*--cell
    (vector '("TODO" "FIXME" "HACK" "BUG" "XXX" "NOTE")))
  ...
  (def (cmd-hl-todo-previous app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if prev-pos
             (begin
               (qt-plain-text-edit-set-cursor-position! ed prev-pos)
               (echo-message! (app-state-echo app) "Found TODO keyword"))
             (echo-message!
               (app-state-echo app)
               "No previous TODO keywords"))))
         ...
         (async-process!
           ...
           (lambda (result)
             (let* ([fr (app-state-frame app)]
               ...
               (qt-text-document-set-modified! (buffer-doc-pointer buf) #f)
               (qt-plain-text-edit-set-cursor-position! ed 0)
               (when read-only? (qt-plain-text-edit-set-read-only! ed #t))
               (qt-modeline-update! app))))))
  (define *user-shell-commands*--cell
    (vector (make-hash-table)))
  (def (register-shell-command!
         name
         prompt
         command-template
         buffer-template)
  ...
  (def (cmd-run-user-shell-command app)
       ...
       (let* ([names (sort
                       (hash-keys *user-shell-commands*)
                       string<?)]
              [name (if (null? names)
                        (begin
                          (echo-message!
                            (app-state-echo app)
                            "No shell commands registered")
                          #f)
                        (qt-echo-read-string
                          app
                          (string-append
  ...
  (define *workspaces*--cell (vector (make-hash-table)))
  (define *current-workspace*--cell (vector "default"))
  (define *workspace-buffers*--cell
    (vector (make-hash-table)))
  ...
  (def (workspace-add-buffer! ws-name buf-name)
       ...
       (let ([bufs (or (hash-get *workspaces* ws-name) (list))])
       ...
       (let ([name (qt-echo-read-string
         ...
         (when (and name (> (string-length name) 0))
           (if (hash-get *workspaces* name)
               ...
               (begin
                 (hash-put! *workspaces* name (list "*scratch*"))
                 (echo-message!
                   (app-state-echo app)
  ...
  (def (cmd-workspace-delete app)
       ...
       (let* ([names (sort
                       (filter
                         (lambda (n) (not (string=? n "default")))
                         (hash-keys *workspaces*))
                       string<?)]
              [name (if (null? names)
                        (begin
                          (echo-message!
  ...
  (def (cmd-workspace-add-buffer app)
       ...
       (let* ([buf (qt-current-buffer (app-state-frame app))]
         ...
         (echo-message!
           (app-state-echo app)
           (string-append "Added '" buf-name "' to workspace '"
             *current-workspace* "'"))))
  (def (cmd-workspace-list app)
       "List all workspaces and their buffers."
       (let* ([fr (app-state-frame app)]
              [ed (current-qt-editor app)]
              [lines (let loop ([names (sort
                                         (hash-keys *workspaces*)
                                         string<?)]
                                [acc (list)])
                       (if (null? names)
                           ...
                           (let* ([name (car names)]
                                  ...
                                  [active? (string=?
                                             name
                                             *current-workspace*)]
                                  [header (string-append (if active? "* " "  ") name
                                            " ("
                                            (number->string (length bufs))
                                            " buffers)")]
                                  [buf-lines (map (lambda (b)
                                                    (string-append
                                                      "    "
                                                      b))
                                                  bufs)])
  ...
  (def (cmd-insert-buffer app)
       ...
       (let* ([bufs (buffer-list)]
         ...
         (when (and target-name (> (string-length target-name) 0))
           (let ([target-buf (find
                               (lambda (b)
                                 (string=? (buffer-name b) target-name))
                               bufs)])
             (if (not target-buf)
                 (echo-error!
                   (app-state-echo app)
                 ...
                 (let* ([fr (app-state-frame app)]
                        [target-text (let loop ([wins (qt-frame-windows
                                                        fr)])
                                       (cond
                                         [(null? wins) #f]
                                         [(eq? (qt-edit-window-buffer
                                                 (car wins))
                                               target-buf)
                         ...
                         (let loop ([wins (qt-frame-windows fr)])
                           (when (pair? wins)
                             (if (eq? (qt-edit-window-buffer (car wins))
                                      target-buf)
                                 (let ([target-ed (qt-edit-window-editor
                                                    (car wins))])
                                   (qt-plain-text-edit-set-cursor-position!
                                     target-ed
                                     0)
             ...
             (if (not mark)
                 ...
                 (let* ([pos (qt-plain-text-edit-cursor-position ed)]
                   ...
                   (let-values ([(start-line end-line)
                     ...
                     (let ([rect-lines (let loop ([i start-line]
                                                  [acc (list)])
                                         (if (> i end-line)
                                             (reverse acc)
                                             (let* ([l (if (< i
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (let ([history (or (hash-get *undo-history* buf-name)
           ...
           (if (null? history)
               ...
               (let* ([now (inexact->exact
                             (floor (time->seconds (current-time))))]
                      [fr (app-state-frame app)]
                      [tree-lines (let loop ([entries history]
                                             [i 0]
                                             [acc (list)])
                                    (if (null? entries)
                                        (reverse acc)
                                        (let* ([e (car entries)]
                                          ...
                                          (loop
                                            ...
                                            (+ i 1)
                                            (cons
                                              (string-append "  " connector "-- ["
                                                (number->string i) "] "
                                                age-str "  "
                                                (number->string tlen)
                                                " chars, "
                                                (number->string line-count)
                                                " lines" marker "\n  |   "
                                                preview)
                                              acc)))))]
                      [header (string-append "Undo Tree: " buf-name "\n" "Snapshots: "
                 ...
                 (qt-text-document-set-modified!
                   (buffer-doc-pointer tree-buf)
                   #f)
                 (qt-plain-text-edit-set-cursor-position! ed 0))))))
  (define *dired-expanded-dirs*--cell
    (vector (make-hash-table)))
  (define *project-tree-expanded*--cell
    (vector (make-hash-table)))
                       ...
                       (let* ([f (car es)]
                              ...
                              [is-dir (and (file-exists? fp)
                                           (eq? (file-info-type
                                                  (file-info fp))
                                                'directory))]
                              [is-hidden (and (> (string-length f) 0)
                                              (char=?
                                                (string-ref f 0)
                                                #\.))]
                              [expanded (hash-get
                         ...
                         (if is-hidden
                             ...
                             (let ([line (string-append
                               ...
                               (if (and is-dir expanded)
                                   (let ([children (project-tree-render
                                                     fp
                                                     (+ depth 1)
                                                     max-depth)])
                                     (loop
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (not (string-prefix? "*Dired:" name))
             ...
             (let* ([pos (sci-send ed SCI_GETCURRENTPOS 0 0)]
               ...
               (when (and last-part
                          (not (string-empty? last-part))
                          (not (member last-part '("." ".." "total"))))
                 (let ([full-path (if dir-match
                                      (path-expand last-part dir-match)
                                      last-part)])
                   (if (and (file-exists? full-path)
                            (eq? (file-info-type (file-info full-path))
                                 'directory))
                       (let ([expanded (or (hash-get
                                             *dired-expanded-dirs*
                                             name)
                                           (make-hash-table))])
                         (if (hash-get expanded full-path)
                             (begin
                               ...
                               (let rm ([nl (+ line-num 1)])
                                 (let* ([ns (sci-send
                                        ...
                                        [ne (sci-send
                                              ...
                                              0)])
                                   (when (> ne ns)
                                     (let ([nt (qt-plain-text-edit-text-range
                                                 ed
                                                 ns
                                                 ne)])
                               ...
                               (echo-message!
                                 echo
                                 (string-append "Collapsed: " last-part)))
                             (with-catch
                               (lambda (e)
                                 (echo-message!
                                   echo
             ...
             (let* ([term-name (or (hash-get *project-terminals* root)
               ...
               (if existing
                   ...
                   (begin
                     ...
                     (echo-message!
                       echo
                       (string-append "New terminal in: " root))))))))
  (def (rg-available?)
       ...
       (with-catch
         ...
         (lambda ()
           (let ([p (open-process
                      (list 'path: "rg" 'arguments: '("--version")
  ...
  (def (run-rg pattern dir)
       ...
       (with-catch
         ...
         (lambda ()
           (let* ([p (open-process
                       (list 'path: "rg" 'arguments:
                         (list "--vimgrep" "--color" "never" "--max-count"
                           "500" pattern dir)
                         'stdout-redirection: #t 'stderr-redirection: #f))]
           ...
           (let ([results (run-rg pattern root)])
             (if (null? results)
                 ...
                 (let* ([choice (qt-echo-read-with-narrowing
                   ...
                   (if parsed
                       (let ([file (car parsed)] [line-num (cadr parsed)])
  ...
  (def (cmd-consult-bookmark app)
       ...
       (let* ([bmarks (app-state-bookmarks app)]
         ...
         (if (null? names)
             ...
             (let* ([display-items (map (lambda (name)
                                          (let* ([entry (hash-get
                                                          bmarks
                                                          name)]
                                                 [file (and entry
  ...
  (define *qt-goto-address-active*--cell (vector #f))
  (define *qt-goto-address-indicator*--cell (vector 12))
  (def (qt-goto-address-setup! ed)
       "Setup indicator style for URL highlighting."
       (sci-send
         ed
         SCI_INDICSETSTYLE
         *qt-goto-address-indicator*
         6)
       (sci-send
         ed
         SCI_INDICSETSTYLE
  ...
  (def (qt-goto-address-scan! ed)
       ...
       (qt-goto-address-clear! ed) (qt-goto-address-setup! ed)
       (let* ([text (qt-plain-text-edit-text ed)]
              [len (string-length text)])
         (sci-send
           ed
         ...
         (let loop ([i 0])
           (when (< i (- len 7))
             (if (and (char=? (string-ref text i) #\h)
                      (or (string-prefix?
                            "http://"
                            (substring text i (min len (+ i 8))))
                          (string-prefix?
                            "https://"
                            (substring text i (min len (+ i 9))))))
                 (let url-end ([j (+ i 7)])
                   (if (or (>= j len)
  ...
  (def (cmd-goto-address-mode app)
       ...
       (let ([ed (current-qt-editor app)])
         (set! *qt-goto-address-active*
           (not *qt-goto-address-active*))
         (if *qt-goto-address-active*
             ...
             (begin
               ...
               (echo-message!
                 (app-state-echo app)
                 "Goto-address-mode OFF")))))
  (define *qt-subword-mode*--cell (vector #f))
         ...
         (when (> end pos)
           (let ([killed (substring text pos end)])
             ...
             (echo-message!
               ...
               (string-append
                 "Killed: "
                 (if (> (string-length killed) 30)
                     (string-append (substring killed 0 30) "...")
                     killed)))))))
  (define-syntax *wgrep-mode*
    (identifier-syntax
      [id (vector-ref *wgrep-mode*--cell 0)]
      [(set! id val) (vector-set! *wgrep-mode*--cell 0 val)]))
  (define-syntax *wgrep-original-lines*
    (identifier-syntax
      [id (vector-ref *wgrep-original-lines*--cell 0)]
      [(set! id val) (vector-set!
                       *wgrep-original-lines*--cell
                       0
                       val)]))
  (define-syntax *qt-quoted-insert-pending*
    (identifier-syntax
      [id (vector-ref *qt-quoted-insert-pending*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-quoted-insert-pending*--cell
                       0
                       val)]))
  (define-syntax *session-path*
    (identifier-syntax
      [id (vector-ref *session-path*--cell 0)]
      [(set! id val) (vector-set! *session-path*--cell 0 val)]))
  (define-syntax *hl-todo-mode*
    (identifier-syntax
      [id (vector-ref *hl-todo-mode*--cell 0)]
      [(set! id val) (vector-set! *hl-todo-mode*--cell 0 val)]))
  (define-syntax *hl-todo-keywords*
    (identifier-syntax
      [id (vector-ref *hl-todo-keywords*--cell 0)]
      [(set! id val) (vector-set!
                       *hl-todo-keywords*--cell
                       0
                       val)]))
  (define-syntax *user-shell-commands*
    (identifier-syntax
      [id (vector-ref *user-shell-commands*--cell 0)]
      [(set! id val) (vector-set!
                       *user-shell-commands*--cell
                       0
                       val)]))
  (define-syntax *workspaces*
    (identifier-syntax
      [id (vector-ref *workspaces*--cell 0)]
      [(set! id val) (vector-set! *workspaces*--cell 0 val)]))
  (define-syntax *current-workspace*
    (identifier-syntax
      [id (vector-ref *current-workspace*--cell 0)]
      [(set! id val) (vector-set!
                       *current-workspace*--cell
                       0
                       val)]))
  (define-syntax *workspace-buffers*
    (identifier-syntax
      [id (vector-ref *workspace-buffers*--cell 0)]
      [(set! id val) (vector-set!
                       *workspace-buffers*--cell
                       0
                       val)]))
  (define-syntax *dired-expanded-dirs*
    (identifier-syntax
      [id (vector-ref *dired-expanded-dirs*--cell 0)]
      [(set! id val) (vector-set!
                       *dired-expanded-dirs*--cell
                       0
                       val)]))
  (define-syntax *project-tree-expanded*
    (identifier-syntax
      [id (vector-ref *project-tree-expanded*--cell 0)]
      [(set! id val) (vector-set!
                       *project-tree-expanded*--cell
                       0
                       val)]))
    ...
    (identifier-syntax
      ...
      [(set! id val) (vector-set!
                       *project-terminals*--cell
                       0
                       val)]))
  (define-syntax *qt-goto-address-active*
    (identifier-syntax
      [id (vector-ref *qt-goto-address-active*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-goto-address-active*--cell
                       0
                       val)]))
  (define-syntax *qt-goto-address-indicator*
    (identifier-syntax
      [id (vector-ref *qt-goto-address-indicator*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-goto-address-indicator*--cell
                       0
                       val)]))
  (define-syntax *qt-subword-mode*
    (identifier-syntax
      [id (vector-ref *qt-subword-mode*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-subword-mode*--cell
                       0
                       val)])))
