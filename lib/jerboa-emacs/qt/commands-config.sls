...
;;; Source: src/jerboa-emacs/qt/commands-config.ss

(library (jerboa-emacs qt commands-config)
  (export cmd-display-fill-column-indicator
   cmd-display-line-numbers-relative cmd-font-lock-mode
   cmd-customize-face get-monospace-fonts
   apply-font-to-all-editors! cmd-set-frame-font
   cmd-set-font-size *named-colors* cmd-list-colors
   *user-themes-dir* discover-user-themes load-user-theme-file!
   cmd-load-theme cmd-describe-theme cmd-fold-level
   cmd-ansi-term cmd-diff-backup cmd-eldoc cmd-recover-session
   cmd-revert-buffer-with-coding *encoding-list*
   cmd-set-buffer-file-coding cmd-set-language-environment
   cmd-sudo-find-file *which-function-mode*
   which-function-at-point cmd-which-function cmd-widen-all
  ...
  (import
   ...
   (only (jerboa-emacs persist) record-face-customization!
     custom-faces-save! theme-settings-save! buffer-local-get
     buffer-local-set!)
  ...
  (def (cmd-customize-face app)
       ...
       (let* ([face-names (sort
              ...
              [face-input (qt-echo-read-string-with-completion
                            app
                            "Customize face: "
                            face-name-strs)])
         (when (and face-input (not (string-empty? face-input)))
           (let* ([face-sym (string->symbol face-input)]
             ...
             (if (not face)
                 ...
                 (begin
                   (let ([current-desc (string-append "Current: "
                                         (if (face-fg face)
                                             (string-append
                                               "fg:"
                                               (face-fg face)
                   ...
                   (let ([fg-input (qt-echo-read-string
                                     app
                                     "Foreground (#hex or empty to keep): ")]
                         [bg-input #f]
                         [bold-input #f]
                         [italic-input #f])
                     ...
                     (set! bg-input
                       (qt-echo-read-string
                         app
                         "Background (#hex or empty to keep): "))
                     ...
                     (set! bold-input
                       (qt-echo-read-string
                         app
                         "Bold (y/n/empty to keep): "))
                     (when (and bold-input
                                (not (string-empty? bold-input)))
                       (let ([bold-val (cond
                                         [(or (string=? bold-input "y")
                                              (string=? bold-input "yes"))
                                          #t]
                                         [(or (string=? bold-input "n")
                                              (string=? bold-input "no"))
                                          #f]
                                         [else 'unset])])
                     ...
                     (set! italic-input
                       (qt-echo-read-string
                         app
                         "Italic (y/n/empty to keep): "))
                     (when (and italic-input
                                (not (string-empty? italic-input)))
                       (let ([italic-val (cond
                                           [(or (string=? italic-input "y")
                                                (string=?
                                                  italic-input
                                                  "yes"))
                                            #t]
                                           [(or (string=? italic-input "n")
                                                (string=?
                                                  italic-input
                                                  "no"))
                                            #f]
       ...
       (with-catch
         ...
         (lambda ()
           (let* ([proc (open-process
                          (list 'path: "/usr/bin/fc-list" 'arguments:
                            (list ":spacing=mono" "family")
  ...
  (def (cmd-list-colors app)
       ...
       (let* ([fr (app-state-frame app)]
              ...
              [lines (let loop ([cs *named-colors*]
                                [row (list)]
                                [acc (list)])
                       (cond
                         [(null? cs)
              ...
              [text (string-append header (string-join lines "\n") "\n\n"
                      (number->string (length *named-colors*))
                      " colors total")])
  ...
  (def (load-user-theme-file! theme-name)
       ...
       (with-catch
         (lambda (e) #f)
         (lambda ()
           (let ([theme-path (path-expand
                               (string-append
                                 (symbol->string theme-name)
                                 ".ss")
                               *user-themes-dir*)])
                ...
                (if (load-user-theme-file! sym)
                    (if (theme-get sym)
                        ...
                        (echo-error!
                          (app-state-echo app)
                          (string-append
                            "Theme file loaded but no theme defined: "
                            input)))
                    (echo-error!
                      (app-state-echo app)
                      (string-append
       ...
       (let* ([builtin-themes (theme-names)]
              ...
              [input (qt-echo-read-string-with-completion
                       ...
                       (string-append
                         ...
                         (symbol->string *current-theme*)
                         "): ")
                       names)])
         (let* ([theme-name (if (or (not input)
                                    (string-empty? input))
                                *current-theme*
                                (string->symbol input))]
           ...
           (cond
             [theme
              (let* ([fr (app-state-frame app)]
                     [ed (current-qt-editor app)]
                     [buf-name (string-append
                                 "*theme: "
                                 (symbol->string theme-name)
                                 "*")]
                     [buf (or (buffer-by-name buf-name)
                              (qt-buffer-create! buf-name ed #f))])
                (let ([desc (string-append "Theme: " (symbol->string theme-name) "\n"
                              (make-string 60 #\=) "\n\n"
         ...
         (let loop ([i 0])
           (when (< i total)
             (let ([fl (sci-send ed SCI_GETFOLDLEVEL i 0)])
               (when (and (not (zero?
                                 (bitwise-and fl SC_FOLDLEVELHEADERFLAG)))
                          (<= (bitwise-and fl 4095) (+ 1024 target-level)))
  ...
  (define *encoding-list*--cell
    (vector
      '("utf-8" "utf-8-with-bom" "latin-1" "iso-8859-1" "iso-8859-15"
         "windows-1252" "ascii" "utf-16le" "utf-16be" "shift-jis"
         "euc-jp" "gb2312" "big5" "koi8-r" "iso-8859-2")))
  (def (cmd-set-buffer-file-coding app)
       ...
       (let* ([buf (current-qt-buffer app)]
              [current-enc (or (buffer-local-get buf 'file-coding)
                               "utf-8")]
  ...
  (def (cmd-set-language-environment app)
       ...
       (let* ([envs '("UTF-8" "Latin-1" "Latin-2" "Latin-9" "Windows-1252"
                       "Japanese" "Chinese-GB" "Chinese-BIG5" "Korean"
                       "Cyrillic-KOI8" "ASCII")]
              [choice (qt-echo-read-with-narrowing
                        app
  ...
  (def (cmd-sudo-find-file app)
       "Open file as root."
       (cmd-sudo-write app))
  (define *which-function-mode*--cell (vector #f))
                ...
                (let* ([open-paren? (char=? (string-ref text (+ i 4)) #\()]
                       ...
                       [name-end (let nloop ([j ns])
                                   (if (or (>= j len)
                                       ...
                                       j
                                       (nloop (+ j 1))))])
                  (if (> name-end ns) (substring text ns name-end) #f))]
               [(and (>= (- len i) 4)
                     (or (and (or (= i 0)
                                  (char=?
                                    (string-ref text (- i 1))
                       ...
                       (filter
                         (lambda (l)
                           (and (> (string-length l) 0)
                                (char-whitespace?
                                  (string-ref l (- (string-length l) 1)))))
                         lines))])
         (echo-message!
           (app-state-echo app)
           (string-append
  ...
  (def (cmd-term app)
       ...
       (let* ([fr (app-state-frame app)]
              [ed (current-qt-editor app)]
              [name (begin
                      (set! terminal-buffer-counter
                        (+ terminal-buffer-counter 1))
  ...
  (def (cmd-term-interrupt app)
       ...
       (let* ([buf (current-qt-buffer app)]
              [ts (and (terminal-buffer? buf)
                       (hash-get *terminal-state* buf))])
         (cond
           [(not ts)
            (let ([ss (and (shell-buffer? buf)
                           (hash-get *shell-state* buf))])
              ...
              (let* ([text (qt-plain-text-edit-text ed)]
                ...
                (if (string=? input "")
                    (begin
                      (terminal-stop! ts)
                      (kill-and-switch!
                        (lambda () (hash-remove! *terminal-state* buf)))
                      (echo-message!
                        (app-state-echo app)
                        "Terminal exited"))
                    (echo-message!
              ...
              (let* ([text (qt-plain-text-edit-text ed)]
                ...
                (if (string=? input "")
                    (begin
                      (shell-stop! ss)
                      (kill-and-switch!
                        (lambda () (hash-remove! *shell-state* buf)))
                      (echo-message! (app-state-echo app) "Shell exited"))
                    (echo-message!
                      (app-state-echo app)
                      "Use 'exit' to close (input not empty)"))))]
           [(and (gsh-eshell-buffer? buf)
                 (hash-get *gsh-eshell-state* buf)) =>
            (lambda (env)
              (kill-and-switch!
                (lambda () (hash-remove! *gsh-eshell-state* buf)))
              (echo-message! (app-state-echo app) "Eshell exited"))]
           [else (cmd-delete-char app)])))
         ...
         (if (null? terms)
             ...
             (let* ([names (map buffer-name terms)]
               ...
               (when (and choice (not (string=? choice "")))
                 (let ([target (find
                                 (lambda (b)
                                   (string=? (buffer-name b) choice))
                                 terms)])
                   (when target (qt-switch-to-terminal! app target))))))))
       ...
       (let* ([terms (get-terminal-buffers)]
         ...
         (cond
           ...
           [else
            (let loop ([rest terms])
              (cond
                [(null? rest) (qt-switch-to-terminal! app (car terms))]
                [(eq? (car rest) cur)
                 (if (null? (cdr rest))
                     (qt-switch-to-terminal! app (car terms))
  ...
  (def (cmd-term-prev app)
       ...
       (let* ([terms (get-terminal-buffers)]
              [cur (current-qt-buffer app)]
              [last-term (and (pair? terms)
                              (list-ref terms (- (length terms) 1)))])
         (cond
           [(null? terms)
            (echo-message! (app-state-echo app) "No terminal buffers")]
           [(not (terminal-buffer? cur))
           ...
           [else
            (let loop ([rest terms] [prev last-term])
              (cond
                [(null? rest) (qt-switch-to-terminal! app prev)]
                [(eq? (car rest) cur) (qt-switch-to-terminal! app prev)]
                  ...
                  (let ([new-line (string-append line-text "  ;; ")])
                    ...
                    (let* ([new-text (qt-plain-text-edit-text ed)]
                      ...
                      (qt-plain-text-edit-set-cursor-position!
                        ed
                        pos)))])))))
  (define-syntax *named-colors*
    (identifier-syntax
      [id (vector-ref *named-colors*--cell 0)]
      [(set! id val) (vector-set! *named-colors*--cell 0 val)]))
  (define-syntax *user-themes-dir*
    (identifier-syntax
      [id (vector-ref *user-themes-dir*--cell 0)]
      [(set! id val) (vector-set!
                       *user-themes-dir*--cell
                       0
                       val)]))
  (define-syntax *encoding-list*
    (identifier-syntax
      [id (vector-ref *encoding-list*--cell 0)]
      [(set! id val) (vector-set! *encoding-list*--cell 0 val)]))
  (define-syntax *which-function-mode*
    (identifier-syntax
      [id (vector-ref *which-function-mode*--cell 0)]
      [(set! id val) (vector-set!
                       *which-function-mode*--cell
                       0
                       val)]))
  (define-syntax *qt-profiler-running*
    (identifier-syntax
      [id (vector-ref *qt-profiler-running*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-profiler-running*--cell
                       0
                       val)]))
  (define-syntax *qt-profiler-start-stats*
    (identifier-syntax
      [id (vector-ref *qt-profiler-start-stats*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-profiler-start-stats*--cell
                       0
                       val)]))
  (define-syntax *profiler-data*
    (identifier-syntax
  ...
  (define-syntax *terminal-copy-mode*
    (identifier-syntax
      [id (vector-ref *terminal-copy-mode*--cell 0)]
      [(set! id val) (vector-set!
                       *terminal-copy-mode*--cell
                       0
                       val)])))
