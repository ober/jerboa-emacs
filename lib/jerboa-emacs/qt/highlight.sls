...
(library (jerboa-emacs qt highlight)
  ...
  (import
    ...
    (jerboa-emacs treesitter) (jerboa-emacs face)
    (jerboa-emacs themes)
  ...
  (define *qt-show-paren-enabled*--cell (vector #t))
  (define *qt-delete-selection-enabled*--cell (vector #t))
  ...
  (def (face-has-bold? face-name)
       ...
       (let ([f (face-get face-name)]) (and f (face-bold f))))
  (def (face-has-italic? face-name)
       "Check if a face has the italic attribute."
       (let ([f (face-get face-name)]) (and f (face-italic f))))
              ...
              (cond
                [(or (not ext) (string=? ext ""))
                 (cond
                   ...
                   [(member base '("Rakefile" "Gemfile" "Vagrantfile"))
                    'ruby]
                   [else #f])]
                ...
                [(member
                   ext
                   '(".xml" ".html" ".htm" ".svg" ".xhtml" ".xsl" ".xsd"
                      ".plist"))
                 'xml]
  ...
  (def *gerbil-keywords*
       (string-join
         '("def" "defvalues" "defalias" "defsyntax" "defrule"
           "defrules" "defstruct" "defclass" "defmethod" "defgeneric"
           "deftype" "defmessage" "definline" "defconst"
           "defcall-actor" "defproto" "deferror-class" "defapi"
           "deftyped" "if" "when" "unless" "cond" "case" "case-lambda"
           "match" "match*" "with" "with*" "begin" "begin0"
           "begin-syntax" "begin-annotation" "begin-foreign"
           "begin-ffi" "let" "let*" "letrec" "letrec*" "let-values"
           "letrec-values" "let-syntax" "letrec-syntax" "let-hash"
           "let/cc" "let/esc" "rec" "alet" "alet*" "awhen" "lambda"
           "lambda%" "import" "export" "declare" "include" "module"
           "extern" "require" "provide" "cond-expand" "set!" "apply"
           "eval" "and" "or" "not" "try" "catch" "finally" "error"
           "raise" "unwind-protect" "with-destroy" "guard"
           "syntax-case" "ast-case" "ast-rules" "with-syntax"
           "with-syntax*" "for" "for*" "for/collect" "for/fold" "while"
           "until" "for-each" "map" "foldl" "foldr" "spawn" "spawn*"
           "spawn/name" "spawn/group" "sync" "wait" "quote"
           "quasiquote" "unquote" "unquote-splicing" "parameterize"
           "parameterize*" "using" "chain" "is" "interface"
           "with-interface" "test-suite" "test-case" "check"
           "run-tests!")
         " "))
  (def *gerbil-builtins*
       (string-join
         '("cons" "car" "cdr" "caar" "cadr" "cdar" "cddr" "list"
           "list?" "null?" "pair?" "append" "reverse" "length" "assoc"
           "assq" "assv" "member" "memq" "memv" "vector" "vector-ref"
           "vector-set!" "vector-length" "make-vector" "vector->list"
           "list->vector" "string" "string-ref" "string-length"
           "string-append" "substring" "string->list" "list->string"
           "number?" "string?" "symbol?" "boolean?" "char?" "integer?"
           "real?" "zero?" "positive?" "negative?" "eq?" "eqv?"
           "equal?" "display" "write" "newline" "read" "read-line"
           "open-input-file" "open-output-file" "close-port"
           "current-input-port" "current-output-port" "make-hash-table"
           "hash-table?" "hash-get" "hash-put!" "hash-remove!"
           "hash-ref" "hash-key?" "hash-keys" "hash-values"
           "hash-for-each" "hash-map" "hash-copy" "void?" "procedure?"
           "fixnum?" "flonum?" "number->string" "string->number"
           "symbol->string" "string->symbol" "file-exists?"
           "delete-file" "rename-file" "directory-files"
           "create-directory" "current-directory" "path-expand"
           "path-directory" "path-strip-directory" "path-extension"
  ...
  (def (setup-cpp-styles! ed keywords (types #f))
       ...
       (when types (sci-send/string ed SCI_SETKEYWORDS types 1)))
  ...
  (def (qt-enable-code-folding! ed)
       ...
       (sci-send
         ...
         SC_MARK_TCORNER)
       (sci-send ed SCI_MARKERSETFORE SC_MARKNUM_FOLDER 8421504)
       (sci-send ed SCI_MARKERSETBACK SC_MARKNUM_FOLDER 2631720)
       (sci-send
         ed
         SCI_MARKERSETFORE
         SC_MARKNUM_FOLDEROPEN
  ...
  (def QT_ORG_TABLE 58)
  (def QT_ORG_SRC_KW 59)
  (def QT_ORG_SRC_STR 60)
  (def QT_ORG_SRC_COMMENT 61)
  (def QT_ORG_SRC_NUMBER 62)
       ...
       (when (> len 0)
         (let ([collector (*org-style-collector*)])
           (if collector
               (set-box!
                 collector
                 (cons (list pos len style) (unbox collector)))
               (begin
  ...
  (def (qt-org-highlight-heading! ed pos line line-len)
       (let* ([level (org-heading-stars-of-line line)]
         ...
         (let ([m (pregexp-match
                    "^(\\*+)\\s+(TODO|NEXT|DOING|WAITING|HOLD)\\s"
                    line)])
           (when m
             ...
             (let* ([tag-str (list-ref m 1)]
               ...
               (when tag-pos
                 (qt-org-style-range!
                   ed
                   (+ pos tag-pos)
                   (string-length tag-str)
                   QT_ORG_TAGS)))))))
  (def (qt-org-highlight-markup-pairs! ed pos line marker
         style)
  ...
  (def (qt-org-highlight-inline! ed pos line line-len)
       ...
       (qt-org-highlight-markup-pairs! ed pos line "/"
         QT_ORG_ITALIC)
       (qt-org-highlight-markup-pairs! ed pos line "_"
         QT_ORG_UNDERLINE)
       (qt-org-highlight-markup-pairs! ed pos line "="
         QT_ORG_VERBATIM)
       ...
       (let* ([lower (string-downcase (string-trim line))]
         ...
         (and m
              (let ([lang-str (list-ref m 1)])
                (cond
                  [(member lang-str '("sh" "shell" "bash" "zsh" "fish"))
                   'shell]
                  [(member lang-str '("python" "python3" "py")) 'python]
                  [(member
                     lang-str
                     '("scheme" "gerbil" "lisp" "elisp" "emacs-lisp" "cl"
                        "common-lisp"))
                   'scheme]
                  [(member
                     lang-str
  ...
  (def (qt-org-src-comment? trimmed lang)
       ...
       (and (> (string-length trimmed) 0)
            (case lang
              [(shell python ruby yaml) (string-prefix? "#" trimmed)]
              [(scheme lisp) (string-prefix? ";" trimmed)]
              [(c javascript go rust java)
               (or (string-prefix? "//" trimmed)
                   (string-prefix? "/*" trimmed))]
              [(sql) (string-prefix? "--" trimmed)]
              [else #f])))
  (def *dquote* (integer->char 34))
             ...
             (cond
               ...
               [(and in-str (char=? ch *dquote*))
                (qt-org-style-range!
                  ...
                  QT_ORG_SRC_STR)
                (loop (+ i 1) #f 0)]
               [else (loop (+ i 1) in-str str-start)])))))
             ...
             (let* ([line (list-ref lines i)]
               ...
               (cond
                 ...
                 [(and (pair? state) (eq? (car state) 'block))
                  (let ([lang (cdr state)])
                    (if lang
                        (qt-org-highlight-src-line! ed pos line line-len
                          lang)
                        (qt-org-style-line!
                          ed
                          pos
  ...
  (def brace-bg-g 64)
  (def brace-bg-b 96)
  (def brace-err-fg-r 255)
  (def brace-err-fg-g 64)
  (def brace-err-fg-b 64)
  (def brace-err-bg-r 96)
  (def brace-err-bg-g 32)
  (def brace-err-bg-b 32)
  (def (brace-open? ch)
  ...
  (def (qt-update-visual-decorations! ed)
       ...
       (let* ([pos (qt-plain-text-edit-cursor-position ed)]
              ...
              [pos (if (> pos doc-len)
                       (begin (sci-send ed SCI_GOTOPOS doc-len) doc-len)
                       pos)]
              [line (qt-plain-text-edit-cursor-line ed)]
              [text (qt-plain-text-edit-text ed)])
         (qt-extra-selections-clear! ed)
         (qt-extra-selection-add-line! ed line cline-bg-r cline-bg-g
           cline-bg-b)
         (when *qt-show-paren-enabled*
           (let check ([check-pos pos])
             (let-values ([(match-pos matched?)
                           (find-matching-brace text check-pos)])
               (cond
                 [match-pos
                  (if matched?
                      (begin
                        (qt-extra-selection-add-range! ed check-pos 1 brace-fg-r brace-fg-g brace-fg-b
                          brace-bg-r brace-bg-g brace-bg-b 'bold: #t)
                        (qt-extra-selection-add-range! ed match-pos 1 brace-fg-r brace-fg-g brace-fg-b
                          brace-bg-r brace-bg-g brace-bg-b 'bold: #t))
                      (begin
                        (qt-extra-selection-add-range! ed check-pos 1 brace-err-fg-r brace-err-fg-g
                          brace-err-fg-b brace-err-bg-r brace-err-bg-g
                          brace-err-bg-b 'bold: #t)
                        (qt-extra-selection-add-range! ed match-pos 1 brace-err-fg-r brace-err-fg-g
                          brace-err-fg-b brace-err-bg-r brace-err-bg-g
                          brace-err-bg-b 'bold: #t)))]
  ...
  (define *search-highlight-active*--cell (vector #f))
  (def search-fg-r 0)
  (def search-fg-g 0)
  (def search-fg-b 0)
  (def search-bg-r 255)
           ...
           (let loop ([i 0])
             (when (< (+ i pat-len) len)
               (let ([found (string-contains text pattern i)])
                 (when found
                   (qt-extra-selection-add-range! ed found pat-len search-fg-r search-fg-g search-fg-b
  ...
  (define-syntax *search-highlight-active*
    (identifier-syntax
      [id (vector-ref *search-highlight-active*--cell 0)]
      [(set! id val) (vector-set!
                       *search-highlight-active*--cell
                       0
                       val)]))
  (define-syntax *qt-show-paren-enabled*
    (identifier-syntax
      [id (vector-ref *qt-show-paren-enabled*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-show-paren-enabled*--cell
                       0
                       val)]))
  (define-syntax *qt-delete-selection-enabled*
    (identifier-syntax
      [id (vector-ref *qt-delete-selection-enabled*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-delete-selection-enabled*--cell
                       0
                       val)])))
