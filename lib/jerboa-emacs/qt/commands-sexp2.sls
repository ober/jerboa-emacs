...
;;; Source: src/jerboa-emacs/qt/commands-sexp2.ss

(library (jerboa-emacs qt commands-sexp2)
  (export *tab-width* cmd-cycle-tab-width *indent-tabs-mode*
   cmd-toggle-indent-tabs-mode cmd-replace-string
   cmd-string-insert-file cmd-goto-column
   cmd-goto-line-relative cmd-recenter-top cmd-recenter-bottom
   cmd-upcase-char cmd-downcase-char cmd-toggle-case-at-point
   cmd-capitalize-region cmd-copy-buffer-name
   cmd-copy-current-line cmd-copy-word cmd-copy-file-path
   cmd-copy-line-number cmd-copy-region-as-kill
   cmd-yank-whole-line cmd-transpose-windows cmd-desktop-save
   cmd-desktop-read cmd-desktop-clear *qt-history-file*
   cmd-savehist-save cmd-savehist-load *savehist-mode-enabled*
   cmd-savehist-mode cmd-paredit-wrap-curly
   cmd-complete-at-point cmd-org-agenda cmd-org-export
   cmd-org-priority inflection-split-to-tokens tokens->snake
   tokens->upper tokens->kebab tokens->camel tokens->pascal
   inflection-detect-style inflection-next-style
   inflection-apply-style qt-word-at-cursor-bounds
   cmd-string-inflection-cycle cmd-string-inflection-snake-case
  ...
  (import
   ...
   (jerboa-emacs qt commands-sexp) (jerboa core)
   (jerboa runtime))
  (define *tab-width*--cell (vector 4))
  (def (cmd-cycle-tab-width app)
       "Cycle tab stop width: 2 -> 4 -> 8 -> 2."
       (set! *tab-width*
         (cond [(= *tab-width* 2) 4] [(= *tab-width* 4) 8] [else 2]))
       (echo-message!
         (app-state-echo app)
         (string-append "Tab width: " (number->string *tab-width*))))
  (define *indent-tabs-mode*--cell (vector #f))
       ...
       (let* ([buf (current-qt-buffer app)]
         ...
         (if path
             (begin
               (app-state-kill-ring-set!
                 app
                 (cons path (app-state-kill-ring app)))
               (echo-message!
                 (app-state-echo app)
                 (string-append "Copied: " path)))
  ...
  (def (cmd-copy-line-number app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (app-state-kill-ring-set!
           app
           (cons s (app-state-kill-ring app)))
         (echo-message!
           (app-state-echo app)
           (string-append "Copied line: " s))))
         ...
         (if (not text)
             ...
             (begin
               (let* ([pos (qt-plain-text-edit-cursor-position ed)]
                      ...
                      [line-start (line-start-position
                                    full
                                    (qt-plain-text-edit-cursor-line ed))])
       ...
       (let* ([fr (app-state-frame app)]
         ...
         (if (< (length wins) 2)
             ...
             (let* ([cur (qt-current-window fr)]
               ...
               (qt-edit-window-buffer-set! cur buf2)
               (qt-edit-window-buffer-set! other buf1)
  ...
  (def (cmd-savehist-save app)
       ...
       (with-catch
         (lambda (e)
           (echo-error! (app-state-echo app) "Error saving history"))
         (lambda ()
           ...
           (echo-message!
             ...
             (string-append
               "History saved: "
               (number->string (length *mx-command-history*))
               " entries")))))
  (def (cmd-savehist-load app)
       "Load command history from disk."
       (with-catch
         (lambda (e)
           (echo-error! (app-state-echo app) "Error loading history"))
         (lambda ()
  ...
  (define *savehist-mode-enabled*--cell (vector #f))
  (def (cmd-savehist-mode app)
       ...
       (when *savehist-mode-enabled* (cmd-savehist-load app))
       (echo-message!
         (app-state-echo app)
         (if *savehist-mode-enabled*
             "savehist-mode ON"
             "savehist-mode OFF")))
  ...
  (def (cmd-complete-at-point app)
       ...
       (let* ([ed (current-qt-editor app)]
              ...
              [pos (qt-plain-text-edit-cursor-position ed)]
              [len (string-length text)])
         (let* ([prefix-start (let loop ([i (- pos 1)])
                                (if (or (< i 0)
                   ...
                   (if (null? words)
                       ...
                       (if (= (length words) 1)
                           (begin
                             ...
                             (echo-message!
                               echo
                               (string-append "Completed: " (car words))))
                           (begin
                             (sci-send
                               ed
  ...
  (def (cmd-org-agenda app)
       ...
       (let* ([fr (app-state-frame app)]
         ...
         (for-each
           (lambda (buf)
             (let ([fp (buffer-file-path buf)] [name (buffer-name buf)])
               (when fp
             ...
             (when (pair? ls)
               (let ([l (car ls)])
                 (when (or (string-contains l "TODO ")
                   ...
                   (set! items
                     (cons
                       (string-append "  " cur-name ":" (number->string n)
                         ": " (string-trim l))
                       items))))
               (loop (cdr ls) (+ n 1)))))
         ...
         (if (or (= (string-length line) 0)
             ...
             (let* ([new-line (or (replace-pri "[#A] " "[#B] ")
               ...
               (echo-message!
                 echo
                 (cond
                   [(string-contains new-line "[#A]") "Priority: A"]
       ...
       (let loop ([chars (string->list word)]
         ...
         (cond
           ...
           [(and (char-upper-case? (car chars))
                 (> (string-length cur) 0)
                 (char-lower-case?
                   (string-ref cur (- (string-length cur) 1))))
            (loop
              (cdr chars)
           ...
           [else
            (loop
              (cdr chars)
              (string-append cur (string (char-downcase (car chars))))
              tokens)])))
  (def (tokens->snake tokens) (string-join tokens "_"))
  (def (tokens->upper tokens)
       (string-upcase (string-join tokens "_")))
           ...
           (apply
             ...
             (map (lambda (t)
                    (if (= (string-length t) 0)
                        ""
                        (string-append
                          (string (char-upcase (string-ref t 0)))
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (not bounds)
             ...
             (let* ([text (qt-plain-text-edit-text ed)]
                    ...
                    [new-word (tokens->snake
                                (inflection-split-to-tokens word))])
               (qt-plain-text-edit-set-selection!
                 ed
                 (car bounds)
                 (cdr bounds))
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (not bounds)
             ...
             (let* ([text (qt-plain-text-edit-text ed)]
               ...
               (qt-plain-text-edit-insert-text! ed new-word)
               (echo-message! echo (string-append word " → " new-word))))))
  (define-syntax *tab-width*
    (identifier-syntax
      [id (vector-ref *tab-width*--cell 0)]
      [(set! id val) (vector-set! *tab-width*--cell 0 val)]))
  (define-syntax *indent-tabs-mode*
    (identifier-syntax
      [id (vector-ref *indent-tabs-mode*--cell 0)]
      [(set! id val) (vector-set!
                       *indent-tabs-mode*--cell
                       0
                       val)]))
  (define-syntax *qt-history-file*
    (identifier-syntax
      [id (vector-ref *qt-history-file*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-history-file*--cell
                       0
                       val)]))
  (define-syntax *savehist-mode-enabled*
    (identifier-syntax
      [id (vector-ref *savehist-mode-enabled*--cell 0)]
      [(set! id val) (vector-set!
                       *savehist-mode-enabled*--cell
                       0
                       val)])))
