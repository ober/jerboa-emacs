...
(library (jerboa-emacs qt magit)
  (export magit-run-git magit-run-git-stdin magit-run-git/async
   magit-parse-status magit-status-label
   magit-partition-entries magit-format-status
   magit-file-at-point magit-get-line magit-find-section
   magit-in-diff-line? magit--scan-backward-for
   magit--skip-to-eol magit--find-hunk-end magit-hunk-at-point
   magit-branch-names magit-remote-names magit-current-branch
   magit-upstream-branch magit-recent-commits)
  (import
       ...
       (with-catch
         ...
         (lambda ()
           (let* ([proc (open-process
                          (list 'path: "/usr/bin/git" 'arguments: args
  ...
  (def (magit-run-git-stdin input args dir)
       ...
       (with-catch
         ...
         (lambda ()
           (let ([proc (open-process
                         (list 'path: "/usr/bin/git" 'arguments: args
                           'directory: dir 'stdin-redirection: #t
  ...
  (def (magit-format-status entries branch dir)
       ...
       (let-values ([(staged unstaged untracked)
                     (magit-partition-entries entries)])
         (let ([out (open-output-string)])
           ...
           (when (not (null? untracked))
             (display
               ...
               out)
             (for-each
               (lambda (e) (display (string-append "  " (cdr e) "\n") out))
               untracked))
           (when (and (null? staged)
                      (null? unstaged)
                      (null? untracked))
           ...
           (display
             ...
             out)
           (display
             "      b=branch k=checkout f=fetch F=pull P=push z=stash Z=pop\n"
             out)
           (display
             "      x=cherry-pick X=revert w=worktree g=refresh n/p=nav q=quit\n"
             out)
           (display
             "      (s/u on diff hunk = stage/unstage hunk)\n"
             out)
             ...
             [(char=? (string-ref text i) #\newline)
              (let ([next (+ i 1)])
                (cond
                  [(and (<= (+ next 16) len)
                        (string=?
                          (substring text next (+ next 16))
                          "Unstaged changes"))
                   'unstaged]
                  [(and (<= (+ next 14) len)
                        (string=?
                          (substring text next (+ next 14))
                          "Staged changes"))
                   'staged]
                  [(and (<= (+ next 15) len)
                        (string=?
                          (substring text next (+ next 15))
                          "Untracked files"))
                   'untracked]
                  [else (loop (- i 1))]))]
             [else (loop (- i 1))]))))
         ...
         (let loop ([i pos])
           (cond
             ...
             [(and (or (= i 0)
                   ...
                   (string=?
                     (substring text i (+ i (string-length prefix)))
                     prefix))
              i]
             [else (loop (- i 1))]))))
             ...
             [(char=? (string-ref text i) #\newline)
              (let ([next (+ i 1)])
                (cond
                  ...
                  [(and (<= (+ next 11) len)
                        (string=?
                          (substring text next (+ next 11))
                          "diff --git "))
                   next]
                  [(and (<= (+ next 8) len)
                        (or (string-prefix?
                              "Staged "
                              (substring text next (min len (+ next 16))))
                            (string-prefix?
                              "Unstaged"
                              (substring text next (min len (+ next 16))))
                            (string-prefix?
  ...
  (def (magit-hunk-at-point text pos)
       ...
       (let ([len (string-length text)]
         ...
         (if (not (magit-in-diff-line? line))
             (values #f #f)
             (let ([diff-start (magit--scan-backward-for
                                 text
                                 pos
                                 "diff --git ")])
               (if (not diff-start)
                   ...
                   (let* ([header-end (magit--skip-to-eol text diff-start)]
                          ...
                          [plus3-start (magit--scan-backward-for
                                         text
                                         pos
                                         "+++ ")]
                          [plus3-fwd (or plus3-start
                                         (magit--scan-backward-for
                                           text
                                           (+ diff-start 20)
                                           "+++ "))]
                                         ...
                                         (let loop ([i diff-start])
                                           (cond
                                             ...
                                             [(and (or (= i 0)
                                              ...
                                              i]
                                             [else (loop (+ i 1))])))]
                          [header-end-pos (if plus3-pos
                                              (magit--skip-to-eol
                                                text
                                                plus3-pos)
                                              header-end)]
                          [diff-header (substring
                                         text
                                         diff-start
                                         header-end-pos)]
                          [hunk-start (magit--scan-backward-for
                                        text
                                        pos
                                        "@@")]
                          [hunk-end (and hunk-start
                                         (magit--find-hunk-end
                                           text
                                           hunk-start))])
                     (if (and hunk-start hunk-end file-name)
                         (let ([hunk-text (substring
                                            text
                                            hunk-start
                                            hunk-end)])
                           (values
  ...
  (def (magit-current-branch dir)
       ...
       (string-trim
         (magit-run-git '("rev-parse" "--abbrev-ref" "HEAD") dir)))
