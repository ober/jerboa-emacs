...
;;; Source: src/jerboa-emacs/qt/commands-core2.ss

(library (jerboa-emacs qt commands-core2)
  (export *editor-completers* word-char-for-complete?
   get-word-prefix collect-buffer-words
   get-or-create-completer! text-line-position
   cmd-keyboard-quit cmd-set-scroll-margin
   cmd-toggle-scroll-margin cmd-toggle-save-place-mode
  ...
  (import
   ...
   (jerboa-emacs qt commands-core) (jerboa core)
   (jerboa runtime))
  (define *editor-completers*--cell
  ...
  (def (get-word-prefix ed)
       ...
       (let* ([pos (qt-plain-text-edit-cursor-position ed)]
         ...
         (let loop ([i (- pos 1)])
           (if (or (< i 0)
                   (not (word-char-for-complete? (string-ref text i))))
               (if (< (+ i 1) pos) (substring text (+ i 1) pos) "")
               (loop (- i 1))))))
  (def (collect-buffer-words text)
       "Collect unique words from buffer text."
       (let ([words (make-hash-table)] [len (string-length text)])
         (let loop ([i 0])
           (if (>= i len)
               (hash-keys words)
  ...
  (def (get-or-create-completer! ed app)
       ...
       (or (hash-get *editor-completers* ed)
           (let ([c (qt-completer-create (list))])
             ...
             (cond
               ...
               [(char=? (string-ref text i) #\newline)
                (if (= (+ line 1) line-num)
                    ...
                    (loop (+ i 1) (+ line 1)))]
               [else (loop (+ i 1) line)]))))
  (def (cmd-keyboard-quit app) (quit-flag-set!)
         ...
         (let pp ([val obj] [level 0])
           (let ([prefix (make-string (* level indent) #\space)])
             (cond
               [(hash-table? val)
                (display "{\n" out)
                (let ([keys (sort (hash-keys val) string<?)] [first #t])
                  (for-each
                    (lambda (k)
                      (unless first (display ",\n" out))
                      (display
                        (make-string (* (+ level 1) indent) #\space)
                        out)
                      (write k out)
                      (display ": " out)
         ...
         (with-catch
           ...
           (lambda ()
             (let* ([obj (call-with-input-string text read-json)]
               ...
               (qt-plain-text-edit-set-cursor-position!
                 ed
                 (min pos (string-length minified)))
               (echo-message!
                 echo
                 (string-append
                   "JSON minified ("
                   (number->string (string-length minified))
                   " bytes)")))))))
  (def (cmd-json-pretty-print-region app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (= sel-start sel-end)
             (echo-message!
               (app-state-echo app)
               "Select JSON region first")
             (let* ([text (qt-plain-text-edit-text ed)]
                    [region (substring text sel-start sel-end)])
               (with-catch
                 (lambda (e)
                   (echo-error! (app-state-echo app) "Invalid JSON"))
                 (lambda ()
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (= sel-start sel-end)
             ...
             (let* ([text (qt-plain-text-edit-text ed)]
               ...
               (qt-plain-text-edit-set-cursor-position!
                 ed
                 (+ sel-start (string-length encoded)))
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (= sel-start sel-end)
             ...
             (let* ([text (qt-plain-text-edit-text ed)]
               ...
               (qt-plain-text-edit-set-cursor-position!
                 ed
                 (+ sel-start (string-length decoded)))
  ...
  (def (cmd-xml-format app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (with-catch
           (lambda (e)
             (echo-error! (app-state-echo app) "xmllint not available"))
           (lambda ()
             (let* ([proc (open-process
                            (list 'path: "xmllint" 'arguments:
                              '("--format" "-") 'stdin-redirection: #t
         ...
         (if (= sel-start sel-end)
             ...
             (let* ([text (qt-plain-text-edit-text ed)]
               ...
               (qt-plain-text-edit-set-selection!
                 ed
                 sel-start
                 (+ sel-start (string-length result)))
         ...
         (let loop ([i 0] [count 0] [in-word #f])
           (if (>= i (string-length line-text))
               (echo-message!
                 ...
                 (string-append
                   "Words in line: "
                   (number->string (if in-word (+ count 1) count))))
              ...
              (if (and file hunk-line)
                  (let* ([offset (- cur-line (+ l 1))]
                    ...
                    (if (file-exists? file)
                        ...
                        (echo-error!
                          (app-state-echo app)
                          (string-append "File not found: " file))))
                  (echo-error!
                    (app-state-echo app)
                    "No diff context found"))]
             [else
              (let ([line-text (if (< l (length lines))
                                   (list-ref lines l)
                                   "")])
                (cond
                  [(and (not hunk-line) (string-prefix? "@@ " line-text))
                   (let* ([plus-pos (string-contains line-text "+")]
                          [comma-pos (and plus-pos
                                          (string-contains
                                            line-text
                                            ","
                                            (+ plus-pos 1)))]
                          [space-pos (and plus-pos
                                          (string-contains
                                            line-text
                                            " "
                                            (+ plus-pos 1)))]
                          [end-pos (or comma-pos
                                       space-pos
                                       (string-length line-text))]
                          [num-str (and plus-pos
                                        (substring
                                          line-text
                                          (+ plus-pos 1)
                                          end-pos))]
  ...
  (def (cmd-insert-date-iso app)
       ...
       (let* ([ed (current-qt-editor app)]
              [out (open-process
                     (list 'path: "date" 'arguments: '("+%Y-%m-%d")))]
  ...
  (def (cmd-org-insert-src-block app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (echo-message!
           (app-state-echo app)
           "Source block inserted")))
  (define *qt-org-clock-line*--cell (vector #f))
  (define *qt-org-clock-heading*--cell (vector #f))
  ...
  (def (cmd-org-clock-out app)
       ...
       (let* ([ed (current-qt-editor app)]
              ...
              [lines (string-split text #\newline)]
              [echo (app-state-echo app)]
              [clock-line (let loop ([i (- (length lines) 1)])
                            (cond
                              [(< i 0) #f]
  ...
  (define-syntax *editor-completers*
    (identifier-syntax
      [id (vector-ref *editor-completers*--cell 0)]
      [(set! id val) (vector-set!
                       *editor-completers*--cell
                       0
                       val)]))
  (define-syntax *qt-tab-width*
    (identifier-syntax
      [id (vector-ref *qt-tab-width*--cell 0)]
      [(set! id val) (vector-set! *qt-tab-width*--cell 0 val)]))
  (define-syntax *qt-org-clock-line*
    (identifier-syntax
      [id (vector-ref *qt-org-clock-line*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-org-clock-line*--cell
                       0
                       val)]))
  (define-syntax *qt-org-clock-heading*
    (identifier-syntax
      [id (vector-ref *qt-org-clock-heading*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-org-clock-heading*--cell
                       0
                       val)])))
