...
;;; Source: src/jerboa-emacs/qt/commands-file2.ss

(library (jerboa-emacs qt commands-file2)
  (export cmd-tabify cmd-untabify cmd-base64-encode-region
   cmd-base64-decode-region rot13-char cmd-rot13-region
   cmd-hexl-mode *word-wrap-on* cmd-toggle-word-wrap
   *whitespace-mode-on* cmd-toggle-whitespace ws-bg-r ws-bg-g
   ws-bg-b qt-highlight-trailing-whitespace!
   cmd-toggle-truncate-lines *case-fold-search-qt*
   cmd-toggle-case-fold-search *overwrite-mode*
   cmd-toggle-overwrite-mode *fill-column-indicator*
   cmd-toggle-auto-fill cmd-set-fill-column
   *highlighting-disabled* cmd-toggle-highlighting
   cmd-toggle-visual-line-mode cmd-toggle-fill-column-indicator
   cmd-calc cmd-describe-bindings cmd-describe-char
   cmd-describe-key-briefly cmd-bookmark-bmenu-list
   cmd-clone-indirect-buffer cmd-apply-macro-to-region
   cmd-magit-unstage-file cmd-text-scale-increase
   cmd-text-scale-decrease cmd-text-scale-reset qt-md-wrap
   cmd-markdown-bold cmd-markdown-italic cmd-markdown-code
   cmd-markdown-insert-bold cmd-markdown-insert-code
   cmd-markdown-insert-italic cmd-markdown-insert-image
   cmd-markdown-insert-list-item cmd-markdown-preview-outline
   cmd-markdown-image qt-md-hlevel cmd-markdown-heading
   cmd-markdown-hr cmd-markdown-checkbox
   cmd-markdown-code-block cmd-markdown-table cmd-markdown-link
   cmd-markdown-list-item qt-run-cmd cmd-git-blame-line
   cmd-eval-region-and-replace cmd-comment-box
   cmd-goto-matching-bracket cmd-insert-uuid-v4
   cmd-insert-date-formatted cmd-insert-date-time-stamp
  ...
  (import
   ...
   (jerboa-emacs qt commands-file) (jerboa core)
   (jerboa runtime))
  (def (cmd-tabify app)
  ...
  (def (cmd-base64-decode-region app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (not mark)
             ...
             (with-catch
               (lambda (e)
                 (echo-error! (app-state-echo app) "Base64 decode error"))
               (lambda ()
                 (let* ([pos (qt-plain-text-edit-cursor-position ed)]
                        [start (min mark pos)]
                        [end (max mark pos)]
       ...
       (cond
         [(and (char>=? ch #\a) (char<=? ch #\z))
          (integer->char
            (+ (char->integer #\a)
               (modulo
                 (+ (- (char->integer ch) (char->integer #\a)) 13)
                 26)))]
         [(and (char>=? ch #\A) (char<=? ch #\Z))
          (integer->char
            (+ (char->integer #\A)
           ...
           (when (< offset len)
             (let* ([end (min (+ offset 16) len)]
               ...
               (let* ([addr (let ([h (number->string offset 16)])
                              (string-append
                                (make-string
                                  (max 0 (- 8 (string-length h)))
                                  #\0)
                                h))]
                      [hex-str (string-join (reverse hex-parts) " ")]
                      [pad (make-string
                             (max 0 (- 48 (string-length hex-str)))
                             #\space)]
                      [ascii-str (apply
                                   string-append
                                   (reverse ascii-parts))])
                 (set! lines
                   (cons
                     (string-append addr "  " hex-str pad "  |" ascii-str
                       "|")
                     lines))))
             (loop (+ offset 16))))
  ...
  (define *word-wrap-on*--cell (vector #f))
  ...
  (define *whitespace-mode-on*--cell (vector #f))
  (def (cmd-toggle-whitespace app)
       ...
       (if *whitespace-mode-on*
           (begin
             ...
             (echo-message!
               (app-state-echo app)
               "Whitespace highlighting enabled"))
           (begin
             (qt-update-visual-decorations! (current-qt-editor app))
             (echo-message!
               (app-state-echo app)
               "Whitespace highlighting disabled"))))
  (def ws-bg-r 128)
  (def ws-bg-g 32)
  (def ws-bg-b 32)
                   ...
                   [else
                    (let ([trail-start (+ i 1)])
                      (when (< trail-start line-len)
                        (qt-extra-selection-add-range! ed (+ pos trail-start) (- line-len trail-start)
                          255 255 255 ws-bg-r ws-bg-g ws-bg-b 'bold:
                          #f)))])))
  ...
  (define *case-fold-search-qt*--cell (vector #t))
  ...
  (define *overwrite-mode*--cell (vector #f))
  (def (cmd-toggle-overwrite-mode app)
       "Toggle overwrite mode (Insert vs Overwrite)."
       (set! *overwrite-mode* (not *overwrite-mode*))
       (let ([ed (current-qt-editor app)])
         (sci-send ed 2186 (if *overwrite-mode* 1 0)))
       (echo-message!
         (app-state-echo app)
         (if *overwrite-mode*
             "Overwrite mode ON"
             "Overwrite mode OFF")))
  (define *fill-column-indicator*--cell (vector #f))
  ...
  (define *highlighting-disabled*--cell (vector #f))
  ...
  (def (cmd-toggle-fill-column-indicator app)
       ...
       (let ([ed (current-qt-editor app)])
         (if *fill-column-indicator*
             (begin
               (sci-send ed 2361 *fill-column* 0)
               (sci-send ed 2363 1 0))
             (sci-send ed 2363 0 0))
         (echo-message!
           (app-state-echo app)
           (if *fill-column-indicator*
               (string-append
                 "Fill column indicator at "
                 (number->string *fill-column*))
  ...
  (def (cmd-clone-indirect-buffer app)
       ...
       (let* ([fr (app-state-frame app)]
         ...
         (echo-message!
           (app-state-echo app)
           (string-append "Cloned to " clone-name))))
         ...
         (if (= ss se)
             (let* ([p (qt-plain-text-edit-cursor-position ed)]
                    [n (string-append
                         ...
                         pre
                         suf
                         (substring text p (string-length text)))])
               (qt-plain-text-edit-set-text! ed n)
               (qt-plain-text-edit-set-cursor-position!
                 ed
                 (+ p (string-length pre))))
             (let* ([s (substring text ss se)]
                    [n (string-append (substring text 0 ss) pre s suf
                         (substring text se (string-length text)))])
               (qt-plain-text-edit-set-text! ed n)
               (qt-plain-text-edit-set-cursor-position!
                 ed
                 (+ se (string-length pre) (string-length suf)))))))
  (def (cmd-markdown-bold app)
       "Bold."
       (qt-md-wrap (current-qt-editor app) "**" "**"))
  ...
  (def (cmd-markdown-heading app)
       ...
       (let* ([ed (current-qt-editor app)]
              ...
              [level (qt-md-hlevel line)]
              [rep (cond
                     [(= level 0) (string-append "# " line)]
                     [(>= level 6)
                      (let lp ([s line])
                        (if (and (> (string-length s) 0)
                                 (char=? (string-ref s 0) #\#))
                            (lp (substring s 1 (string-length s)))
                            (string-trim s)))]
         ...
         (qt-plain-text-edit-set-text!
           ...
           (string-append
             (substring text 0 ls)
             rep
             (substring text le len)))
         (qt-plain-text-edit-set-cursor-position!
           ed
           (+ ls (min (string-length rep) (- pos ls))))))
  ...
  (def (cmd-markdown-table app)
       "Table."
       (let* ([c (or (string->number
                       (or (qt-echo-read-string
                             app
                             "Columns (default 3): ")
                           "3"))
                     3)]
              [h (string-join (make-list c " Header ") "|")]
              [s (string-join (make-list c "--------") "|")]
  ...
  (def (cmd-markdown-link app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (when (and url (> (string-length url) 0))
           (let* ([ss (qt-plain-text-edit-selection-start ed)]
                  [se (qt-plain-text-edit-selection-end ed)]
                  [text (qt-plain-text-edit-text ed)]
                  [lt (if (= ss se) url (substring text ss se))]
                  [lk (string-append "[" lt "](" url ")")])
             (if (= ss se)
                 (qt-plain-text-edit-insert-text! ed lk)
                 (let ([n (string-append
                            (substring text 0 ss)
                            lk
                            (substring text se (string-length text)))])
  ...
  (def (qt-run-cmd args)
       ...
       (with-exception-catcher
         ...
         (lambda ()
           (let ([p (open-process
                      (list 'path: (car args) 'arguments: (cdr args)
  ...
  (def (cmd-git-blame-line app)
       ...
       (let* ([buf (current-qt-buffer app)]
         ...
         (if (not path)
             ...
             (with-catch
               (lambda (e)
                 (echo-error! (app-state-echo app) "Git blame failed"))
               (lambda ()
                 (let* ([ed (current-qt-editor app)]
                        [text (qt-plain-text-edit-text ed)]
                        [pos (qt-plain-text-edit-cursor-position ed)]
                                 ...
                                 (if (>= i pos)
                                     ...
                                     (lp (+ i 1)
                                         (if (char=?
                                             ...
                                             n)))))]
                        [out (qt-run-cmd
                               (list "git" "-C" (path-directory path) "blame"
                                 "-L"
                                 (string-append
                                   (number->string ln)
                                   ","
                        ...
                        [commit (if (pair? ls)
                                    (let ([p (string-split
                                      ...
                                      (if (pair? p)
                                          ...
                                          "?"))
                                    "?")]
                        [author (let lp ([l (if (pair? ls) (cdr ls) '())])
                                  (if (null? l)
                                      "?"
                                      (if (string-prefix?
                                            "author "
  ...
  (def (cmd-eval-region-and-replace app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (= ss se)
             ...
             (with-catch
               (lambda (e) (echo-error! (app-state-echo app) "Eval error"))
               (lambda ()
                 (let* ([expr (substring
  ...
  (def (cmd-comment-box app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (= ss se)
             (echo-message! (app-state-echo app) "No selection")
             (let* ([text (qt-plain-text-edit-text ed)]
                    [sel (substring text ss se)]
                    [lines (string-split sel #\newline)]
                    [mx (apply max (map string-length lines))]
                    [bdr (string-append ";; " (make-string (+ mx 2) #\-))]
                    [bx (with-output-to-string
                          (lambda ()
                            (display bdr)
                            (display "\n")
                            (for-each
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (when (< pos len)
           (let* ([ch (string-ref text pos)]
                       ...
                       [(memv ch opn)
                        (let lp ([i (+ pos 1)] [d 1])
                          (cond
                            [(>= i len) #f]
                            [(= d 0) (- i 1)]
                            [(memv (string-ref text i) opn)
                             (lp (+ i 1) (+ d 1))]
                            [(memv (string-ref text i) cls)
                             (lp (+ i 1) (- d 1))]
                            [else (lp (+ i 1) d)]))]
                       [(memv ch cls)
                        (let lp ([i (- pos 1)] [d 1])
             ...
             (if m
                 (begin
                   (qt-plain-text-edit-set-cursor-position! ed m)
  ...
  (def (cmd-insert-uuid-v4 app)
       ...
       (let* ([hx "0123456789abcdef"]
              [rh (lambda ()
                    (string
                      (string-ref hx (random-integer 16))
                      (string-ref hx (random-integer 16))))]
              [uuid (string-append (rh) (rh) (rh) (rh) "-" (rh) (rh) "-4"
                     (substring (rh) 1 2) (rh) "-"
                     (string (string-ref hx (+ 8 (random-integer 4))))
                     (substring (rh) 1 2) (rh) "-" (rh) (rh) (rh) (rh) (rh)
                     (rh))])
         (qt-plain-text-edit-insert-text!
           (current-qt-editor app)
           uuid)
  ...
  (def (cmd-insert-date-formatted app)
       ...
       (let ([fmt (qt-echo-read-string
                    app
                    "Date format (e.g. %Y-%m-%d): ")])
         (when (and fmt (> (string-length fmt) 0))
           (let ([str (string-trim
                        (qt-run-cmd
                          (list "date" (string-append "+" fmt))))])
             (if (string=? str "")
                 (echo-error! (app-state-echo app) "Invalid format")
                 (qt-plain-text-edit-insert-text!
                   (current-qt-editor app)
                   str))))))
  (def (cmd-insert-date-time-stamp app)
       ...
       (qt-plain-text-edit-insert-text!
         (current-qt-editor app)
         (string-trim (qt-run-cmd '("date" "+%Y-%m-%d %H:%M:%S")))))
  (def (cmd-insert-char-by-code app)
       "Insert char by code point."
       (let ([input (qt-echo-read-string
                      app
                      "Code point (65 or #x41): ")])
         (when (and input (> (string-length input) 0))
           (let ([n (cond
                      [(string-prefix? "#x" input)
                       (string->number
                         (substring input 2 (string-length input))
                         16)]
                      [(string-prefix? "0x" input)
                       (string->number
                         (substring input 2 (string-length input))
                         16)]
             ...
             (if (and n (> n 0) (< n 1114112))
                 ...
                 (echo-error!
                   (app-state-echo app)
                   "Invalid code point"))))))
  (define-syntax *word-wrap-on*
    (identifier-syntax
      [id (vector-ref *word-wrap-on*--cell 0)]
      [(set! id val) (vector-set! *word-wrap-on*--cell 0 val)]))
  (define-syntax *whitespace-mode-on*
    (identifier-syntax
      [id (vector-ref *whitespace-mode-on*--cell 0)]
      [(set! id val) (vector-set!
                       *whitespace-mode-on*--cell
                       0
                       val)]))
  (define-syntax *case-fold-search-qt*
    (identifier-syntax
      [id (vector-ref *case-fold-search-qt*--cell 0)]
      [(set! id val) (vector-set!
                       *case-fold-search-qt*--cell
                       0
                       val)]))
  (define-syntax *overwrite-mode*
    (identifier-syntax
      [id (vector-ref *overwrite-mode*--cell 0)]
      [(set! id val) (vector-set! *overwrite-mode*--cell 0 val)]))
  (define-syntax *fill-column-indicator*
    (identifier-syntax
      [id (vector-ref *fill-column-indicator*--cell 0)]
      [(set! id val) (vector-set!
                       *fill-column-indicator*--cell
                       0
                       val)]))
  (define-syntax *highlighting-disabled*
    (identifier-syntax
      [id (vector-ref *highlighting-disabled*--cell 0)]
      [(set! id val) (vector-set!
                       *highlighting-disabled*--cell
                       0
                       val)])))
