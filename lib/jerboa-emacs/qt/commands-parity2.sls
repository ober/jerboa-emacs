...
;;; Source: src/jerboa-emacs/qt/commands-parity2.ss

(library (jerboa-emacs qt commands-parity2)
  (export cmd-text-mode *prog-modes* qt-set-major-mode!
   cmd-shell-script-mode cmd-python-mode cmd-c-mode
   cmd-c++-mode cmd-js-mode cmd-typescript-mode cmd-go-mode
   cmd-rust-mode cmd-ruby-mode cmd-markdown-mode
   cmd-org-mode-switch cmd-yaml-mode cmd-json-mode cmd-sql-mode
   cmd-lua-mode cmd-html-mode cmd-css-mode cmd-scheme-mode
   cmd-check-parens cmd-count-lines-page cmd-how-many
   cmd-delete-directory cmd-set-file-modes cmd-dired-do-chown
   cmd-butterfly *qt-debug-on-entry-list* cmd-debug-on-entry
   cmd-cancel-debug-on-entry cmd-vc-pull cmd-vc-push
   cmd-magit-stage-file *qt-dap-breakpoints* *qt-dap-process*
   *qt-dap-program* *qt-dap-output* gdb-send!
   gdb-read-until-prompt! gdb-show-output! cmd-dap-debug
   cmd-dap-breakpoint-toggle dap-step-command!
   cmd-dap-step-over cmd-dap-step-in cmd-dap-step-out
   cmd-dap-continue cmd-dap-repl *qt-smerge-mine-marker*
   *qt-smerge-sep-marker* *qt-smerge-other-marker*
   qt-smerge-find-conflict qt-smerge-count
   qt-smerge-extract-mine qt-smerge-extract-other
   cmd-smerge-next cmd-smerge-prev cmd-smerge-keep-mine
   cmd-smerge-keep-other cmd-smerge-keep-both cmd-smerge-mode
   qt-agenda-parse-line cmd-org-agenda-goto cmd-org-agenda-todo
   *qt-flyspell-active* qt-aspell-check-word
   qt-flyspell-is-word-char? qt-flyspell-extract-words
   *flyspell-indicator* flyspell-clear-indicators!
   flyspell-apply-indicators! cmd-flyspell-mode take-n
   qt-tab-save-current! qt-tab-restore! cmd-tab-new
   cmd-tab-close cmd-tab-next cmd-tab-previous cmd-tab-rename
   cmd-tab-move *qt-rainbow-active* *rainbow-colors*
   *rainbow-indic-base* rainbow-setup-indicators!
   rainbow-clear-indicators! rainbow-colorize-buffer!
   cmd-rainbow-delimiters-mode *qt-dedicated-windows*
   cmd-set-window-dedicated cmd-toggle-window-dedicated
   cmd-org-sparse-tree org-find-entry-bounds
   cmd-org-encrypt-entry cmd-org-decrypt-entry
   *qt-edit-positions* *qt-edit-pos-index*
   qt-record-edit-position! cmd-goto-last-change-reverse
   cmd-rename-visited-file *selective-display-level*
  ...
  (define *prog-modes*--cell
    (vector
      '(python c c++ javascript typescript go rust ruby scheme
         bash lua sql css html)))
  (def (qt-set-major-mode! app lang-sym mode-name)
       "Set major mode by changing lexer language and re-highlighting."
       (let* ([ed (current-qt-editor app)]
              [fr (app-state-frame app)]
              [buf (qt-edit-window-buffer (qt-current-window fr))])
         (buffer-lexer-lang-set! buf lang-sym)
              ...
              (if (null? stk)
                  ...
                  (let* ([pos (car stk)]
                    ...
                    (echo-message!
                      (app-state-echo app)
                      (string-append
                        "Unmatched opener at line "
                        (number->string (+ line 1))))))]
             [else
              (let ([ch (string-ref text i)])
                (cond
                  [(assoc ch pairs) (lp (+ i 1) (cons i stk))]
  ...
  (def (cmd-set-file-modes app)
       ...
       (let* ([buf (current-qt-buffer app)]
         ...
         (if (not path)
             (echo-message!
               (app-state-echo app)
               "No file in current buffer")
             (let ([mode (qt-echo-read-string
                           (app-state-echo app)
                           (string-append "chmod " path " to: "))])
               (when (and mode (not (string=? mode "")))
                 (with-catch
                   (lambda (e)
                     (echo-message! (app-state-echo app) "chmod failed"))
                   (lambda ()
                     (run-process
       ...
       (let* ([buf (current-qt-buffer app)]
         ...
         (if (not path)
             (echo-message!
               (app-state-echo app)
               "No file in current buffer")
             (let ([owner (qt-echo-read-string
                            (app-state-echo app)
                            (string-append "chown " path " to: "))])
               (when (and owner (not (string=? owner "")))
                 (with-catch
                   (lambda (e)
                     (echo-message! (app-state-echo app) "chown failed"))
                   (lambda ()
                     (run-process
                       (list "chown" owner path)
                       'coprocess:
                       void)
                     (echo-message!
                       (app-state-echo app)
  ...
  (define *qt-debug-on-entry-list*--cell (vector (list)))
           ...
           (let ([sym (string->symbol name)])
             (unless (member sym *qt-debug-on-entry-list*)
               (set! *qt-debug-on-entry-list*
                 (cons sym *qt-debug-on-entry-list*)))
             (with-catch
               (lambda (e)
                 (echo-message!
                   (app-state-echo app)
  ...
  (def (cmd-cancel-debug-on-entry app)
       ...
       (if (null? *qt-debug-on-entry-list*)
           (echo-message!
             (app-state-echo app)
             "No functions marked for debug-on-entry")
           (let ([name (qt-echo-read-string
                         app
                         (string-append
                           "Cancel debug on entry to ["
             ...
             (let ([sym (if (or (not name) (string=? name ""))
               ...
               (set! *qt-debug-on-entry-list*
                 (filter
                   (lambda (s) (not (eq? s sym)))
                   *qt-debug-on-entry-list*))
               (with-catch
                 (lambda (e) (void))
  ...
  (def (cmd-vc-push app)
       ...
       (async-process! "git push" 'callback:
         (lambda (result)
           (echo-message!
             (app-state-echo app)
             (string-append
  ...
  (define *qt-dap-breakpoints*--cell
    (vector (make-hash-table)))
  (define *qt-dap-process*--cell (vector #f))
  (define *qt-dap-program*--cell (vector #f))
  (define *qt-dap-output*--cell (vector '()))
  (def (gdb-send! cmd)
           ...
           (let loop ()
             (let ([line (with-exception-catcher
               ...
               (cond
                 ...
                 [else (set! lines (cons line lines)) (loop)]))))))
  (def (gdb-show-output! app lines)
       ...
       (let* ([fr (app-state-frame app)]
         ...
         (qt-edit-window-buffer-set! win gdb-buf)
         (set! *qt-dap-output*
           (append *qt-dap-output* (or lines (list))))
         (qt-plain-text-edit-set-text!
           ed
           (string-join *qt-dap-output* "\n"))
  ...
  (def (cmd-dap-debug app)
       ...
       (let ([program (qt-echo-read-string
                        app
                        "Program to debug: ")])
                   ...
                   (set! *qt-dap-process*
                     (open-process
                       (list 'path: "gdb" 'arguments:
                         (list "--quiet" "--interpreter=mi2" program)
  ...
  (def (dap-step-command! app mi-cmd label)
       ...
       (if (not *qt-dap-process*)
           (echo-error!
             (app-state-echo app)
             "No debug session (use M-x dap-debug first)")
           (begin
             (gdb-send! mi-cmd)
             (let ([output (gdb-read-until-prompt!)])
       ...
       (if (not *qt-dap-process*)
           ...
           (let ([cmd (qt-echo-read-string app "GDB> ")])
             (when (and cmd (> (string-length cmd) 0))
               ...
               (let ([output (gdb-read-until-prompt!)])
                 (gdb-show-output! app output))))))
  (define *qt-smerge-mine-marker*--cell (vector "<<<<<<<"))
  (define *qt-smerge-sep-marker*--cell (vector "======="))
  (define *qt-smerge-other-marker*--cell (vector ">>>>>>>"))
                       ...
                       (let ([mine-start i])
                         (let find-sep ([j (+ i 7)])
                           (if (>= j len)
                               ...
                               (if (and (<= (+ j 7) len)
                                        (string=?
                                          (substring text j (+ j 7))
                                          *qt-smerge-sep-marker*)
                                        (or (= j 0)
                                            (char=?
                                              (string-ref text (- j 1))
                                              #\newline)))
  ...
  (def (qt-smerge-extract-mine text mine-start sep-start)
       ...
       (let ([mine-line-end (let find-eol ([i (+ mine-start 7)])
  ...
  (def (qt-smerge-extract-other text sep-start other-end)
       ...
       (let* ([sep-line-end (let find-eol ([i (+ sep-start 7)])
                              (if (or (>= i (string-length text))
                                      (char=?
                                        (string-ref text i)
                                        #\newline))
                                  (min (+ i 1) (string-length text))
                                  (find-eol (+ i 1))))]
              [other-line-start (let find-marker ([k sep-line-end])
                                  (if (>= k other-end)
                                      other-end
                                      (if (and (<= (+ k 7)
                                                   (string-length text))
                                               (string=?
                                                 (substring text k (+ k 7))
                                                 *qt-smerge-other-marker*)
                                               (or (= k 0)
                                                   (char=?
                                                     (string-ref
  ...
  (def (cmd-smerge-next app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (let-values ([(mine sep other)
                       (qt-smerge-find-conflict text pos 'next)])
           (if mine
               (begin
                 ...
                 (echo-message!
                   ...
                   (string-append
                     "Conflict ("
                     (number->string (qt-smerge-count text))
                     " total)")))
  ...
  (def (cmd-smerge-prev app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (let-values ([(mine sep other)
                       (qt-smerge-find-conflict text pos 'prev)])
           (if mine
               (begin
                 (qt-plain-text-edit-set-cursor-position! ed mine)
                 (echo-message!
         ...
         (let-values ([(mine sep other)
           ...
           (if (and mine (<= mine pos) (< pos other))
               (let* ([content (qt-smerge-extract-mine text mine sep)]
                 ...
                 (qt-plain-text-edit-set-text!
                   ed
                   (string-append before content after))
                 (qt-plain-text-edit-set-cursor-position! ed mine)
                 (echo-message! (app-state-echo app) "Kept mine"))
               (let-values ([(mine2 sep2 other2)
                             (qt-smerge-find-conflict text pos 'next)])
                 (if mine2
                     (let* ([content (qt-smerge-extract-mine
                                       text
                                       mine2
                                       sep2)]
         ...
         (let-values ([(mine sep other)
           ...
           (if (and mine (<= mine pos) (< pos other))
               ...
               (let-values ([(mine2 sep2 other2)
                             (qt-smerge-find-conflict text pos 'next)])
                 (if mine2
                     (let* ([content (qt-smerge-extract-other
                                       text
                                       mine2
                                       sep2)]
                            [before (substring text 0 mine2)]
         ...
         (let-values ([(mine sep other)
           ...
           (if (and mine (<= mine pos) (< pos other))
               (let* ([mine-content (qt-smerge-extract-mine text mine sep)]
                      [other-content (qt-smerge-extract-other
                                       text
                                       sep
                                       other)]
                      [before (substring text 0 mine)]
                 ...
                 (qt-plain-text-edit-set-text!
                   ed
                   (string-append before mine-content other-content after))
                 (qt-plain-text-edit-set-cursor-position! ed mine)
                 (echo-message! (app-state-echo app) "Kept both"))
               (let-values ([(mine2 sep2 other2)
                             (qt-smerge-find-conflict text pos 'next)])
                 (if mine2
                     (let* ([mine-content (qt-smerge-extract-mine
                                            text
                                            mine2
                                            sep2)]
                            [other-content (qt-smerge-extract-other
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (> count 0)
             (begin
               ...
               (let-values ([(mine sep other)
                             (qt-smerge-find-conflict text 0 'next)])
                 (when mine
                   (qt-plain-text-edit-set-cursor-position! ed mine))))
             (echo-message!
               (app-state-echo app)
             ...
             (let* ([buf-name (car parsed)]
               ...
               (if target-buf
                   (let* ([fr (app-state-frame app)]
                     ...
                     (qt-edit-window-buffer-set! win target-buf)
                     ...
                     (if fp
                         (begin
                           ...
                           (let* ([new-ed (current-qt-editor app)]
                                  [new-text (qt-plain-text-edit-text
                                              new-ed)]
                                  [lines (string-split new-text #\newline)]
                                  [pos (let loop ([ls lines]
                                                  [n 0]
                                                  [offset 0])
                                         (if (or (null? ls)
                                                 (= n (- src-line 1)))
                             ...
                             (let* ([content (call-with-input-file
                               ...
                               (when (and (>= idx 0)
                                 ...
                                 (let* ([line (list-ref lines idx)]
                                   ...
                                   (let* ([agenda-text (qt-plain-text-edit-text
                                                         ed)]
                                          [agenda-lines (string-split
                                                          agenda-text
                                                          #\newline)]
                                          [new-agenda-lines (let loop ([ls agenda-lines]
                                                                       [n 0]
                                                                       [acc '()])
                                                              (if (null?
                                                                    ls)
                                   ...
                                   (echo-message!
                                     ...
                                     (if (string-contains new-line "DONE")
                                         "TODO → DONE"
                                         "DONE → TODO"))))))))))))))
  (define *qt-flyspell-active*--cell (vector #f))
       ...
       (with-catch
         ...
         (lambda ()
           (let* ([proc (open-process
                          (list 'path: "aspell" 'arguments: '("pipe")
                            'stdin-redirection: #t 'stdout-redirection: #t
               ...
               (if (qt-flyspell-is-word-char? (string-ref text i))
                   (let find-end ([j (+ i 1)])
                     (if (or (>= j len)
                         ...
                         (find-end (+ j 1))))
                   (loop (+ i 1) words))))))
  (define *flyspell-indicator*--cell (vector 28))
  ...
  (def (flyspell-apply-indicators! ed misspelled-entries)
       ...
       (sci-send ed SCI_INDICSETSTYLE *flyspell-indicator* 1)
       (sci-send
         ed
         SCI_INDICSETFORE
         *flyspell-indicator*
       ...
       (for-each
         (lambda (entry)
           (let ([start (cadr entry)] [end (caddr entry)])
                     ...
                     (let ([word (car entry)])
                       (when (> (string-length word) 1)
                         (let ([suggestions (qt-aspell-check-word word)])
                           (when suggestions
                             (set! misspelled-entries
                               (cons entry misspelled-entries)))))))
                   words)
                 (let ([count (length misspelled-entries)])
                   (when (> count 0)
                     (flyspell-apply-indicators!
                       ed
                       (reverse misspelled-entries)))
                   (if (= count 0)
                       (echo-message!
                         (app-state-echo app)
                         (string-append
                           "Flyspell: no misspelled words ("
                           (number->string (length words))
                           " checked)"))
                       (echo-message!
                         (app-state-echo app)
                         (string-append
  ...
  (def (qt-tab-restore! app tab)
       ...
       (let* ([buffers (cadr tab)]
         ...
         (let ([max-idx (- (length wins) 1)])
           (qt-frame-current-idx-set! fr (min win-idx max-idx)))
  ...
  (def (cmd-tab-new app)
       ...
       (let* ([echo (app-state-echo app)]
              ...
              [new-tab-name (string-append
                              "Tab "
                              (number->string new-tab-num))]
              [new-tab (list new-tab-name (list buf-name) 0)])
         (qt-tab-save-current! app)
         (app-state-tabs-set! app (append tabs (list new-tab)))
         (app-state-current-tab-idx-set!
           app
           (- (length (app-state-tabs app)) 1))
         (echo-message!
           echo
           (string-append "Created " new-tab-name))))
             ...
             (let* ([new-idx (modulo (+ idx n) (length tabs))]
               ...
               (echo-message!
                 ...
                 (string-append
                   ...
                   (number->string (+ new-idx 1))))))))
  (define *qt-rainbow-active*--cell (vector #f))
  (define *rainbow-colors*--cell
    (vector
      (vector 16737894 4508927 56797 6741350 16763972 16746564
        16737996 11184895)))
  (define *rainbow-indic-base*--cell (vector 20))
       ...
       (let* ([text (qt-plain-text-edit-text ed)]
         ...
         (let loop ([i 0]
           ...
           (when (< i len)
             (let ([ch (string-ref text i)])
               (cond
                 [escape (loop (+ i 1) depth in-string in-comment #f)]
                 [(and in-string (char=? ch #\\))
                  (loop (+ i 1) depth in-string in-comment #t)]
                 [(and in-string (char=? ch #\"))
                  (loop (+ i 1) depth #f in-comment #f)]
                 [in-string (loop (+ i 1) depth in-string in-comment #f)]
                 [(and in-comment (char=? ch #\newline))
                  (loop (+ i 1) depth in-string #f #f)]
                 [in-comment (loop (+ i 1) depth in-string in-comment #f)]
                 [(char=? ch #\;) (loop (+ i 1) depth in-string #t #f)]
                 [(char=? ch #\") (loop (+ i 1) depth #t in-comment #f)]
                 ...
                 [(or (char=? ch #\)) (char=? ch #\]) (char=? ch #\}))
                  (let* ([d (max 0 (- depth 1))]
                    ...
                    (sci-send ed SCI_SETINDICATORCURRENT indic)
                    (sci-send ed SCI_INDICATORFILLRANGE i 1))
                  (loop (+ i 1) (max 0 (- depth 1)) in-string in-comment
                    #f)]
                 [else (loop (+ i 1) depth in-string in-comment #f)]))))))
  ...
  (define *qt-dedicated-windows*--cell
    (vector (make-hash-table)))
  ...
  (def (cmd-toggle-window-dedicated app)
       ...
       (let* ([fr (app-state-frame app)]
              ...
              [echo (app-state-echo app)]
              [buf-name (buffer-name (qt-edit-window-buffer win))]
              [currently-dedicated (hash-get
                                     *qt-dedicated-windows*
                                     buf-name)])
                         ...
                         (when (< i total)
                           (let ([line (list-ref lines i)])
                             (when (and (org-heading-line? line)
                               ...
                               (let ([level (org-heading-stars-of-line
                                              line)])
                                 (let ploop ([j (- i 1)])
                                   (when (>= j 0)
                   ...
                   [(and (> (string-length line) 0)
                    ...
                    (let ([line-level (let count ([j 0])
                                        (if (and (< j (string-length line))
                                            ...
                                            j))])
                      (if (<= offset pos)
                          (loop (+ i 1) line-end offset line-end
                            line-level)
                          (if (<= line-level level)
                              (values entry-start offset heading-end)
                              (loop (+ i 1) line-end entry-start
                                heading-end level))))]
                   [else
                    (loop (+ i 1) line-end entry-start heading-end
                      level)]))))))
  (def (cmd-org-encrypt-entry app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (let-values ([(entry-start entry-end heading-end)
                       (org-find-entry-bounds text pos)])
           (let ([body (substring text heading-end entry-end)])
             (if (or (string=? (string-trim-both body) "")
                 ...
                 (with-catch
                   ...
                   (lambda ()
                     (let* ([proc (open-process
                                    (list 'path: "gpg" 'arguments:
                                      (list "--symmetric" "--armor"
                                        "--batch" "--yes" "--passphrase-fd"
                                        "0")
                                      'stdin-redirection: #t
                                      'stdout-redirection: #t
                                      'stderr-redirection: #t))]
                            [pass (qt-echo-read-string
                                    app
                                    "Passphrase: ")])
                     ...
                     (echo-error!
                       ...
                       (string-append
                         ...
                         (with-output-to-string
                           (lambda () (display-exception e))))))
                   (lambda ()
                     (let* ([pgp-start (string-contains
                                         body
                                         "-----BEGIN PGP MESSAGE-----")]
                            [pgp-end-marker "-----END PGP MESSAGE-----"]
                            [pgp-end-pos (string-contains
                                           body
                                           pgp-end-marker)]
                             ...
                             (when decrypted
                               (let ([new-text (string-append
                                 ...
                                 (echo-message!
                                   echo
                                   "Entry decrypted"))))))))))))))
  (define *qt-edit-positions*--cell
    (vector (make-hash-table)))
  (define *qt-edit-pos-index*--cell
    (vector (make-hash-table)))
  ...
  (define *selective-display-level*--cell (vector #f))
             ...
             (let* ([ed (current-qt-editor app)]
               ...
               (echo-message!
                 ...
                 (string-append
                   ...
                   " lines indented > "
                   (number->string level)))))))
  (define-syntax *prog-modes*
    (identifier-syntax
      [id (vector-ref *prog-modes*--cell 0)]
      [(set! id val) (vector-set! *prog-modes*--cell 0 val)]))
  (define-syntax *qt-debug-on-entry-list*
    (identifier-syntax
      [id (vector-ref *qt-debug-on-entry-list*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-debug-on-entry-list*--cell
                       0
                       val)]))
  (define-syntax *qt-dap-breakpoints*
    (identifier-syntax
      [id (vector-ref *qt-dap-breakpoints*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-dap-breakpoints*--cell
                       0
                       val)]))
  (define-syntax *qt-dap-process*
    (identifier-syntax
      [id (vector-ref *qt-dap-process*--cell 0)]
      [(set! id val) (vector-set! *qt-dap-process*--cell 0 val)]))
  (define-syntax *qt-dap-program*
    (identifier-syntax
      [id (vector-ref *qt-dap-program*--cell 0)]
      [(set! id val) (vector-set! *qt-dap-program*--cell 0 val)]))
  (define-syntax *qt-dap-output*
    (identifier-syntax
      [id (vector-ref *qt-dap-output*--cell 0)]
      [(set! id val) (vector-set! *qt-dap-output*--cell 0 val)]))
  (define-syntax *qt-smerge-mine-marker*
    (identifier-syntax
      [id (vector-ref *qt-smerge-mine-marker*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-smerge-mine-marker*--cell
                       0
                       val)]))
  (define-syntax *qt-smerge-sep-marker*
    (identifier-syntax
      [id (vector-ref *qt-smerge-sep-marker*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-smerge-sep-marker*--cell
                       0
                       val)]))
  (define-syntax *qt-smerge-other-marker*
    (identifier-syntax
      [id (vector-ref *qt-smerge-other-marker*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-smerge-other-marker*--cell
                       0
                       val)]))
  (define-syntax *qt-flyspell-active*
    (identifier-syntax
      [id (vector-ref *qt-flyspell-active*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-flyspell-active*--cell
                       0
                       val)]))
  (define-syntax *flyspell-indicator*
    (identifier-syntax
      [id (vector-ref *flyspell-indicator*--cell 0)]
      [(set! id val) (vector-set!
                       *flyspell-indicator*--cell
                       0
                       val)]))
  (define-syntax *qt-rainbow-active*
    (identifier-syntax
      [id (vector-ref *qt-rainbow-active*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-rainbow-active*--cell
                       0
                       val)]))
  (define-syntax *rainbow-colors*
    (identifier-syntax
      [id (vector-ref *rainbow-colors*--cell 0)]
      [(set! id val) (vector-set! *rainbow-colors*--cell 0 val)]))
  (define-syntax *rainbow-indic-base*
    (identifier-syntax
      [id (vector-ref *rainbow-indic-base*--cell 0)]
      [(set! id val) (vector-set!
                       *rainbow-indic-base*--cell
                       0
                       val)]))
  (define-syntax *qt-dedicated-windows*
    (identifier-syntax
      [id (vector-ref *qt-dedicated-windows*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-dedicated-windows*--cell
                       0
                       val)]))
  (define-syntax *qt-edit-positions*
    (identifier-syntax
      [id (vector-ref *qt-edit-positions*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-edit-positions*--cell
                       0
                       val)]))
  (define-syntax *qt-edit-pos-index*
    (identifier-syntax
      [id (vector-ref *qt-edit-pos-index*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-edit-pos-index*--cell
                       0
                       val)]))
  (define-syntax *selective-display-level*
    (identifier-syntax
      [id (vector-ref *selective-display-level*--cell 0)]
      [(set! id val) (vector-set!
                       *selective-display-level*--cell
                       0
                       val)])))
