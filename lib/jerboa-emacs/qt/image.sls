...
(library (jerboa-emacs qt image)
  (export image-file? qt-view-image! qt-ensure-image-widget!
    qt-show-image-buffer! qt-hide-image-buffer! qt-image-zoom!
    qt-image-cleanup!)
  (import
    (except (chezscheme) make-hash-table hash-table? iota \x31;+ \x31;-
      getenv path-extension path-absolute? thread? make-mutex
      mutex? mutex-name)
    (std sugar) (std srfi srfi-13) (jerboa-emacs qt sci-shim)
    (jerboa-emacs core) (jerboa-emacs qt window) (jerboa core)
    (jerboa runtime))
         ...
         (when (and state win)
           (let* ([pixmap (list-ref state 0)]
             ...
             (echo-message!
               (app-state-echo app)
               (string-append
                 (number->string (inexact->exact (round (* new-zoom 100))))
                 "%"))))))
       ...
       (let* ([pixmap (qt-pixmap-load filename)])
         (if (qt-pixmap-null? pixmap)
             ...
             (let* ([dialog (qt-dialog-create 'parent: parent)]
                    ...
                    [btn-zoom-in (qt-push-button-create
                                   "+"
                                   'parent:
                                   dialog)]
                    [btn-zoom-out (qt-push-button-create
                                    "-"
                                    'parent:
                                    dialog)]
                    [btn-zoom-fit (qt-push-button-create
                                    "Fit"
                                    'parent:
                                    dialog)]
                    [btn-zoom-100 (qt-push-button-create
                                    "100%"
                                    'parent:
                                    dialog)]
               ...
               (qt-dialog-set-title!
                 dialog
                 (string-append "Image: " (path-strip-directory filename)))
               (qt-widget-resize!
                 dialog
                 (min 1200 (+ orig-w 40))
                 (min 800 (+ orig-h 80)))
               ...
               (qt-on-clicked!
                 btn-zoom-100
                 (lambda () (set-box! zoom-ref 1.0) (update-image!)))
               (qt-on-clicked!
                 btn-zoom-fit
                 (lambda ()
                   (let* ([container-w (qt-widget-width scroll-area)]
                     ...
                     (when (and (> orig-w 0)
                                ...
                                (> container-w 0)
                                (> container-h 0))
                       (let* ([scale-w (/ (exact->inexact container-w)
                                          orig-w)]
                              [scale-h (/ (exact->inexact container-h)
                                          orig-h)]
               ...
               (qt-on-key-press!
                 ...
                 (lambda ()
                   (let ([key (qt-last-key-code)])
                     (cond
                       [(= key QT_KEY_ESCAPE) (qt-dialog-reject! dialog)]
                       [(or (= key 43) (= key 61))
                        (set-box!
                          zoom-ref
                          (min 10.0 (* (unbox zoom-ref) 1.25)))
                        (update-image!)]
                       [(= key 45)
                        (set-box!
                          zoom-ref
                          (max 0.1 (/ (unbox zoom-ref) 1.25)))
                        (update-image!)]
                       [(= key 48)
                        (let* ([container-w (qt-widget-width scroll-area)]
                               [container-h (qt-widget-height
                                              scroll-area)])
                          (when (and (> orig-w 0)
                                     (> orig-h 0)
                                     (> container-w 0)
