...
;;; Source: src/jerboa-emacs/qt/commands-vcs.ss

(library (jerboa-emacs qt commands-vcs)
  (export cmd-transpose-sexps cmd-transpose-paragraphs
   cmd-zap-up-to-char cmd-zap-to-char-inclusive
   *qt-last-regexp-search* cmd-search-forward-regexp
   cmd-query-replace-regexp cmd-copy-from-below
   cmd-copy-symbol-at-point cmd-copy-word-at-point
   cmd-delete-to-end-of-line cmd-delete-to-beginning-of-line
   cmd-delete-horizontal-space-forward cmd-cycle-spacing
   cmd-swap-windows cmd-rotate-windows cmd-fold-all
   cmd-unfold-all cmd-toggle-auto-pair-mode cmd-mark-page
   cmd-mark-whole-buffer cmd-view-lossage cmd-bookmark-rename
   cmd-view-register cmd-sort-imports cmd-replace-string-all
   cmd-replace-in-region cmd-write-region
   cmd-search-forward-word cmd-search-backward-word
   cmd-count-occurrences cmd-kill-matching-buffers
   *recent-files* *recent-files-max* *recent-files-path*
   recent-files-add! recent-files-save! recent-files-load!
   ...
   *eldoc-last-sym* eldoc-display! *xref-back-stack*
   ...
   find-def-in-text find-def-in-project cmd-goto-definition
   cmd-xref-back *project-root-markers* detect-project-root
  ...
  (import
   ...
   (jerboa-emacs qt commands-ide2) (jerboa core)
   (jerboa runtime))
  (def (cmd-transpose-sexps app)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (let* ([before-end pos]
                [before-start (let loop ([i (- pos 1)] [depth 0])
                                (cond
                                  [(< i 0) 0]
                                  [(and (memq
                                          (string-ref text i)
                ...
                [after-end (let loop ([i pos])
                             (cond
                               ...
                               [(char-whitespace? (string-ref text i))
                                (loop (+ i 1))]
                               [else (+ i 1)]))])
           (when (and (< before-start before-end)
             ...
             (let ([sexp1 (substring text before-start before-end)]
               ...
               (qt-plain-text-edit-insert-text!
                 ed
                 (string-append sexp2 mid sexp1))
               (echo-message!
                 (app-state-echo app)
                 "S-expressions transposed"))))))
  (def (cmd-transpose-paragraphs app)
       ...
       (let* ([ed (current-qt-editor app)]
              ...
              [pos (qt-plain-text-edit-cursor-position ed)]
              [len (string-length text)])
         (let* ([para-start (let loop ([i (- pos 1)])
                              (cond
                                [(< i 0) 0]
                                [(and (char=?
                                        (string-ref text i)
                ...
                [para-end (let loop ([i pos])
                            (cond
                              ...
                              [(and (char=? (string-ref text i) #\newline)
                               ...
                               i]
                              [else (loop (+ i 1))]))]
                [next-start (let loop ([i (+ para-end 1)])
                              (cond
                                [(>= i len) #f]
                                [(not (or (char=?
                                            (string-ref text i)
           ...
           (if (and next-start next-end)
               (let* ([para1 (substring text para-start para-end)]
                 ...
                 (qt-plain-text-edit-insert-text!
                   ed
                   (string-append para2 sep para1))
                 (echo-message!
                   (app-state-echo app)
                   "Paragraphs transposed"))
               (echo-message!
                 (app-state-echo app)
                 "No next paragraph to transpose")))))
           ...
           (let* ([ch (string-ref input 0)]
             ...
             (if found
                 ...
                 (echo-error!
                   (app-state-echo app)
                   (string-append "'" (string ch) "' not found")))))))
  (define *qt-last-regexp-search*--cell (vector ""))
               ...
               (let* ([ed (current-qt-editor app)]
                 ...
                 (if match
                     (let* ([pair (car match)]
                       ...
                       (qt-extra-selection-add-range! ed start
                         (- end start) 0 0 0 0 221 255 'bold: #t)
                       (qt-extra-selections-apply! ed))
                     (let ([match2 (pregexp-match-positions
                                     pattern
                                     text
                                     0)])
                       (if match2
                           (let* ([pair2 (car match2)]
                             ...
                             (qt-plain-text-edit-ensure-cursor-visible! ed)
                             (qt-update-visual-decorations! ed)
                             (qt-extra-selection-add-range! ed start2 (- end2 start2) 0 0 0 0 221 255
                               'bold: #t)
                             (qt-extra-selections-apply! ed)
                             (echo-message!
                               (app-state-echo app)
         ...
         (if (< (+ cur-line 1) (length lines))
             (let ([below (list-ref lines (+ cur-line 1))])
               (if (< col (string-length below))
                   (qt-plain-text-edit-insert-text!
                     ed
                     (string (string-ref below col)))
                   (echo-message!
                     (app-state-echo app)
                     "Line below too short")))
         ...
         (when (< pos len)
           (let* ([sym-char? (lambda (ch)
                  ...
                  [start (let loop ([i pos])
                           (if (and (> i 0)
                                    (sym-char? (string-ref text (- i 1))))
                               (loop (- i 1))
                               i))]
                  [end (let loop ([i pos])
                         (if (and (< i len)
                                  (sym-char? (string-ref text i)))
                             (loop (+ i 1))
                             i))]
  ...
  (def (cmd-delete-to-end-of-line app)
       ...
       (let* ([ed (current-qt-editor app)]
              ...
              [lines (string-split text #\newline)]
              [line-end (+ (line-start-position text line)
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (cond
           ...
           [(= ws-len 1)
            (qt-plain-text-edit-set-selection! ed start end)
            (qt-plain-text-edit-remove-selected-text! ed)]
           [else (qt-plain-text-edit-insert-text! ed " ")])))
  ...
  (def (cmd-bookmark-rename app)
       ...
       (let* ([bm (app-state-bookmarks app)]
              ...
              [old-name (qt-echo-read-string-with-completion
                          app
                          "Rename bookmark: "
                          names)])
           ...
           (if (hash-get bm old-name)
               (let ([new-name (qt-echo-read-string app "New name: ")])
                 (when (and new-name (> (string-length new-name) 0))
                   ...
                   (echo-message!
                     (app-state-echo app)
                     (string-append old-name " -> " new-name))))
               (echo-error!
                 (app-state-echo app)
                 (string-append "No bookmark: " old-name))))))
                          ...
                          [(null? ls)
                           (if (pair? imports)
                               (reverse
                                 (append
                                   (reverse
                                     (sort (reverse imports) string<?))
                                   acc))
                               (reverse acc))]
                          [(or (string-contains (car ls) "(import ")
                               (and in-import?
                                    (> (string-length (car ls)) 0)
                                    (or (char=?
                                          (string-ref (car ls) 0)
                                          #\space)
                                        (char=?
  ...
  (def (cmd-replace-string-all app)
       ...
       (let ([from (qt-echo-read-string app "Replace all: ")])
         (when from
           (let ([to (qt-echo-read-string
                       app
                       (string-append "Replace \"" from "\" with: "))])
             (when to
               (let* ([ed (current-qt-editor app)]
                      [text (qt-plain-text-edit-text ed)])
  ...
  (def (cmd-replace-in-region app)
       ...
       (let* ([ed (current-qt-editor app)]
              ...
              [pos (qt-plain-text-edit-cursor-position ed)])
         (if mark
             (let ([search (qt-echo-read-string
                             app
                             "Replace in region: ")])
               (when search
                 (let ([replace (qt-echo-read-string
                                  app
                                  "Replace with: ")])
                   (when replace
                     (let* ([start (min pos mark)]
                            ...
                            [result (let loop ([p 0] [acc (list)])
                                      (let ([f (string-contains
                                                 region
                                                 search
                                                 p)])
                                        (if f
             ...
             (let* ([start (let loop ([p pos])
               ...
               (if found
                   (begin
                     ...
                     (echo-message!
                       (app-state-echo app)
                       (string-append "Found: " word)))
                   (echo-error!
                     (app-state-echo app)
                     (string-append "\"" word "\" not found below"))))
         ...
         (if (and (< pos len)
             ...
             (let* ([start (let loop ([p pos])
               ...
               (if found
                   ...
                   (echo-error!
                     (app-state-echo app)
                     (string-append "\"" word "\" not found above"))))
             (echo-error! (app-state-echo app) "Not on a word"))))
           ...
           (let ([killed 0])
             ...
             (echo-message!
               ...
               (string-append
                 ...
                 " matching buffers"))))))
  (define *recent-files*--cell (vector (list)))
  (define *recent-files-max*--cell (vector 50))
       ...
       (when (and path (string? path) (> (string-length path) 0))
         (let ([abs-path (path-expand path)])
           (set! *recent-files*
             (cons
               abs-path
               (let loop ([files *recent-files*] [acc (list)])
                 (cond
           ...
           (when (> (length *recent-files*) *recent-files-max*)
             (set! *recent-files*
               (let loop ([files *recent-files*] [n 0] [acc (list)])
         ...
         (lambda ()
           (call-with-output-file
             *recent-files-path*
             (lambda (port)
               (for-each
                 (lambda (f) (display f port) (newline port))
                 *recent-files*))))))
  ...
  (def (cmd-list-recent-files app)
       ...
       (if (null? *recent-files*)
           ...
           (let* ([text (string-join *recent-files* "\n")]
             ...
             (qt-edit-window-buffer-set! (qt-current-window fr) buf)
             (qt-plain-text-edit-set-text! ed text))))
  (def (cmd-clear-recent-files app) "Clear the recent files list."
       (set! *recent-files* (list)) (recent-files-save!)
             ...
             (when (and choice (> (string-length choice) 0))
               (cond
                 [(and (file-exists? choice)
                       (eq? 'directory
                            (file-info-type (file-info choice))))
  ...
  (def (cmd-multi-occur app)
       ...
       (let ([pat (qt-echo-read-string app "Multi-occur: ")])
         (when pat
           (let* ([fr (app-state-frame app)]
                  [file-results (let loop ([bufs (buffer-list)]
                                           [acc (list)])
                                  (if (null? bufs)
                                      (reverse acc)
                                      (let* ([buf (car bufs)]
             ...
             (echo-message!
               (app-state-echo app)
               (string-append
                 (number->string (length file-results))
                 " matches for: "
                 pat))))))
             ...
             (if mark
                 (let* ([start (min pos mark)]
                        ...
                        [max-col (let loop ([ls lines] [max-c 0])
                                   (if (null? ls)
                                       max-c
                                       (let ([p (string-contains
                                                  (car ls)
                                                  sep)])
                                         (loop
                                           (cdr ls)
                                           (if p (max max-c p) max-c)))))]
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if mark
             (let* ([text (qt-plain-text-edit-text ed)]
                    ...
                    [new-lines (let loop ([i 0] [ls lines] [acc (list)])
                                 (if (null? ls)
                                     (reverse acc)
                                     (if (and (>= i start-line)
                                              (<= i end-line))
  ...
  (def (cmd-describe-face app)
       ...
       (echo-message!
         (app-state-echo app)
         "QPlainTextEdit uses uniform styling"))
  (def (cmd-describe-function app)
       ...
       (let* ([cmd-names (sort
         ...
         (when (and name (> (string-length name) 0))
           (let* ([sym (string->symbol name)] [cmd (find-command sym)])
             (if cmd
                 (let* ([ed (current-qt-editor app)]
                        [fr (app-state-frame app)]
  ...
  (def (cmd-describe-variable app)
       ...
       (let* ([names (map symbol->string (custom-list-all))]
              [selection (qt-echo-read-with-narrowing
                           app
                           "Describe variable:"
                           names)])
       ...
       (let* ([ed (current-qt-editor app)]
         ...
         (if (< pos len)
             (let* ([ch (string-ref text pos)]
               ...
               (echo-message!
                 (app-state-echo app)
                 (string-append "Char: " (string ch) " (U+" (number->string code 16)
                   ") " char-class ", style=" (number->string style))))
                                ...
                                (let sym-end ([j start])
                                  (if (and (< j len)
                                      ...
                                      (if (> j start)
                                          ...
                                          #f)))))]
                         [else (scan (- i 1) depth)])))))))
  (define *eldoc-last-sym*--cell (vector #f))
  ...
  (define *xref-back-stack*--cell (vector (list)))
  (def (xref-push-location! app)
       ...
       (let* ([buf (current-qt-buffer app)]
         ...
         (set! *xref-back-stack*
           (cons
             (cons (or (buffer-file-path buf) (buffer-name buf)) pos)
             (if (> (length *xref-back-stack*) 50)
                 (let loop ([l *xref-back-stack*] [n 0] [acc (list)])
                   (if (or (null? l) (>= n 50))
                       (reverse acc)
  ...
  (define *def-patterns*--cell
    (vector
      '("(def (" "(def " "(defstruct " "(defclass " "(defrule "
         "(defmethod " "(defsyntax " "(defmacro " "(defun "
         "(define (" "(define ")))
                          ...
                          (list 'path: "/usr/bin/grep" 'arguments:
                            (list "-rnl"
                              (string-append
                                ...
                                "\\|def[a-z]* "
                                sym)
                              "--include=*.ss" "--include=*.scm" root)
                            'stdout-redirection: #t 'stderr-redirection:
                            #f))]
                  [output (read-line proc #f)])
             (close-port proc)
             (and output
                  (let ([files (let loop ([s output] [acc (list)])
            ...
            (let loop ([dir (path-directory path)])
              (if (string=? dir "/")
                  ...
                  (if (let check ([markers *project-root-markers*])
                        (and (pair? markers)
                             (or (file-exists?
                                   (path-expand (car markers) dir))
                                 (check (cdr markers)))))
                      dir
                      (let ([parent (path-directory
                                      (string-append dir "/"))])
                        (if (string=? parent dir) #f (loop parent))))))))
  (def (current-project-root app)
       ...
       (let ([path (buffer-file-path (current-qt-buffer app))])
         (or (detect-project-root path) (current-directory))))
  (define-syntax *qt-last-regexp-search*
    (identifier-syntax
      [id (vector-ref *qt-last-regexp-search*--cell 0)]
      [(set! id val) (vector-set!
                       *qt-last-regexp-search*--cell
                       0
                       val)]))
  (define-syntax *recent-files*
    (identifier-syntax
      [id (vector-ref *recent-files*--cell 0)]
      [(set! id val) (vector-set! *recent-files*--cell 0 val)]))
  (define-syntax *recent-files-max*
    (identifier-syntax
      [id (vector-ref *recent-files-max*--cell 0)]
      [(set! id val) (vector-set!
                       *recent-files-max*--cell
                       0
                       val)]))
  (define-syntax *recent-files-path*
    (identifier-syntax
      [id (vector-ref *recent-files-path*--cell 0)]
      [(set! id val) (vector-set!
  ...
  (define-syntax *eldoc-last-sym*
    (identifier-syntax
      [id (vector-ref *eldoc-last-sym*--cell 0)]
      [(set! id val) (vector-set! *eldoc-last-sym*--cell 0 val)]))
  (define-syntax *xref-back-stack*
    (identifier-syntax
      [id (vector-ref *xref-back-stack*--cell 0)]
      [(set! id val) (vector-set!
                       *xref-back-stack*--cell
                       0
                       val)]))
  (define-syntax *def-patterns*
    (identifier-syntax
      [id (vector-ref *def-patterns*--cell 0)]
      [(set! id val) (vector-set! *def-patterns*--cell 0 val)]))
  (define-syntax *project-root-markers*
    (identifier-syntax
      [id (vector-ref *project-root-markers*--cell 0)]
      [(set! id val) (vector-set!
                       *project-root-markers*--cell
                       0
                       val)]))
