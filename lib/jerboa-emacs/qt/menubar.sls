  ...
  (def (qt-setup-menubar! app win)
       ...
       (let ([menu-bar (qt-main-window-menu-bar win)])
         (let ([menu (qt-menu-bar-add-menu menu-bar "&File")])
           (add-menu-command! menu win app "&New" "Ctrl+X,Ctrl+N"
             'find-file)
           (add-menu-command! menu win app "&Open..." "Ctrl+X,Ctrl+F"
             'find-file)
           (add-menu-command! menu win app "&Save" "Ctrl+X,Ctrl+S"
             'save-buffer)
           (add-menu-command! menu win app "Save &As..."
             "Ctrl+X,Ctrl+W" 'write-file)
           (add-menu-command! menu win app "&Revert" "" 'revert-buffer)
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "&Quit" "Ctrl+X,Ctrl+C"
             'quit))
         (let ([menu (qt-menu-bar-add-menu menu-bar "&Edit")])
           ...
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "Cu&t" "Ctrl+W"
             'kill-region)
           (add-menu-command! menu win app "&Copy" "Alt+W"
             'copy-region)
           (add-menu-command! menu win app "&Paste" "Ctrl+Y" 'yank)
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "&Find..." "Ctrl+S"
             'search-forward)
           (add-menu-command! menu win app "R&eplace..." "Alt+%"
             'query-replace)
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "Select &All" "Ctrl+X,H"
             'select-all)
           (add-menu-command! menu win app "&Go to Line" "Alt+G,G"
             'goto-line))
         (let ([menu (qt-menu-bar-add-menu menu-bar "&View")])
           (add-menu-command! menu win app "Toggle &Line Numbers"
             "Ctrl+X,L" 'toggle-line-numbers)
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "Split &Below" "Ctrl+X,2"
             'split-window)
           (add-menu-command! menu win app "Split &Right" "Ctrl+X,3"
             'split-window-right)
           (add-menu-command! menu win app "&Close Split" "Ctrl+X,0"
             'delete-window)
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "Zoom &In" "Ctrl+="
             'zoom-in)
           (add-menu-command! menu win app "Zoom &Out" "Ctrl+-"
             'zoom-out))
         (let ([menu (qt-menu-bar-add-menu menu-bar "&Buffer")])
           (add-menu-command! menu win app "&Switch Buffer" "Ctrl+X,B"
             'switch-buffer)
           (add-menu-command! menu win app "&List Buffers"
             "Ctrl+X,Ctrl+B" 'list-buffers)
           (add-menu-command! menu win app "&Kill Buffer" "Ctrl+X,K"
             'kill-buffer-cmd))
         (let ([menu (qt-menu-bar-add-menu menu-bar "&Tools")])
           (add-menu-command! menu win app "&REPL" "Ctrl+C,Z" 'repl)
           (add-menu-command! menu win app "&Eshell" "Ctrl+C,E"
             'eshell)
           (add-menu-command! menu win app "&Shell" "Ctrl+C,$" 'shell)
           (qt-menu-add-separator! menu)
           (add-menu-command! menu win app "Eval E&xpression" "Alt+:"
             'eval-expression))
         ...
         (let ([toolbar (qt-toolbar-create "Main" win)])
           ...
           (qt-toolbar-add-separator! toolbar)
           (add-toolbar-command! toolbar win app "Find"
             'search-forward))))
  (def (add-menu-command! menu win app label shortcut
         command-name)
       ...
       (let* ([display-label (if (and (string? shortcut)
