...
(library (jerboa-emacs qt automation)
  ...
  (import
    ...
    (jerboa-emacs core) (jerboa-emacs qt window)
    (jerboa-emacs qt echo) (jerboa core) (jerboa runtime))
         ...
         (if special
             ...
             (let parse ([s key-str] [mods 0])
               (cond
                 ...
                 [(and (>= (string-length s) 2)
                       (string=? (substring s 0 2) "S-"))
                  (parse
                    (substring s 2 (string-length s))
                    (bitwise-ior mods QT_MOD_SHIFT))]
                 [(emacs-special-key s) =>
                  (lambda (pair)
                    (values (car pair) (bitwise-ior mods (cdr pair)) ""))]
                 [(= (string-length s) 1)
                  (let* ([ch (string-ref s 0)] [code (char->qt-key ch)])
                    (values code mods (string ch)))]
                 [else (error "automation: unrecognized key" key-str)])))))
  (def (char->qt-key ch)
  ...
  (def (automation-screenshot! app path)
       (let* ([fr (app-state-frame app)]
         ...
         (if main-win (qt-widget-screenshot! main-win path) #f)))
  (def (automation-state app)
       (let* ([fr (app-state-frame app)]
         ...
         (list (cons 'buffer (if buf (buffer-name buf) "#<none>"))
           (cons
             'point
         ...
         (let loop ()
           (let ([state (automation-state app)])
             (cond
               ...
               [else (qt-app-process-events! *qt-app-ref*) (loop)])))))
  (def (current-time-ms)
       (let ([t (current-time)])
         (+ (* (time-second t) 1000)
