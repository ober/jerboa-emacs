#!chezscheme
;;; editor-extra-org.sls — Org-mode, calendar, diary, focus/zen mode,
;;; killed buffers, window dedication, and miscellaneous editing commands.
;;;
;;; Ported from gerbil-emacs/editor-extra-org.ss to R6RS Chez Scheme.

(library (jerboa-emacs editor-extra-org)
  (export
    ;; Org-mode basics
    org-heading-line?
    org-heading-level
    org-find-subtree-end
    org-on-checkbox-line?
    org-get-current-line
    org-replace-line
    ;; Org commands
    cmd-org-mode
    cmd-org-todo
    ...
    cmd-org-agenda
    cmd-org-export
    cmd-org-table-create
    cmd-org-link
    cmd-org-store-link
    cmd-org-open-at-point
    cmd-org-cycle
    cmd-org-shift-tab
    cmd-org-sparse-tree
    cmd-org-promote
    ...
    cmd-appt-check
    ;; Calendar helpers
    tui-holidays-for-month
    tui-current-month
    tui-current-year
    tui-org-items-for-month
    tui-calendar-org-footer
    ;; Focus/zen mode
    cmd-toggle-focus-mode
    cmd-toggle-zen-mode
    ;; Killed buffers
    remember-killed-buffer!
    cmd-reopen-killed-buffer
    ;; File operations
    cmd-copy-file-name-only
    cmd-open-containing-folder
    cmd-new-empty-buffer
    ;; Window dedication
    cmd-set-window-dedicated
    cmd-toggle-window-dedicated
    ;; Which-key
    cmd-toggle-which-key-mode
    cmd-which-key-describe-prefix
    ;; Window ops
    cmd-transpose-windows
    cmd-fold-toggle-at-point
    cmd-imenu-list
    ;; Batch 31
    cmd-titlecase-region
    cmd-goto-matching-bracket
    cmd-toggle-block-comment
    cmd-move-to-window-center
    cmd-reverse-region-chars
    cmd-toggle-relative-line-numbers
    cmd-toggle-cua-mode
    cmd-exchange-dot-and-mark
    cmd-sort-paragraphs
    cmd-insert-mode-line
    cmd-push-mark-command
    ;; Batch 35
    cmd-toggle-global-auto-complete
    cmd-toggle-which-function
    cmd-toggle-display-line-numbers
    cmd-toggle-selective-display
    cmd-toggle-global-font-lock
    cmd-insert-register-content
    cmd-insert-date-iso
    cmd-toggle-word-wrap-column
    cmd-clone-indirect-buffer
    cmd-toggle-auto-dim-other-buffers
    cmd-toggle-global-eldoc
    cmd-open-line-below
    ;; Batch 43 toggles
    cmd-toggle-desktop-save-mode
    cmd-toggle-recentf-mode
    cmd-toggle-savehist-mode
    cmd-toggle-winner-mode
    cmd-toggle-midnight-mode
    cmd-toggle-global-undo-tree
    cmd-toggle-diff-hl-mode
    cmd-toggle-volatile-highlights
    cmd-toggle-vertico-mode
    cmd-toggle-marginalia-mode
    ;; Batch 52 toggles
    cmd-toggle-global-cwarn
    cmd-toggle-global-hideshow
    cmd-toggle-global-abbrev
  ...
  (import (except (chezscheme) make-hash-table hash-table? iota 1+ 1- sort sort!
          ...
          (only (std srfi srfi-13) string-join string-contains string-prefix?
                string-suffix? string-index string-index-right
                string-trim string-trim-right string-pad)
  ...
  ;; Local helper (no srfi-1 in jerboa)
  (define (last lst)
    (if (null? (cdr lst)) (car lst) (last (cdr lst))))

  (define *org-stored-link* #f)

  (define (org-heading-line? line)
  ...
  (define (org-find-subtree-end lines cur-line level)
  ...
  (define (org-replace-line ed line-num new-line)
  ...
  (define (cmd-org-open-at-point app)
                ...
                (let* ((content (substring text i j))
                  ...
                  (cond
                       ...
                       (if (file-exists? path)
                         (begin
                           (let* ((new-buf (buffer-create! (path-strip-directory path) ed))
                             ...
                             (buffer-file-path-set! new-buf path)
                    ...
                    ((or (string-prefix? "http://" url) (string-prefix? "https://" url))
                     (guard (e (#t (echo-message! echo "Failed to open URL")))
                       (run-process (list "xdg-open" url))
                       (echo-message! echo (string-append "Opening: " url))))
                    (else (echo-message! echo (string-append "Link: " url))))))
               (else (find-end (+ j 1))))))
          (else (find-start (- i 1)))))))

  (define (cmd-org-cycle app)
  ...
  ;;;==========================================================================
  (define (cmd-org-sparse-tree app)
  ...
  (define (cmd-org-move-subtree-up app)
  ...
  (define (cmd-org-move-subtree-down app)
  ...
  (define (cmd-org-toggle-checkbox app)
  ...
  (define (cmd-org-insert-heading app)
  ...
  (define (cmd-org-insert-src-block app)
    ...
    (let* ((fr (app-state-frame app))
           ...
           (lang-str (if (and lang (not (string=? lang "")))
                       (string-append " " lang)
                       ""))
  ...
  (define *tui-org-clock-marker* #f)    ; line number where clock was started
  (define *tui-org-clock-heading* #f)   ; heading text for display

  (define (cmd-org-clock-in app)
    ...
    (let* ((fr (app-state-frame app))
           ...
           (now (guard (e (#t ""))
                  (let ((out (run-process
                               (list "date" "+[%Y-%m-%d %a %H:%M]"))))
                    (if (string=? out "") "" (string-trim out))))))
      (when (not (string=? now ""))
  ...
  (define (cmd-org-clock-cancel app)
  ...
  (define *org-structure-templates*
  ...
  (define (org-template-lookup key)
  ...
  (define (cmd-org-template-expand app)
    "Expand org structure template at point."
    (let* ((fr (app-state-frame app))
           (win (current-window fr))
           (ed (edit-window-editor win))
           (text (editor-get-text ed))
        ...
        (let* ((key (substring trimmed 1 (string-length trimmed)))
          ...
          (if (not tmpl)
            ...
            (let* ((block-type (car tmpl))
                   ...
                   (end-line-str (string-append indent "#+END_" end-type))
  ...
  (define (tui-current-month)
    "Get current month number."
    (guard (e (#t 1))
      (let ((out (run-process (list "/bin/date" "+%m"))))
        (if (string=? out "") 1 (or (string->number (string-trim out)) 1)))))

  (define (tui-current-year)
    "Get current year number."
    (guard (e (#t 2026))
      (let ((out (run-process (list "/bin/date" "+%Y"))))
        (if (string=? out "") 2026 (or (string->number (string-trim out)) 2026)))))

  (define (cmd-calendar app)
    ...
    (let* ((year *tui-calendar-year*)
           ...
           (cal-text (guard (e (#t "Calendar not available"))
                       (run-process
                         (list "cal" "-3" (number->string month) (number->string year)))))
  ...
  (define (cmd-diary-view-entries app)
  ...
  (define (cmd-diary-insert-entry app)
    "Add a diary entry for today to ~/.jemacs-diary."
    (let* ((date-str (guard (e (#t "2026-01-01"))
                       (let ((out (run-process (list "/bin/date" "+%Y-%m-%d"))))
                         (if (string=? out "") "2026-01-01" (string-trim out)))))
           (entry (app-read-string app (string-append "Diary entry (" date-str "): "))))
      (when (and entry (not (string=? entry "")))
        (guard (e (#t (echo-error! (app-state-echo app) "Failed to write diary")))
          (let ((port (open-file-output-port *tui-diary-file*
                        (file-options no-fail no-truncate)
                        (buffer-mode block)
                        (native-transcoder))))
            (set-port-position! port (port-length port))
            (display (string-append date-str " " entry "\n") port)
            (close-port port))
          (echo-message! (app-state-echo app)
            (string-append "Diary entry added for " date-str))))))

  ;;; --- Calendar-org integration ---
  (define (tui-org-items-for-month year month)
  ...
  (define (tui-calendar-org-footer year month)
  ...
  (define (cmd-appt-check app)
    "Check for upcoming appointments in the next 15 minutes."
    (guard (e (#t (echo-message! (app-state-echo app) "No appointment data")))
      (let* ((now-str (run-process (list "/bin/date" "+%Y %m %d %H %M")))
  ...
  (define *focus-mode* #f)
  (define *focus-margin-width* 20)

  (define (cmd-toggle-focus-mode app)
  ...
  (define (cmd-toggle-zen-mode app)
  ...
  (define *killed-buffers* '())   ; list of (name file-path text) triples
  (define *max-killed-buffers* 20)

  (define (remember-killed-buffer! name file-path text)
  ...
  (define (cmd-reopen-killed-buffer app)
  ...
  (define (cmd-copy-file-name-only app)
  ...
  (define (cmd-open-containing-folder app)
        ...
        (let* ((parts (string-split filepath #\/))
               (dir (if (<= (length parts) 1) "."
                      (string-join
                        ...
                        "/"))))
          (guard (e (#t (echo-message! echo "Cannot open folder")))
            (let ((opener (cond
  ...
  (define (cmd-new-empty-buffer app)
  ...
  (define *dedicated-windows* (make-hash-table))

  (define (cmd-set-window-dedicated app)
  ...
  (define (cmd-toggle-window-dedicated app)
  ...
  (define (cmd-toggle-which-key-mode app)
  ...
  (define (cmd-which-key-describe-prefix app)
                         ...
                         (lambda ()
                           ...
                           (for-each
                             (lambda (e)
                               ...
                               (display (cdr e))
                               (display "\n"))
                             (list-sort (lambda (a b) (string<? (car a) (car b))) matches)))))))
          (editor-set-text ed text)
          (editor-goto-pos ed 0)
          (echo-message! echo
  ...
  (define (cmd-transpose-windows app)
    ...
    (let* ((fr (app-state-frame app))
      ...
      (if (< (length wins) 2)
        ...
        (let* ((cur (current-window fr))
          ...
          (edit-window-buffer-set! cur buf2)
          (edit-window-buffer-set! other buf1)
          (echo-message! echo "Windows transposed")))))

  ;;; --- Fold toggle at point ---

  (define (cmd-fold-toggle-at-point app)
           ...
           (defs (let loop ((ls lines) (n 1) (acc '()))
                   (if (null? ls) (reverse acc)
                     (let ((line (string-trim (car ls))))
                       (loop (cdr ls) (+ n 1)
                         (if (or (string-prefix? "(define " line)
                                 (string-prefix? "(define* " line)
                                 (string-prefix? "(define-record-type " line)
                                 (string-prefix? "(define-syntax " line)
  ...
  (define (cmd-titlecase-region app)
  ...
  (define (cmd-goto-matching-bracket app)
  ...
  (define (cmd-toggle-block-comment app)
  ...
  (define (cmd-move-to-window-center app)
  ...
  (define (cmd-reverse-region-chars app)
  ...
  (define *relative-line-numbers* #f)

  (define (cmd-toggle-relative-line-numbers app)
  ...
  (define (cmd-toggle-cua-mode app)
  ...
  (define (cmd-exchange-dot-and-mark app)
  ...
  (define (cmd-sort-paragraphs app)
  ...
  (define (cmd-insert-mode-line app)
  ...
  (define (cmd-push-mark-command app)
  ...
  ;;; =========================================================================
  ;;; Batch 35: auto-complete, which-function, line-numbers, selective-display
  ;;; =========================================================================

  (define *global-auto-complete-mode* #f)
  (define *which-function-mode* #f)
  (define *display-line-numbers-mode* #t)
  (define *selective-display-level* #f)
  (define *global-font-lock-mode* #t)
  (define *auto-dim-other-buffers* #f)
  (define *global-eldoc-mode* #t)
  (define *word-wrap-column* 80)

  (define (cmd-toggle-global-auto-complete app)
  ...
  (define (cmd-toggle-which-function app)
  ...
  (define (cmd-toggle-display-line-numbers app)
  ...
  (define (cmd-toggle-selective-display app)
  ...
  (define (cmd-toggle-global-font-lock app)
  ...
  (define (cmd-insert-register-content app)
  ...
  (define (cmd-insert-date-iso app)
  ...
  (define (cmd-toggle-word-wrap-column app)
  ...
  (define (cmd-clone-indirect-buffer app)
  ...
  (define (cmd-toggle-auto-dim-other-buffers app)
  ...
  (define (cmd-toggle-global-eldoc app)
  ...
  (define (cmd-open-line-below app)
  ...
  (define *recentf-mode* #t)
  (define *savehist-mode* #t)
  (define *winner-mode* #t)
  (define *midnight-mode* #f)
  (define *global-undo-tree* #f)
  (define *diff-hl-mode* #f)
  (define *volatile-highlights* #f)
  (define *vertico-mode* #f)
  (define *marginalia-mode* #f)

  (define (cmd-toggle-desktop-save-mode app)
  ...
  (define (cmd-toggle-recentf-mode app)
  ...
  (define (cmd-toggle-savehist-mode app)
  ...
  (define (cmd-toggle-winner-mode app)
  ...
  (define (cmd-toggle-midnight-mode app)
  ...
  (define (cmd-toggle-global-undo-tree app)
  ...
  (define (cmd-toggle-diff-hl-mode app)
  ...
  (define (cmd-toggle-volatile-highlights app)
  ...
  (define (cmd-toggle-vertico-mode app)
  ...
  (define (cmd-toggle-marginalia-mode app)
  ...
  (define *global-cwarn* #f)
  (define *global-hideshow* #f)
  (define *global-abbrev* #t)
  (define *global-diff-auto-refine* #t)
  (define *global-eldoc-box* #f)
  (define *global-flyspell-lazy* #f)
  (define *global-so-clean* #f)

  (define (cmd-toggle-global-cwarn app)
  ...
  (define (cmd-toggle-global-hideshow app)
  ...
  (define (cmd-toggle-global-abbrev app)
  ...
  (define (cmd-toggle-global-diff-auto-refine app)
  ...
  (define (cmd-toggle-global-eldoc-box app)
  ...
  (define (cmd-toggle-global-flyspell-lazy app)
  ...
  (define (cmd-toggle-global-so-clean app)
